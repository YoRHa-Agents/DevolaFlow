...
class TestCascadeGateFieldsValidator:
    """v11.1.0 PV-04 — soft validator coverage for ``validate_cascade_gate_fields``.

    Branch matrix (per ``src/devolaflow/gate/scorer.py`` lines 137-216):

    * ``gate_block is None`` → empty list (early return).
    * ``cascade_required`` falsy / missing → empty list (no cascade
      required by the dispatch).
    * ``cascade_required: True`` + ``cascade_min_layers: 4`` +
      ``actual_layers >= cascade_min_layers`` → empty list (PASS).
    * ``cascade_required: True`` + ``actual_layers <
      cascade_min_layers`` → 1 warning containing ``cascade depth
      violation``.
    * ``cascade_required`` non-bool truthy → warning containing
      ``must be bool``.
    * ``cascade_min_layers`` non-int (or bool, by explicit
      ``isinstance(..., bool)`` short-circuit) → warning containing
      ``must be int``.

    All assertions check substring presence rather than exact-match
    formatting so future PV-05 message refinements that preserve the
    canonical phrase remain green by construction.
    """

    def test_validate_cascade_gate_fields_none_block_returns_empty(self) -> None:
        """``gate_block=None`` short-circuits to an empty warning list."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields

        assert validate_cascade_gate_fields(None) == []

    def test_validate_cascade_gate_fields_missing_cascade_required_returns_empty(self) -> None:
        """Missing ``cascade_required`` key is canonical absence-as-default — no warnings."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields

        assert validate_cascade_gate_fields({"coverage": 90}) == []

    def test_validate_cascade_gate_fields_falsy_cascade_required_returns_empty(self) -> None:
        """``cascade_required: False`` is canonical opt-out — no warnings."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields

        assert validate_cascade_gate_fields({"cascade_required": False}) == []

    def test_validate_cascade_gate_fields_passes_when_4_layers_dispatched(self) -> None:
        """Cascade satisfied (actual_layers == cascade_min_layers) → empty warning list."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields

        warnings = validate_cascade_gate_fields(
            {"cascade_required": True, "cascade_min_layers": 4},
            actual_layers=4,
        )
        assert warnings == []

    def test_validate_cascade_gate_fields_warns_when_collapsed_to_l3(self) -> None:
        """Collapsed-to-L3 dispatch (actual_layers < cascade_min_layers) emits a soft warning."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields
        ...
        assert "cascade depth violation" in warnings[0]

    def test_validate_cascade_gate_fields_warns_on_invalid_bool_type(self) -> None:
        """Non-bool truthy ``cascade_required`` surfaces a ``must be bool`` warning."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields

        warnings = validate_cascade_gate_fields({"cascade_required": "yes"})
        assert any("must be bool" in w for w in warnings)

    def test_validate_cascade_gate_fields_warns_on_invalid_int_type(self) -> None:
        """Non-int ``cascade_min_layers`` (e.g. string) surfaces a ``must be int`` warning."""
        from devolaflow.gate.scorer import validate_cascade_gate_fields

        warnings = validate_cascade_gate_fields(
            {"cascade_required": True, "cascade_min_layers": "four"},
        )
        assert any("must be int" in w for w in warnings)
