...
V6_BASELINE_PATH = BASELINES_DIR / "v7.0.0_baseline.json"
...
class TestBaselineFile:
    ...
    def test_v6_baseline_exists(self) -> None:
        """v7.0.0_baseline.json must be present in baselines/ (regenerated each MAJOR cut)."""
    ...
    def test_runner_prefers_latest_baseline(self) -> None:
        """load_baseline() picks the v7.0.0 baseline over older files, falls back as needed."""
        newest = _newest_baseline_path()
        assert newest is not None
        assert newest.name == "v7.0.0_baseline.json", (
            f"Expected load_baseline() to prefer v7.0.0_baseline.json; got {newest.name}"
        )
        ...
        assert entry is not None, (
            ...
            "the v7.0.0 baseline — runner is still falling back to v2.1.0."
        )
        assert entry["composite"] > 0
