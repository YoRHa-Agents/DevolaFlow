...
class TestSiChipInCanonicalRegistry:
    ...
    def test_canonical_registry_loads_with_si_chip(self) -> None:
        """v9.5.0 PV-01: registry now has 4 plugins (nines, ui-pro, rtk, si-chip)."""
        ...
        assert plugin_ids == ["nines", "ui-pro", "rtk", "si-chip"], (
            f"v9.5.0 PV-01 contract: si-chip MUST be the 4th plugin, "
            f"appended at the END of the list (registry-order discipline). "
            f"Actual order: {plugin_ids!r}"
        )
