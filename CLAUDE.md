...
2. Compare with current version (5.1.0-pre).
...
4. If current, respond: "DevolaFlow v5.1.0-pre is the latest version."
...
**Reinforcement (v5.1+):** When Gate FAIL triggers a new convergence round, the dispatch
includes `applicable_rules.reinforcement` with mandates from the prior round's findings.
L3 Task Agents MUST address all reinforcement rules before other work.
