...
| `workspace-compact` | `control` | 任务目录内的非破坏式收缩：风险按生命周期独立建档、判决独立追加成账，已关闭的原件搬入归档并留下可校验的映射与摘要。 | `workspace-compact`, `risk-parking`, `judgment`, `compaction`, `mapping`, `digest`, `report-only` |
