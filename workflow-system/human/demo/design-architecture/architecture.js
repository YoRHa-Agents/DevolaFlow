...
(function () {
  ...
  var translations = {
    en: {
      ...
      'arch.flow.authoring.body': 'SKILL defines operator-facing orchestration; the manifest owns the 27-reference install set; the registry owns 24 checklist seeds; context profiles own 5K / 5K / 8K layer budgets; schemas own message shapes; and .rules/*.mdc owns governance prose.',
      'arch.inventory.summary': 'Browse 27 references + 6 knowledge files',
      ...
      'arch.inventory.references': 'Tier-2 references · 27',
      ...
      'arch.flow.compiled.body': 'The rule compiler renders .rules/ into AGENTS.md, the on-demand Cursor repo-governance surface, and the Style reference. The install manifest selects SKILL, 27 references, and examples for each host profile. These outputs are generated views, not editable rule sources.',
    ...
    zh: {
      ...
      'arch.flow.authoring.body': 'SKILL 定义面向操作者的编排；manifest 拥有 27 个参考文件的安装集合；registry 拥有 24 个清单种子；context profiles 拥有 5K / 5K / 8K 分层预算；schemas 拥有消息形状；.rules/*.mdc 拥有治理规则正文。',
      'arch.inventory.summary': '浏览 27 个参考 + 6 个知识文件',
      ...
      'arch.inventory.references': '二级参考 · 27',
      ...
      'arch.flow.compiled.body': '规则编译器把 .rules/ 渲染为 AGENTS.md、按需加载的 Cursor repo-governance 表面和 Style 参考文档。安装 manifest 为各宿主选择 SKILL、27 个参考与示例。这些输出是生成视图，不是可编辑的规则源。',
      ...
      'arch.flow.runtime.body': 'TemplateRegistry.load_seed(name) 把 24 个种子之一转成清单分解知识。种子的 source_stages 字段只记录来源，不安排工作。TemplateRegistry.load_template("change-driven") 加载 templates/builtin/change-driven.yaml；它是唯一可执行生命周期，负责签署预检、有边界清单轮次、证据门控与归档。',
