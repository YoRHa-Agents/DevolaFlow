...
(function () {
  ...
  var TRANSLATIONS = {
    en: {
      ...
      'landing.whatsNew.v910':     'What\'s New',
      'landing.whatsNew.v910.desc': 'v9.1.0 — MINOR closure of `.local/` and `.rules/` utilization gaps when DevolaFlow runs in an independent clean context. Three rule-drift channels silently decayed before v9.1.0: doc-vs-code lies (`.rules/soul.mdc` §S-9 cited enforcement files that did not exist on disk), a lifecycle activation gap (`devola-init local` never auto-compiled `.rules/` sources into `.cursor/rules/repo-governance.mdc` / `AGENTS.md`), and a discoverability gap (`CLAUDE.md` / `README.md` / `Makefile` had no pointer to the edit-then-recompile workflow). v9.1.0 closes 12 of 18 catalogued gap items (G-001..G-004 + G-007..G-011 + G-013..G-014 + G-016). Soul Rule S-9 handoff envelope append-only enforcement now lands on disk via the new `tests/test_handoff_envelope_immutable.py` (203 LOC, 3 tests) and the new `src/devolaflow/lifecycle/check_envelope_append_only.py` lifecycle hook (150 LOC, CEA001 blocker on path collision; `DEFAULT_EVENTS` 6 → 7 via A-2.4-compliant append at position 7). `devola-init local` auto-compiles `.rules/` into the compiled governance corpus immediately, with `--no-compile` opt-out for legacy flows. New Makefile targets `compile-rules` + `check-rules-drift` wire into `make all` and `make release-preflight`. `.rules/index.md` token-budget table corrected (cursor 8000 → 12000, agents_md 6000 → 12000) and locked against future drift by `tests/test_rules_index_accuracy.py` and `tests/test_local_layer_completeness.py`. W-16 wholesale baseline regen covers all 53 scenarios with 0 regressions > 5pp vs v9.0.0. W-9 / SI-10 6/6 PASS with 3457 tests. Rules layer unchanged at 58; Soul-set frozen at 10 per W-21.',
    ...
    zh: {
      ...
      'landing.whatsNew.v910':     '最新更新',
      'landing.whatsNew.v910.desc': 'v9.1.0 — MINOR：修复 DevolaFlow 在独立 clean context 下 `.local/` 与 `.rules/` 利用不足的缺口。在 v9.1.0 之前存在三条规则漂移通道：文档与代码对不上（`.rules/soul.mdc` §S-9 引用的落盘强制文件实际并不存在）、生命周期激活缺口（`devola-init local` 从未自动把 `.rules/` 源编译进 `.cursor/rules/repo-governance.mdc` / `AGENTS.md`）、以及可发现性缺口（`CLAUDE.md` / `README.md` / `Makefile` 均未指向"编辑后重新编译"的工作流）。本版本关闭 18 个已编目缺口中的 12 个（G-001..G-004 + G-007..G-011 + G-013..G-014 + G-016）。Soul 规则 S-9（handoff envelope 追加不可变）的强制检查首次真正落盘：新增 `tests/test_handoff_envelope_immutable.py`（203 行，3 个测试）在 `HandoffStore` API 边界处固定该不变式；新增 `src/devolaflow/lifecycle/check_envelope_append_only.py` lifecycle hook（150 行，路径冲突触发 CEA001 blocker；`DEFAULT_EVENTS` 6 → 7，按 A-2.4 规则在位置 7 追加）。`devola-init local` 现在会在种下 `compile-config.yaml` 之后自动调用 `RuleCompiler.compile_all()`，新仓库开箱即带编译好的治理规则集（`--no-compile` 保留旧流程）。Makefile 新增 `compile-rules` 与 `check-rules-drift` 目标，已并入 `make all` 与 `make release-preflight`。`.rules/index.md` token 预算表修正（cursor 8000 → 12000，agents_md 6000 → 12000），并由 `tests/test_rules_index_accuracy.py` 与 `tests/test_local_layer_completeness.py` 固定防止再次漂移。W-16 整盘重算基线覆盖 53 个场景，相对 v9.0.0 回退 > 5pp 的场景为 0。W-9 / SI-10 六项全部 PASS，3457 条测试通过。规则层保持 58 条不变；Soul 集合按 W-21 冻结在 10 条。',
