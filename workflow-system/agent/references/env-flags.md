...
* **unwired** rows — a documented name with NO read site on ANY
  ...
  marked `**Read surface** | unwired`). The one v14.2.2-audit instance
  (§2.5) was RETIRED at v15.2.0 B-6; its row remains as a tombstone
  (keeps §-numbering stable) and stays marked unwired.
...
| Field | Value |
|---|---|
| **Status** | **RETIRED** — the wiring-or-removal decision deferred at v14.4.0 (G-023) resolved to REMOVAL at v15.2.0 B-6 (dependency suggestion-ization). The v14.2.2 G-024 AST audit had already established NO `src/devolaflow/` env-read site ever existed for this name; the flag was a documentation ghost from v8.3.0 PV-01. |
| **Read surface** | unwired — retired tombstone; NO env read ever existed and none may be added under this name (author a new W-20-justified flag instead) |
| **Live control surface** | `runtime-plugins.yaml#defaults.auto_install` (a YAML knob, NOT an env var — flipped `true` → `false` at v15.2.0 B-6: bare `ensure_plugin(pid)` calls probe-and-report instead of network-installing) + the `ensure_plugin(auto_install=...)` parameter (explicit opt-in surfaces — the §2.12 hooks and `devola-init --global` bundling — pass `True`) |
| **Section number** | kept as a tombstone so §2.6..§2.17 cross-references throughout the corpus stay stable |
...
| **Effect when active** | Per Architecture rule A-6: when complexity ≥ STANDARD, L0 MUST scaffold `.local/.agent/active/<id>/` and materialize the four drafts (`entrance.md`, `goal.md`, `checklist.md`, `preflight.md`) plus the complete first-batch artifact set (`stage.md`, `spec.md`, `STATUS.yaml`, `owned_files.txt`, and `evidence/`) before dispatching the first L1 wave; the same surface is reused by `pre_handoff`, `/devola:propose`, and the `file_write` / `task_stop` runtime adapters. Since v17.0.0 R4 the flag is also REUSED (W-20 — same workspace-engagement activation surface, no new flag) by the session-start resume adapter `python -m devolaflow.hostbridge resume` (Cursor `sessionStart` / Claude `SessionStart` hooks): flag off → empty stdout, exit 0, zero filesystem IO; see `references/host-bridges.md` §8 |
