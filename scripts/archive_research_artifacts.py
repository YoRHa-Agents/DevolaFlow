...
* **``--misc``** — archives the NON-versioned ``.local/research/`` root
  files (name not matching ``v<digit>``) into the single flat
...
def _write_misc_readme(
    ...
    lines = [
        ...
        "`.local/research/` root artifacts (file name not matching `v<digit>`) —",
...
def archive_misc(*, dry_run: bool = False, refresh: bool = False) -> int:
    ...
    Gathers root-level FILES whose name does not match ``v<digit>``
    ...
    print(f"  source: {research_dir.relative_to(root)} (root files not matching 'v<digit>')")
    print(f"  dest:   {archive_dir.relative_to(root)}")
    if dry_run:
        print("  (dry run — no files will be copied)\n")
    else:
