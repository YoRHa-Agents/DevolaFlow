...
**Zero breaking changes.** Every change in the entire 5-PATCH + 1-MINOR cycle is additive:

- **2 NEW Python APIs** (both library-only / dispatch-surface; backward-compat preserved):
  - `devolaflow.feedback.dispatch_dogfood_cycle(workflow_name, *, skill_files=None, runs_dir=None, baseline_dir=None, threshold=0.10, work_dir=None) -> SiChipResult` (v10.2.1; PV-02; D-S-2 closure) — thin delegation wrapper that bridges the L0/L1 workflow-stage surface to `devolaflow.si_chip_bridge.runner.run_dogfood_cycle`. P1 dispatcher-not-implementer preserved (the wrapper enqueues; an L3 task agent runs the actual cycle).
  - `devolaflow.si_chip_bridge.install_resolver.read_installed_si_chip_version(install: SiChipInstall) -> str | None` (v10.2.0; PV-01; D-P-3 closure) — parses the installed SKILL.md frontmatter `version:` field; replaces the hardcoded `echo si-chip/0.4.0` heuristic.
- **1 NEW Python script** — `scripts/nines_to_sichip_eval_adapter.py` (412 LOC, 23 unit tests; v10.2.2; PV-03; D-N-1 closure) — converts a NineS V3.3.0 `self-eval` JSON output into Si-Chip's `runs-dir`/`baseline-dir` layout so `aggregate_eval.py` can compute `iteration_delta` against NineS-derived task scores WITHOUT requiring fresh real-LLM eval runs. Closes the v9.5.0 OA-1 blocker.
- **1 NEW Makefile target** — `iteration-delta-gate` (v10.2.1; PV-02; D-S-3 / D-V-1 wire) — invokes `tests/test_sichip_iteration_delta_gate.py`; wired into `release-preflight` as the 7th SI-10 step for the cycle's duration. Telegraphed for v10.4.0 to formalize as W-9 step #7.
- **1 NEW MVP-8 nested-key contract** — `MetricsReport.from_yaml_dict` (v10.2.3; PV-04 bridge defect fix) now reads MVP-8 nested keys (`metrics.task_quality.T1_pass_rate`, `summary.baseline_delta`, `metrics.context_economy.{C1_metadata_tokens,C2_body_tokens}`) with legacy top-level shape preserved as backward-compat fallback. UNBLOCKS the iteration_delta machinery — pass #3 + pass #4 both APPLY at +0.9.
- **3 mechanical CC reductions** (radon B-rated; behaviour byte-identical via private-helper extractions):
  - `pre_plugin_invocation` 18 → ≤10 (v10.2.3; PV-04 Track B-1) via `_resolve_upgrade_threshold_hours` + `_run_install_then_upgrade_for_plugin` helpers.
  - `post_skill_edit` 13 → ≤7 (v10.2.3; PV-04 Track B-2) via `_compute_fingerprint` + `_load_existing_fingerprints` + `_run_si_chip_evaluation` helpers.
  - `installer.read_last_checked` 15 → 8 (v10.2.4; PV-05 round 2) via `_parse_log_event_timestamp` helper + `_LAST_CHECKED_SUCCESSFUL_EVENTS` module-level constant.
- **1 NEW dedup behaviour** — `post_skill_edit._write_feedback_doc` (v10.2.1; PV-02; D-S-5 closure) — SHA-256-fingerprints the `(sorted(skill_files), sorted(notes), verdict)` triple and dedupes against an append-only sidecar at `.local/feedbacks/.sichip_deferred_fingerprints.txt`. Identical inputs return the prior path; distinct inputs write a new dated doc. Idempotent — repeats are no-ops.
- **1 NEW daily-upgrade integration** — `pre_plugin_invocation` (v10.2.1; PV-02; D-P-2 BLOCKER closure) — auto-fires `upgrade_plugin(plugin_id)` after `ensure_plugin` succeeds when `is_plugin_stale(plugin_id, threshold_hours=24)` returns True. **REUSES** the existing `DEVOLAFLOW_AUTO_INSTALL_PLUGINS=1` env flag per W-20 §3 reuse-first analysis (the activation surface is the same — "auto-manage plugin lifecycle" — so a NEW flag would violate the orthogonality test).
- **0 NEW env flags** in the entire cycle (W-20 §3 reuse-first applied — D-P-2 + D-P-5 both reuse `DEVOLAFLOW_AUTO_INSTALL_PLUGINS`). The 5 PRE-existing v10.0.0-cycle env flags (`DEVOLAFLOW_SIMPLE_SHORTCUT`, `DEVOLAFLOW_AUTO_INSTALL_PLUGINS`, `DEVOLAFLOW_SI_CHIP_DEEP`, `DEVOLAFLOW_WARMUP`, `DEVOLAFLOW_AGENT_WORKSPACE`) preserve byte-stable behaviour.
- **0 schema bumps** — schema v6 / 17-position canonical_order byte-stable across all 10 historical multi-baseline byte tests (v7.0.0 → v10.2.0).
- **First APPLY iteration_delta in DevolaFlow history** — Si-Chip dogfood pass #3 (PV-04, post-bridge-fix) + pass #4 (PV-05, round-2 persistence) both APPLY at `iteration_delta = +0.9000` across all 4 probed files (`SKILL.md`, `references/env-flags.md`, `references/shell-proxy.md`, `references/agent-workspace.md`). The v9.5.0 OA-1 blocker is resolved end-to-end.

### Cumulative cycle deliverables (per-PV ledger)
...
| PV-03 | `chore(v10.2.2)` | `d64cca2` | NineS deep self-analysis + Si-Chip eval adapter prototype | D-N-1, D-N-3 |
| PV-04 | `chore(v10.2.3)` | `1876fda` | Round 1 — bridge defect fix unlocks +0.9 iteration_delta + 2 CC reductions | bridge_defect (NEW), pre_plugin_invocation_cc18, post_skill_edit_cc13 |
...
| PV-06 | `chore(v10.3.0)` | (this) | Cycle close MINOR + retrospective + W-19 archive + human-docs refresh + PR posture | retrospective, evaluation, cycle_archive, human_docs, pr_posture |

### Headline numbers (cycle-cumulative; v10.0.0 → v10.3.0)

| Area | v10.0.0 baseline | v10.3.0 | Delta |
|------|---:|---:|---:|
| Tests | 3929 | **4091** | +162 (cycle-cumulative; +94 raw NEW test functions including this PV's W-18 lint + parametrize expansions over fixtures) |
| Coverage | 93.13% | **93.04%** | -0.09pp (within ±0.50pp noise; CP-2 80% floor + 13.04pp margin) |
| Plugins registered | 4 | 4 | unchanged (cycle deepened plugin infra via deep review) |
| Lifecycle events | 10 | 10 | unchanged (no new hooks) |
| Si-Chip dogfood verdict | DEFER (v9.5.0) | **APPLY** (passes #3 + #4 = +0.9 each) | **first APPLY in DevolaFlow history** |
| iteration_delta CI gate | not in CI | **wired as 7th SI-10 step** | NEW (`Makefile::release-preflight`) |
| NineS-to-Si-Chip eval adapter | n/a | **412 LOC + 23 tests** | NEW (`scripts/nines_to_sichip_eval_adapter.py`) |
| Bridge MVP-8 nested-key support | top-level keys ONLY | MVP-8 nested + legacy fallback | NEW (`MetricsReport.from_yaml_dict`) |
| Schema bumps | 0 | 0 | unchanged (v6 / 17-position canonical_order byte-stable) |
| New env flags | 4 (cycle-cumulative @ v10.0.0) | 4 + **0** | unchanged (W-20 §3 reuse-first; D-P-2 reuses `DEVOLAFLOW_AUTO_INSTALL_PLUGINS`) |
| Soul-set count | 10 | 10 | unchanged (W-21 freeze; S-11 candidate evaluated OUT, re-telegraphed v10.4.0) |
| NineS overall composite | 0.907332 | **0.906924** | -0.000408 (within ±0.001 noise floor; byte-stable) |
| Multi-baseline byte test pins | 9 (v7.0.0 → v9.7.0) | **10** | +1 (v10.2.0 added at PV-01 W-16 wholesale) |
| W-3 SI-3 composite | 9.20 (v10.0.0 MAJOR) | **9.385** | +0.185 (margin +0.385 over STRICT MINOR-cycle-close ≥9.0) |

### What landed (per PV summary)

- **PV-01 (v10.2.0; SHA `4627b9b`) — Plugin deep review + W-16 wholesale baseline regen.** 4 D-P-* gaps closed (D-P-1 end-to-end `refresh_all` e2e against the real subprocess pipeline; D-P-3 si-chip `version_check_cmd` swap from hardcoded `echo si-chip/0.4.0` to a real SKILL.md frontmatter probe via `read_installed_si_chip_version`; D-P-4 registry-walk smoke pinning all 4 plugins; D-P-6 never-installed staleness pin). 22 NEW test functions (within W-17 +30/PV cap). W-16 wholesale `v10.2.0_baseline.json` byte-identical to `v9.7.0_baseline.json` (zero drift since v10.0.0 MAJOR). 10th multi-baseline pin via `layout_invariant_v10.2.0.yaml`.
- **PV-02 (v10.2.1; SHA `3b8484c`) — Formal Si-Chip integration + 7-step SI-10 + daily-upgrade scheduler.** 5 gaps closed (D-S-2 + D-S-3 + D-S-5 + D-S-6 + D-P-2 BLOCKER). NEW `dispatch_dogfood_cycle` wrapper at L0/L1 dispatch surface. NEW `iteration_delta` CI gate wired as 7th SI-10 step in `Makefile::release-preflight`. NEW DEFER feedback doc dedup via SHA-256 fingerprint sidecar. `run_dogfood_cycle.work_dir` defaults to `__version__`-tracking path. Daily-upgrade integration in `pre_plugin_invocation` REUSES `DEVOLAFLOW_AUTO_INSTALL_PLUGINS` (W-20 §3 reuse-first). 29 NEW test functions.
- **PV-03 (v10.2.2; SHA `d64cca2`) — NineS deep self-analysis + Si-Chip eval adapter prototype.** 2 gaps closed (D-N-1 + D-N-3). NEW `scripts/nines_to_sichip_eval_adapter.py` (412 LOC, 23 unit tests; verdict APPROVE — R-1 did NOT fire). 3 NineS deep-analyses on `src/devolaflow/si_chip_bridge/`, `src/devolaflow/plugins/`, `src/devolaflow/lifecycle/` (62 findings, 10 complexity warnings — 3 actioned in PV-04 + PV-05; 7 deferred to v10.3.x / v10.4.0). Dogfood pass #2 surfaces NEW bridge defect (deterministic; flagged for PV-04). 24 NEW test functions.
- **PV-04 (v10.2.3; SHA `1876fda`) — Self-iteration round 1.** Bridge defect FIX in `MetricsReport.from_yaml_dict` (MVP-8 nested-key support; legacy fallback preserved) + 2 mechanical CC reductions (`pre_plugin_invocation` 18 → ≤10, `post_skill_edit` 13 → ≤7) per PV-03 NineS hotspots. Dogfood pass #3 verdict APPLY with `iteration_delta = +0.9` across all 4 probed files — first APPLY in DevolaFlow history. 14 NEW test functions.
- **PV-05 (v10.2.4; SHA `4224555`) — Self-iteration round 2 + W-17 mid-cycle audit + W-8 stagnation predicate.** 1 mechanical CC reduction (`installer.read_last_checked` 15 → 8 via `_parse_log_event_timestamp` extraction) + dogfood pass #4 (+0.9 byte-identical to pass #3 — round-1 effects persist post-edit). W-8 stagnation predicate `(round_count >= 2) AND (last_2_rounds_iteration_delta < +0.05)` evaluated FALSE → CONTINUE (no escalation). W-17 mid-cycle audit at PV-05 verdict GREEN (cumulative 93 / 150 cap; 62.0%). 5 NEW test functions (including W-18 lint).
- **PV-06 (v10.3.0; this commit) — Cycle close MINOR.** Canonical 7 sync 10.2.4 → 10.3.0; CHANGELOG cycle-close entry; demo landing v10.3.0 What's New section + 3 highlight cards; `versions.json` v10.3.0 entry (63rd); README "What's New in v10.3.0 (MINOR cycle close)" section; benchmark-results SAMPLE_DATA bumped to 10.3.0 (canonical 7 sync #8); 16 EN/ZH user guides regenerated via `make sync-human-docs`; W-3 SI-3 evaluation composite **9.385/10** (margin +0.385 over STRICT MINOR-cycle-close ≥9.0); W-7 SI-8 retrospective with 4 mandatory sections + 8 explicit deferrals + 7 key learnings; W-2 SI-2 NineS cycle-close self-eval (overall 0.906924; byte-stable vs v10.0.0 0.907332); W-19 cycle archive at `docs/cycle-archive/v10.3.0/` (24 files).

### New env flags

**0 NEW env flags** in the entire cycle. Per W-20 §3 reuse-first analysis, both D-P-2 (daily-upgrade scheduler) and D-P-5 (`DEVOLAFLOW_UPGRADE_FREQUENCY_HOURS` integer override) REUSE the v9.4.0 `DEVOLAFLOW_AUTO_INSTALL_PLUGINS=1` activation surface (same activation surface = "auto-manage plugin lifecycle" — orthogonality test fails for any new flag).

### Schema bumps

**0 schema bumps** in the entire cycle. Schema v6 (17-position `canonical_order`; set in v9.7.0 PV-02) byte-stable across all 10 historical multi-baseline byte tests (v7.0.0 → v10.2.0). PV-01 added the 10th baseline witness via W-16 wholesale regen.

### New modules / NEW CLIs / NEW Python APIs (cycle-cumulative)

- `scripts/nines_to_sichip_eval_adapter.py` (NEW; v10.2.2; PV-03; 412 LOC) — NineS V3.3.0 self-eval JSON → Si-Chip `runs-dir`/`baseline-dir` layout adapter. 7 public functions (`load_nines_json`, `validate_nines_shape`, `build_runs`, `build_baselines`, `write_runs_dir`, `write_baseline_dir`, `main`). Two modes: `synthetic` (baseline pass_rate=0) and `sample` (baseline scores from a second NineS JSON for cycle-over-cycle comparison).
- `devolaflow.feedback.dispatch_dogfood_cycle()` (NEW Python API; v10.2.1; PV-02) — thin delegation wrapper at the L0/L1 workflow surface (P1 dispatcher-not-implementer preserved).
- `devolaflow.si_chip_bridge.install_resolver.read_installed_si_chip_version()` (NEW Python API; v10.2.0; PV-01) — parses the installed SKILL.md frontmatter `version:` field (replaces the hardcoded heuristic).
- 5 NEW lifecycle private helpers (cycle-cumulative): `_resolve_upgrade_threshold_hours`, `_run_install_then_upgrade_for_plugin` (PV-04 Track B-1; `pre_plugin_invocation`); `_compute_fingerprint`, `_load_existing_fingerprints`, `_run_si_chip_evaluation` (PV-04 Track B-2; `post_skill_edit`).
- 1 NEW plugins/installer private helper: `_parse_log_event_timestamp` + `_LAST_CHECKED_SUCCESSFUL_EVENTS` module-level constant (PV-05; `installer.py`).
- 1 NEW Makefile target: `iteration-delta-gate` (PV-02; wired into `release-preflight` as 7th SI-10 step).

### Cycle hygiene

- **W-1 SI-1 gap analysis** at `.local/research/v10.2.0_gap_analysis.md` — **24 gaps enumerated** across 6 lattices (4 BLOCKER + 7 CRITICAL + 9 MAJOR + 4 MINOR); 5 BLOCKERs closed cycle-cumulative; deferrals documented in retrospective §3.
- **W-2 SI-2 NineS deep-analysis** ran in PV-03 on 3 packages (62 findings, 10 complexity warnings); cycle-close NineS self-eval at `.local/research/v10.3.0_nines.{json,md}` (overall composite 0.906924; vs v10.0.0 archive 0.907332; delta -0.000408 within ±0.001 noise floor; byte-stable; A-2 `structure_recognition` axis preserved at 1.0).
- **W-3 SI-3 evaluation** composite **9.385/10** (margin +0.385 over the **STRICT MINOR-cycle-close ≥9.0** floor); per-dimension: code quality 9.40, architecture 9.30, test adequacy 9.20, maintainability 9.30, compatibility 10.00, performance 9.40. Full evaluation at `.local/research/v10.3.0_evaluation.md`.
- **W-4 SI-4 benchmark guard** — composite scores stable across all 53 EvoBench scenarios at every PV in the cycle (within ±2% of `v10.2.0_baseline.json` baseline byte-identical to v9.7.0); 10-baseline byte test green.
- **W-7 SI-8 retrospective** at `.local/research/v10.3.0_retrospective.md` — 4 mandatory sections (§1 gaps + §2 implemented + §3 deferred-with-why + §4 key learnings) + 3 supplementary (§5 telegraph for v10.4.0 + §6 cycle-cumulative metrics + §7 references). 8 explicit deferrals; 7 key learnings.
- **W-9 SI-10 7-gate green** at every commit in the cycle: pytest / ruff check / ruff format / test_version / test_benchmarks / make check-cursor-skill PLUS the cycle-wide D-V-1 7th step (Si-Chip iteration_delta gate). The 7-step protocol holds for the cycle's duration; telegraphed for v10.4.0 to formalize as W-9 step #7.
- **W-16 wholesale baseline regen** at PV-01 cycle-start — `v10.2.0_baseline.json` + `layout_invariant_v10.2.0.yaml` byte-identical to v9.7.0 (zero drift since v10.0.0 MAJOR; cleanest possible cycle-start).
- **W-17 cycle-cumulative test cap** — **+93 / +150** (62% utilisation; well within cap). Per-PV deltas declined monotonically from PV-02 onward (29 → 24 → 14 → 5). High-information disposition per `v10.2.4_w17_mid_cycle_audit.md` §4 — every test pins a distinct contract surface; no redundant scaffolding. Mid-cycle audit at PV-05 verdict **GREEN**.
- **W-18 ghost-audit refresh** — 6 W-18 lints across the cycle (one per PV), each authored BEFORE the corresponding CHANGELOG entry per the W-18 precondition. PV-06 lint at `tests/test_no_ghost_features.py::test_v10_3_0_new_symbols_have_coverage` pins 10 distinct surface elements (README "What's New" header, demo landing i18n marker, versions.json entry, benchmark-results SAMPLE_DATA, evaluation doc, retrospective doc, NineS json + md, W-19 cycle archive directory + ≥10 file count, CHANGELOG header + W-3 SI-3 composite literal "9.385").
- **W-19 cycle archive** — `docs/cycle-archive/v10.3.0/` committed in this PV via `python scripts/archive_research_artifacts.py 10.3.0 --extra-prefix v10.2.` (25 files spanning all 5 PATCH PVs + the v10.3.0 cycle-close artifacts: gap_analysis (1), retrospective (1), evaluation (1), nines (9), other (12 incl. cycle plan + 4 dogfood passes + 2 iteration round reports + W-17 audit + W-8 stagnation check + nines self-eval source + cycle-research stderr captures), README.md auto-generated index (1)).
- **W-20 env flag policy** — **0 NEW env flags** in the cycle. D-P-2 + D-P-5 reuse `DEVOLAFLOW_AUTO_INSTALL_PLUGINS` per W-20 §3 reuse-first (orthogonality test fails for any new flag covering the same activation surface).
- **W-21 Soul-set freeze** — Soul count remains at **10** (W-21 cap). No S-11 candidate landed in v10.2.0 cycle. v10.0.0-telegraphed S-11 "Parallel Wave Dispatch Invariant" candidate evaluated OUT in gap analysis §3.6 D-W-1 (orthogonal to user mandate); re-telegraphed for v10.4.0 (cycle N+2 from v10.2.0) per W-21 2-cycle deliberation rule. Retrospective §5 carries the telegraph.
- **A-2 frozen prefix invariant** — multi-baseline byte test green at every commit; positions 1-12 byte-stable since v7.0.0; 10 baselines pinned (v7.0.0 → v10.2.0).
- **PR posture** — `feat/v10.2.0-cycle` (carrying all 6 PV commits) flipped from draft → ready-for-review at this commit. Operator merges into `main` per protected-branch workflow (S-6).

### Files Changed (this commit; v10.3.0 PV-06 cycle close)

| Op | Path | Notes |
|---|---|---|
| EDIT | `src/devolaflow/__init__.py` | canonical 7 sync #1 — `__version__ = "10.3.0"` |
| EDIT | `pyproject.toml` | canonical 7 sync #2 |
| EDIT | `workflow-system/agent/SKILL.md` | canonical 7 sync #3 (frontmatter + banner + body "Current version:") — A-2 frozen prefix preserved |
| EDIT | `workflow-system/agent/workflow-skill.yaml` | canonical 7 sync #4 |
| EDIT | `scripts/generate_human_docs.py` | canonical 7 sync #5 — `SOURCE_VERSION = "10.3.0"` |
| EDIT | `tests/test_smoke.py` | canonical 7 sync #6 |
| EDIT | `README.md` | canonical 7 sync #7 (badge + version example) + NEW "What's New in v10.3.0 (MINOR cycle close)" section prepended |
| EDIT | `workflow-system/human/demo/benchmark-results/index.html` | canonical 7 sync #8 (`SAMPLE_DATA.version`) |
| EDIT | `workflow-system/human/demo/index.html` | NEW v10.3.0 What's New section with 3 highlight cards (`landing.whatsNew.v1030` i18n key) |
| EDIT | `workflow-system/human/demo/version-timeline/versions.json` | NEW 63rd entry — v10.3.0 (ST-7) |
| EDIT | `workflow-system/human/en/*.md` (8 files) | regenerated via `make sync-human-docs` — `source_version: "10.3.0"` (ST-3 + ST-4) |
| EDIT | `workflow-system/human/zh/*.md` (8 files) | regenerated in lock-step via `make sync-human-docs` — ST-3 bilingual completeness |
| EDIT | `tests/test_no_ghost_features.py` | NEW `test_v10_3_0_new_symbols_have_coverage` W-18 lint (10 surface pins) |
| EDIT | `CHANGELOG.md` | this entry |
| NEW | `.local/research/v10.3.0_nines.json` (gitignored; archived) | W-2 SI-2 NineS cycle-close self-eval raw report |
| NEW | `.local/research/v10.3.0_nines.md` (gitignored; archived) | W-2 SI-2 NineS synthesis (overall 0.906924; byte-stable vs v10.0.0 0.907332) |
| NEW | `.local/research/v10.3.0_nines.stderr` (gitignored; archived) | W-2 SI-2 stderr capture |
| NEW | `.local/research/v10.3.0_evaluation.md` (gitignored; archived) | W-3 SI-3 STRICT MINOR-cycle-close evaluation (composite 9.385/10) |
| NEW | `.local/research/v10.3.0_retrospective.md` (gitignored; archived) | W-7 SI-8 cycle-close retrospective (4 mandatory + 3 supplementary sections) |
| NEW | `docs/cycle-archive/v10.3.0/` | W-19 cycle archive — 25 files (24 copied + 1 auto-generated README index) via `archive_research_artifacts.py 10.3.0 --extra-prefix v10.2.` |
| NEW | `.local/.agent/handoff/L0__operator__v10.2.0-cycle-close__0001.yaml` (gitignored) | Cycle-close handoff envelope (operator-actionable) |
...
The scorer lives at `src/devolaflow/writing_style/`. It ports the PV-01 research probe into production code: feature extraction for ten stylometric dimensions, a composite score on a 0-to-100 scale calibrated against a hand-picked human-clean corpus (chezmoi, ruff, caveman, ironclaw READMEs), three tone profiles, and a region classifier that tells the humanizer which spans are safe to rewrite. The package is deliberately isolated from the dispatch hot path — `tests/test_writing_style_isolated.py` walks `task_adaptive_selector.py`, `compressor/`, `lifecycle/`, `feedback.py`, and `agent_workspace/dispatch_executor.py`, and asserts their transitive import graphs do not touch anything under `writing_style/`. The user mandate was to reduce AI-flavor without polluting performance. The scorer exists, and it does not run on any agent dispatch.

The transforms are `emdash`, `bullets`, `signposts`, `headers`, and `cliches`. Each one edits only prose regions. Fenced code, inline backticks, markdown links, version strings like v10.1.0, html tags, and bare URLs are preserved byte-for-byte. The em-dash transform carried most of the CHANGELOG gain: a document-scope density cap at 8 per 1000 words (the chezmoi / ruff / caveman human-clean mean) walks em-dashes in reading order, keeps the first N under budget, and demotes the rest to commas. The headers transform demotes orphan `###`-or-deeper sections shorter than 40 words to inline bold, falling back to plain text when the document is already bold-saturated.

Two operator-visible surfaces ship. `scripts/generate_human_docs.py` now humanizes the EN and ZH guides at generation time, with `--no-humanize` as an opt-out. The new `make humanize-docs` target covers the hand-edited surface (README, CHANGELOG entries). Both call `scripts/humanize_doc.py`, a three-subcommand CLI (`score`, `check`, `apply`) for inspecting and applying transforms on any markdown file.

The scoring is not breaking. Nothing on the dispatch hot path imports anything new. The frozen canonical layout prefix (A-2.1 positions 1 through 12) is untouched. The five new env flags from the v10.0.0 MAJOR stayed out of this cycle entirely; v10.1.0 adds zero env flags. The only behavioural change visible to users of the public Python API is that `devolaflow.writing_style` is now an importable subpackage with a documented public surface: `score_text`, `score_corpus`, `load_profile`, `apply_transforms`, and the dataclasses that describe them.

Research notes, the SI-1 gap analysis, the PV-05 measurement report, and the cycle retrospective live under `.local/research/v10.1.0_*` on the branch. They are gitignored per repo convention; W-19 archives the cycle research at MAJOR or MINOR cycle close into `docs/cycle-archive/v10.1.0/`.
...
The cycle ships three operator-visible defaults that downstream agents notice immediately:

| What changed | Default before v9.2.0 | Default in v9.2.0 | Opt-out path |
|---|---|---|---|
| SKILL.md teaches workspace discovery at session start | not taught | **§"Workspace Engagement (Read at Session Start)" + `scan_workspace` API** | n/a (text-side guidance — no opt-out) |
| `meta.agents_md_slice.enabled` | `false` (full AGENTS.md to every L3) | **`true` (sliced AGENTS.md per task_type)** | `export DEVOLAFLOW_AGENTS_MD_SLICE=0` |
| change-driven activation heuristic + auto_write_handoff lifecycle hook | not wired | **wired when `DEVOLAFLOW_AGENT_WORKSPACE=1`** | env-flag default-OFF; opt-in only |

**R5 strict byte-stable proof**: every NEW capability surface (PV-01 `scan_workspace` + PV-02 activation heuristic + PV-03 `auto_write_handoff` + PV-04 `consult_for_dispatch` + PV-05 `agents_md_slice` slice) produces byte-identical dispatch output when its env-flag is absent. Pinned by `tests/test_handoff_auto_write.py::test_env_flag_off_is_byte_identical_noop` + `tests/test_memory_consult_for_dispatch.py::test_env_flag_off_returns_empty_list` + `tests/test_pv07_agents_md_slice.py::test_agents_md_slice_env_flag_0_opts_out` + `tests/test_capability_e2e.py::test_standard_task_does_not_open_folder_without_env_flag`.

### Per-PV cycle rollup ledger

| PV | Version | Commit | Headline |
|---|---|---|---|
| **PV-01** | `v9.1.1` | `adb5e05` | NEW `scan_workspace(repo_root) -> WorkspaceContext` discovery API + SKILL.md §"Workspace Engagement (Read at Session Start)" + W-16 wholesale baseline regen → `benchmarks/devolaflow_context/baselines/v9.2.0_baseline.json`. +8 NEW tests. |
| **PV-02** | `v9.1.2` | `0318191` | NEW `src/devolaflow/skills/` package: `change_activation.py` (heuristic) + `slash_commands.py` (`/devola:propose|apply|verify|archive` CLI). NEW Architecture rule **A-6** "Workspace Engagement Auto-Activation" (rule count 58 → 59). +26 NEW tests. Closes M-007. |
| **PV-03** | `v9.1.3` | `1ed311e` | NEW `pre_handoff` lifecycle event (DEFAULT_EVENTS 7 → 8 APPENDED) + `lifecycle/auto_write_handoff.py` default handler. **Closes G-005** — `rg "write_envelope\(" src/devolaflow/` returns ≥ 2 hits (definition + first production caller). +25 NEW tests. |
| **PV-04** | `v9.1.4` | `e5f8132` | NEW `memory_router.cache::consult_for_dispatch` advisory hint surface + EXTEND `schemas/lean-dispatch.yaml` `change_context` (position 16) with 3 OPTIONAL NEST sub-fields (`prior_feedback_themes` / `memory_case_hits` / `source_of_truth_excerpt`). canonical_order length STAYS 16 / version STAYS 5. +14 NEW tests. Closes G-015. |
| **PV-05** | `v9.1.5` | `c404aa1` | NEW `agent_workspace/spec_bootstrap.py::seed_initial_spec` (M-004 closure) + `meta.agents_md_slice.enabled` flips false → true (M-007 closure; OPERATOR-VISIBLE behaviour change; R5 strict opt-out via `DEVOLAFLOW_AGENTS_MD_SLICE=0`). +10 NEW tests. |
| **PV-06** | **v9.2.0** | *this commit* | EXTEND `init_project.install_local` with `with_examples=True` kwarg + `_seed_example_artifacts` helper (3 worked-trace fixtures); NEW `tests/test_capability_e2e.py` (10 tests, the LARGEST new file — cycle headline lint); EXTEND `scripts/archive_research_artifacts.py` with `--extra-prefix` flag; W-19 cycle archive populated; W-7/SI-8 retrospective; CHANGELOG cycle rollup. +10 NEW tests. Closes G-006 (other half) + cycle rollup. |

### Highlights (this PV — PV-06)

- **EXTEND `src/devolaflow/init_project.py::install_local`** with `with_examples: bool = False` kwarg (default-OFF at the Python level; CLI default-ON for `mode: full`, default-OFF for `mode: core` per `_parse_with_examples` in `main()`). NEW `_seed_example_artifacts(cwd: Path)` helper seeds 3 worked-trace fixtures: (1) `.local/.agent/active/example-add-dark-mode/` with all 7 artifacts populated (`goal.md` ~106 tokens / `acceptance.md` ~187 / `spec.md` ~399 / `tasks.md` ~263 / `STATUS.yaml` ~71 / `owned_files.txt` ~29 / `README.md`); (2) `.local/.agent/handoff/L0__L2__example-add-dark-mode__0001.yaml` (one TaskDispatch envelope ~110 tokens); (3) `.local/memory/specs/example-domain/spec.md` (minimal valid source-of-truth ~245 tokens). Every artefact respects its **C-9 hard ceiling** — verified at write time by the new `_verify_artifact_within_budget(filename, content)` guard (raises `ValueError` per S-5 if a future template edit would breach) AND at test time by `tests/test_capability_e2e.py::test_artifacts_respect_c9_token_budgets`. **Idempotent**: every write is gated by `Path.exists()`; re-running `devola-init local --with-examples` is a no-op. Per S-9 the handoff envelope is APPEND-ONLY — once seeded the file MUST NOT be modified or deleted.
- **NEW `tests/test_capability_e2e.py`** (~580 LOC, **10 NEW test functions** — the LARGEST new file in the cycle and the cycle's headline lint): each test crosses one or more PV deliverables through a tmp-path repo fixture. (1) `test_l0_detects_feedbacks_specs_active_changes_via_scan_workspace` — PV-01 contract; (2) `test_standard_task_auto_opens_change_folder_when_env_flag_on` — PV-02 + PV-06 contract; (3) `test_standard_task_does_not_open_folder_without_env_flag` — R5 strict env-flag-OFF byte-stable proof across all 4 complexity tiers; (4) `test_l1_l2_dispatch_writes_handoff_envelope_when_flag_on` — PV-03 G-005 closure proof; (5) `test_memory_consult_emits_hit_when_prior_case_present` — PV-04 keyword-scored advisory surface; (6) `test_archive_triggers_source_of_truth_merge_proposal` — PV-05 + A-4 invariant proof (proposed but NOT auto-applied); (7) `test_artifacts_respect_c9_token_budgets` — every artifact respects `ARTIFACT_BUDGETS` from `lint.py` + handoff envelope budget 1200; (8) `test_install_local_with_examples_seeds_three_artifacts` — the cycle plan §PV-06 acceptance criterion #1; (9) `test_install_local_core_mode_skips_examples` — `with_examples=False` matrix; (10) `test_agents_md_slice_default_on_visible_to_dispatch` — the v9.1.5 PV-05 default-ON flip end-to-end.
- **EXTEND `scripts/archive_research_artifacts.py`** with the `--extra-prefix` repeatable argparse flag + the `extra_prefixes: tuple[str, ...]` kwarg on the `archive(cycle_version, ...)` callable. The MINOR-cycle invocation `python scripts/archive_research_artifacts.py 9.2.0 --extra-prefix v9.1.` captures BOTH `v9.1.*` PATCH research AND `v9.2.*` MINOR-cycle research into `docs/cycle-archive/v9.2.0/`. The `_gather_artifacts` helper deduplicates the union by absolute path so a file matched by both prefixes is enumerated exactly once.
- **W-19 cycle archive**: `docs/cycle-archive/v9.2.0/` populated with **31 artefacts** — `README.md` (auto-generated index) + 1 `gap_analysis.md` + 11 `nines/v9.1.*_nines.{json,md,stderr}` + 5 `nines/v9.1.*_nines_planning.{json,stderr}` + 3 `nines/v9.2.0_nines.*` + 2 `evaluation/v9.{1,2}.0_evaluation.md` + 2 `v9.{1,2}.0_retrospective.md`. The directory is committed to the repo so future cycle-N+1 SI-1 planning gates have a stable archive URL to cite without depending on `.local/` (gitignored).
- **W-7 / SI-8 retrospective**: `.local/research/v9.2.0_retrospective.md` covering ALL 4 mandatory sections (§1 Gaps identified / §2 What was implemented with file-level change list / §3 What was deferred + W-21 §1 telegraph for v9.4.0 Soul-set / §4 Key learnings — 8 lessons L-1..L-8). The retrospective explicitly states "**No S-11 candidate proposed for v9.4.0** — v9.4.0 Soul-set stays frozen at 10" per W-21 governance (the v9.2.0 cycle's invariants all landed cleanly under existing S-* / A-* / W-* rules).
- **W-2 NineS cycle-close**: `.local/research/v9.2.0_nines.{json,md}` cycle-rollup composite. `overall: 0.9073 ≥ 0.85` (margin +0.0573); SI-3 weighted composite **9.4750 ≥ 8.5 MINOR floor** (margin +0.9750) per Part A of `.local/research/v9.2.0_evaluation.md`. Critical axes: `structure_recognition` (byte_stability) **1.000**, `architecture` mean (decomposition + abstraction + cross_vertex) **1.000**, `lint_cleanliness` **1.000** (improved +0.04 vs v9.1.5 close from cleaner +650 LOC across init_project.py extension + test_capability_e2e.py).
- **W-18 ghost-audit refresh** — `tests/test_no_ghost_features.py::test_v9_2_0_new_symbols_have_coverage` authored BEFORE this CHANGELOG entry per W-18 precondition. Asserts: (a) `tests/test_capability_e2e.py` + `docs/cycle-archive/v9.2.0/{README,v9.2.0_retrospective}.md` exist with ≥ 200 byte minimum; (b) `install_local` accepts `with_examples` kwarg with `False` default; (c) `_seed_example_artifacts` is callable; (d) `tests/test_capability_e2e.py` defines ≥ 10 test functions; (e) `scripts/archive_research_artifacts.py` exposes both the `--extra-prefix` argparse flag AND the `extra_prefixes` kwarg on `archive()`.
- **A-2 layout invariant intact (the headline I-8 invariant)**: PASS — `schemas/lean-dispatch.yaml#layout_invariant.canonical_order` length **16** / version **5** byte-identical (PV-06 ships repo-init seeds + an e2e test + a script extension; no schema-side changes; the 6 historical baselines + the v9.2.0 witness baseline pinned in PV-04 continue to PASS).
- **A-4 invariant honoured**: source-of-truth files at `.local/memory/specs/<domain>/spec.md` are mutated ONLY at archive time AFTER the gate has PASSED. The PV-06 `_seed_example_artifacts` writes a static placeholder spec at `.local/memory/specs/example-domain/spec.md` for the EXAMPLE domain only; this is NOT an A-4 mutation of a real source-of-truth — it is the worked-trace fixture that demonstrates the SHAPE of the contract (with `last_merged_change: null` + `last_merged_at: null` indicating the bootstrap state).
- **Soul-set count 10 (frozen by W-21)**; Architecture 6; Conventions 9; Workflow 21; Style 13. Total rule count unchanged at **59 / 60** (+1 headroom). PV-06 ships extension code + tests + scripts + docs — none is a new layered rule. `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN.
- **W-20 reuse-first**: PASS — PV-06 ships **0 new env flags**. The cycle plan §"Self-iteration constraint compliance matrix" pin "0 new flags across the entire 7-PV cycle" continues to hold at MINOR cycle close. PV-02 + PV-03 reused `DEVOLAFLOW_AGENT_WORKSPACE`; PV-04 reused `DEVOLAFLOW_MEMORY_ROUTER`; PV-05 wired the existing-but-unwired `DEVOLAFLOW_AGENTS_MD_SLICE`. Cumulative cycle: 0 new flags.

### Cycle-level metrics

| Metric | v9.1.0 baseline | v9.2.0 close | Δ |
|---|---|---|---|
| `__version__` | 9.1.0 | 9.2.0 | MINOR bump |
| Tests collected | ~3490 (post-PV-01) | ~3590 | +100 (within +150 W-17 cycle cap) |
| **NEW** test functions in cycle | — | **+93** | within +150 W-17 cycle cap (+57 headroom) |
| Rule count | 58 | **59** | +1 (A-6 Architecture; within 60-rule HARD cap) |
| Soul-set count | 10 | **10** | flat (W-21 frozen) |
| `schemas/lean-dispatch.yaml#layout_invariant.canonical_order` length | 16 | **16** | flat (NEST extension per A-2.3) |
| Schema version | 5 | **5** | flat |
| New env flags introduced | — | **0** | W-20 reuse-first across all 6 PVs |
| `write_envelope` production callers | 1 | **2** | +1 (closes G-005) |
| EvoBench composite vs baseline | — | within ±5pp | no regressions |

### W-3 SI-3 cycle-close composite

The full Part A scorecard lives at `.local/research/v9.2.0_evaluation.md`. Headline:
...
| Performance impact | 0.15 | 9.5 | 1.4250 |
| **SI-3 composite** | — |, | **9.4750** |

**ACCEPT** — `9.4750 ≥ 8.5 MINOR floor` (margin **+0.9750**). The cycle ships ready for release.

### Files Changed (cycle-rollup summary)

The cycle-rollup commit touches the cycle's rollup-specific artefacts (PV-01..PV-05 each shipped their own per-PV CHANGELOG entries with full file lists). The PV-06 commit specifically touches:

**Source / tracked:**

- `src/devolaflow/__init__.py` — `9.1.5` → `9.2.0` (SSOT)
- `pyproject.toml` — `9.1.5` → `9.2.0`
- `workflow-system/agent/SKILL.md` — `9.1.5` → `9.2.0` frontmatter + banner + body "Current version:"
- `workflow-system/agent/workflow-skill.yaml` — `9.1.5` → `9.2.0`
- `src/devolaflow/init_project.py` — EXTEND `install_local` with `with_examples` kwarg + NEW `_seed_example_artifacts(cwd: Path)` helper + 9 module-level `_EXAMPLE_*` template constants + `_EXAMPLE_ARTIFACT_BUDGETS` dict + `_estimate_tokens` / `_verify_artifact_within_budget` guards + EXTEND `_parse_with_examples(argv, targets)` CLI parser + EXTEND `main()` to wire the flag through TOOLS dispatch
- `scripts/archive_research_artifacts.py` — EXTEND with `--extra-prefix` repeatable argparse flag + `extra_prefixes: tuple[str, ...]` kwarg on `archive()` + dedupe pass in `_gather_artifacts`
- `scripts/generate_human_docs.py` — `SOURCE_VERSION` `9.1.5` → `9.2.0`
- `tests/test_smoke.py` — version assertion `9.1.5` → `9.2.0`
- `README.md` — badge + example version `9.1.5` → `9.2.0`
- `workflow-system/human/demo/benchmark-results/index.html` — SAMPLE_DATA `9.1.5` → `9.2.0`
- `workflow-system/human/demo/version-timeline/versions.json` — v9.2.0 entry appended per ST-7 (49 → 50 entries; 10 highlights citing the 3 operator-visible changes + cycle metrics)
- `workflow-system/human/{en,zh}/*.md` — 16 EN/ZH guides regenerated by `make sync-human-docs` for ST-4 version propagation
- `docs/cycle-archive/v9.2.0/` — **NEW directory** with 31 archived research artefacts (W-19; the directory IS committed per W-19's "archive is created at cycle CLOSE and committed to the repo")

**Tests (all NEW unless noted):**

- `tests/test_capability_e2e.py` — **NEW** (~580 LOC, 10 NEW test functions), cycle headline lint
- `tests/test_no_ghost_features.py` — **EXTENDED** (+1 NEW test function: `test_v9_2_0_new_symbols_have_coverage`; ~150 LOC; W-18 refresh BEFORE this CHANGELOG entry)

**Research (.local/, gitignored — but archived to docs/cycle-archive/v9.2.0/):**

- `.local/research/v9.2.0_nines.json` — W-2 SI-2 cycle-close artifact
- `.local/research/v9.2.0_nines.md` — Markdown cycle-close composite + SI-3 weighted scorecard
- `.local/research/v9.2.0_nines_planning.json` — W-2 SI-2 planning artifact (`nines analyze --target-path src/devolaflow/`)
- `.local/research/v9.2.0_evaluation.md` — Part A binding ACCEPT/REJECT verdict + Part B NineS-anchored advisory snapshot
- `.local/research/v9.2.0_retrospective.md` — W-7 / SI-8 retrospective (4 mandatory sections + W-21 §1 telegraph for v9.4.0)

### Verification Checklist

- W-1 SI-1 planning gate artifact: **PASS** (cycle plan at `.cursor/plans/workspace-capability-activation_ec560bc8.plan.md` §"Diagnosis — capability is taught but never engaged")
- W-2 SI-2 NineS gate (planning + cycle-close): **PASS** (`.local/research/v9.2.0_nines.{json,md,_planning.json}`; composite `overall: 0.9073 ≥ 0.85` margin +0.0573)
- W-3 SI-3 composite ≥ 8.5 MINOR floor: **PASS** (composite **9.4750** margin +0.9750)
- W-4 SI-4 EvoBench regression: **PASS** (the cycle ships 0 regressions > 5pp; the `_seed_example_artifacts` flow is filesystem-side only and does not touch the dispatch hot path)
- W-7 SI-8 retrospective: **PASS** (`.local/research/v9.2.0_retrospective.md` with all 4 mandatory sections + W-21 §1 telegraph)
- W-9 SI-10 all 6 steps: **PASS** (`pytest tests/ -q` 3560 passed / 20 skipped / 2 xfailed; `ruff check src/ tests/` clean; `ruff format --check src/ tests/` 210 files already formatted; `pytest tests/test_version.py -v` 12 passed + 19 skipped (mirror absent self-skip per SF-3); `pytest tests/test_benchmarks.py -v` 36 passed (0 regressions); `make check-cursor-skill` exit 0 (mirror absent — opt-in per SF-3))
- W-11 gate test suite: **N/A** (PV-06 does not modify `src/devolaflow/gate/`)
- W-13 context optimization benchmark: **N/A for benchmark suite** (PV-06 ships repo-init seed templates + e2e tests + a CLI script extension; no dispatch-side compaction changes)
- W-17 per-PV test cap ≤ +30: **PASS** (+11 NEW functions in PV-06: 10 in `tests/test_capability_e2e.py` + 1 in `tests/test_no_ghost_features.py`); cumulative cycle delta **+93 NEW** of +150 cap (+57 headroom)
- W-18 ghost-audit refresh precondition: **PASS** (`test_v9_2_0_new_symbols_have_coverage` authored BEFORE this entry; presence + size + signature + e2e test count + script extension surface all asserted)
- W-19 cycle archive: **PASS** (`docs/cycle-archive/v9.2.0/` populated with 31 artefacts; directory committed to repo)
- W-20 env-flag reuse-first: **PASS** (PV-06 ships 0 new env flags; cycle cumulative 0 — the v9.2.0 cycle plan §"Self-iteration constraint compliance matrix" pin holds)
- W-21 Soul-set freeze (cap 10): **PASS** (10 Soul rules unchanged; the retrospective explicitly states "No S-11 candidate proposed for v9.4.0" per W-21 §1 telegraph; v9.4.0 Soul-set stays frozen at 10)

### Cross-references

- Cycle plan: `.cursor/plans/workspace-capability-activation_ec560bc8.plan.md`
- Per-PV CHANGELOG entries: `## [9.1.1]` … `## [9.1.5]` (below)
- Cycle archive: `docs/cycle-archive/v9.2.0/`
- Cycle-close composite: `.local/research/v9.2.0_nines.md`
- Cycle-close evaluation: `.local/research/v9.2.0_evaluation.md`
- W-7 / SI-8 retrospective: `.local/research/v9.2.0_retrospective.md`
- Canonical DevolaFlow URL: <https://github.com/YoRHa-Agents/DevolaFlow>
- NineS canonical URL: <https://github.com/YoRHa-Agents/NineS>

## [9.1.5] — 2026-05-01

**PATCH — PV-05 of the v9.2.0 MINOR cycle: `spec_bootstrap` (M-004 closure) + `agents_md_slice` flips default-ON (M-007 closure; OPERATOR-VISIBLE behaviour change).** Fifth PV of the v9.2.0 cycle, the most behaviour-flipping one. Closes TWO deferred items from the v9.0.0 retrospective §3.3 simultaneously: **M-004** — source-of-truth first-time seed surface (NEW `seed_initial_spec(domain, archive_id, repo_root, *, force=False)` derives `.local/memory/specs/<domain>/spec.md` from a verified `.local/.agent/archive/<archive_id>/` folder via the existing `ArchiveManager.propose_merge` machinery; A-4 invariant honoured — refuses to overwrite without `force=True`); **M-007 / per-task-type agents_md_slice dispatch wiring** — `workflow-system/agent/context_profiles.yaml#meta.agents_md_slice.enabled` flips from `false` to `true`, so L3 dispatchers receive sliced AGENTS.md content automatically (operator-visible default-ON; opt-out via `DEVOLAFLOW_AGENTS_MD_SLICE=0` per W-20 reuse — the env flag was telegraphed in v9.0.0 PV-07 ADR-007 D3 and the runtime read landed in v9.1.5 PV-05). **OPERATOR-VISIBLE BEHAVIOUR CHANGE**: dispatchers that have not set the opt-out env flag will see trimmed AGENTS.md scoped to their `task_type` starting with v9.1.5; with `DEVOLAFLOW_AGENTS_MD_SLICE=0` the dispatch is byte-identical to v9.1.4 (R5 strict). `schemas/lean-dispatch.yaml#layout_invariant` byte-identical at length 16 / version 5 (I-8 invariant intact through v9.1.5 — PV-05 ships a YAML default flip + a bootstrap module + an env-flag wiring; no schema-side changes). Soul-set frozen at 10 per W-21; rule count unchanged at 59 / 60 (+1 headroom).

### Operator-visible behaviour change (READ FIRST)

| What changed | Default before v9.1.5 | Default in v9.1.5 | Opt-out path |
|---|---|---|---|
| `meta.agents_md_slice.enabled` (per-task AGENTS.md slicing) | `false` (full AGENTS.md to every L3) | **`true` (sliced AGENTS.md per task_type)** | `export DEVOLAFLOW_AGENTS_MD_SLICE=0` |
| `seed_initial_spec` first-time SoT seed | (did not exist) | new public surface | call site explicit; not auto-invoked |

**R5 strict byte-stable proof**: with `DEVOLAFLOW_AGENTS_MD_SLICE=0` set, `select_agents_md_slice(task_type)` returns AGENTS.md byte-identical to v9.1.4 across 5 canonical task types + an unknown-task fallback (verified by `tests/test_pv07_agents_md_slice.py::test_agents_md_slice_env_flag_0_opts_out`). The opt-out env flag handles every loose-variant (`"0.0"` / `" 0 "` / `"false"` / `"00"` / `"true"` / `"yes"` / `"on"`) by FALLING THROUGH to the YAML default per `references/env-flags.md` §6 conjunction contract — only the literal `"0"` / `"1"` strings are honoured.

### Highlights

- **M-004 closure (source-of-truth first-time seed)**: NEW `src/devolaflow/agent_workspace/spec_bootstrap.py` (~150 LOC; **88% module coverage** with 6 NEW tests in `tests/test_spec_bootstrap.py` — well above the S-3 ≥ 80% floor). Public surface: `seed_initial_spec(domain: str, archive_id: str, repo_root: Path, *, force: bool = False) -> Path`. Module constants: `SOURCE_OF_TRUTH_ROOT = Path(".local") / "memory" / "specs"` and `ARCHIVE_ROOT = Path(".local") / ".agent" / "archive"` (resolved at CALL time via `repo_root` so `monkeypatch.chdir(tmp_path)` works in tests). Decision tree: (1) validate `domain` is a non-empty string with no path separators; (2) validate `archive_id` is non-empty; (3) validate `archive_folder = repo_root / ARCHIVE_ROOT / archive_id` is a directory; (4) validate `archive_folder / "spec.md"` exists; (5) if `target_path = repo_root / SOURCE_OF_TRUTH_ROOT / domain / "spec.md"` exists AND `not force` → raise `SpecBootstrapError` (A-4 invariant); (6) if exists AND `force` → log WARNING per S-5 + `target_path.unlink()` (wholesale-replace semantics for disaster-recovery / repo-init reset); (7) strip date prefix from `archive_id` → bare `change_id`; (8) call `ArchiveManager.propose_merge(change_id)`; wrap any `ArchiveError` / `MergeConflict` / `DeltaSpecParseError` / `ChangeNotFoundError` in `SpecBootstrapError` with `__cause__` preserved (S-5 — never silent); (9) verify `proposal.delta_target == domain` (refuses to write a mismatched spec); (10) atomic write via `.tmp` sibling + POSIX rename (mirrors `ArchiveManager.apply_merge`'s atomic-write contract). The `SpecBootstrapError(RuntimeError)` subclass keeps callers free of `agent_workspace.archive` import dependencies.
- **M-007 closure (agents_md_slice default-ON flip)**: `workflow-system/agent/context_profiles.yaml#meta.agents_md_slice.enabled` flips `false` → `true` (line 274 of the YAML). The comment block (lines 240-272) gains a v9.1.5 PV-05 paragraph documenting the default-ON flip + the `DEVOLAFLOW_AGENTS_MD_SLICE=0` opt-out hatch. The `fallback: full` invariant is preserved — unmatched task types still return byte-identical AGENTS.md (R5 strict). Per-profile slices (hotfix / feature / research / refactor / review / design / convergence / rdrr / documentation / default) are unchanged from v9.0.0 PV-07; only the activation default changes.
- **EXTEND `src/devolaflow/task_adaptive_selector.py`** with `_AGENTS_MD_SLICE_ENV_FLAG = "DEVOLAFLOW_AGENTS_MD_SLICE"` constant + `_agents_md_slice_env_override(env=None) -> bool | None` helper (R5 strict — EXACTLY `"0"` returns `False` (force opt-out), EXACTLY `"1"` returns `True` (force opt-in), anything else returns `None` (fall through to YAML). Per `references/env-flags.md` §6 conjunction contract: loose variants like `"true"` / `"yes"` / `"on"` / `" 1 "` / `"01"` / `"0.0"` MUST fall through to YAML default. `select_agents_md_slice()` gains a new `env: dict[str, str] | None = None` kwarg for test-time override; production callers pass `None` to default to `os.environ`. The override precedence is `env=0` (force OFF) > `env=1` (force ON) > YAML `enabled` flag.
- **EXTEND `src/devolaflow/agent_workspace/__init__.py`** to re-export `seed_initial_spec` + `SpecBootstrapError` from the package facade (added to `__all__` in alphabetical position); the v9.1.5 PV-05 paragraph in the module docstring documents M-004 closure semantics + the propose_merge → apply_merge update path distinction.
- **EXTEND `src/devolaflow/agent_workspace/archive.py`** module docstring + `ArchiveManager.archive(propose_merge=True)` kwarg docstring to clarify the v9.1.5 PV-05 wiring: `archive(propose_merge=True)` ONLY computes the merged content; it does NOT auto-apply (caller decides whether to invoke `apply_merge` or use `seed_initial_spec` for first-time seeds). NO behavioural code change — the `propose_merge` kwarg already existed since v8.4.4 PV-04; PV-05 just sharpens the contract documentation.
- **EXTEND `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST`** with `"devolaflow.agent_workspace.spec_bootstrap:seed_initial_spec"` + multi-paragraph rationale comment (mirrors v9.1.4 PV-04 `consult_for_dispatch` + v9.1.1 PV-01 `scan_workspace` allowlist precedents — public surface ships in v9.1.5 PV-05, in-repo Python production caller lands in v9.2.0 PV-06 `tests/test_capability_e2e.py` + the repo-init `--with-examples` seed flow). NOT a domain-SSOT registry symbol per A-5.2 — pure pathlib + archive-engine glue with zero module-level state.
- **NEW `tests/test_spec_bootstrap.py`** (~390 LOC, **6 NEW test functions**): `test_seed_from_valid_archive_creates_spec` (happy path — ADDED Requirement seeds the synthesised H1 + frontmatter scaffold + verbatim Requirement body), `test_seed_refuses_overwrite_without_force` (A-4 invariant proof — pre-existing target spec is NOT overwritten and remains byte-identical to its pre-call content; error message cites A-4 + force=True escape hatch), `test_seed_force_overwrites_with_warning` (S-5 — force=True wholesale-replaces stale content AND emits a WARNING via `logger.warning` captured by `caplog.at_level(logging.WARNING)`), `test_seed_raises_when_archive_missing` (S-5 — missing archive folder raises SpecBootstrapError with the archive_id echoed for diagnostics), `test_seed_proposes_merge_does_not_auto_apply` (gate-score independence — bootstrap path bypasses the apply_merge ≥ 8.5 / ≥ 9.0 threshold; uses a fixture archive with `gate_score: 5.0` to prove the bootstrap completes successfully where apply_merge would have raised `GateThresholdNotMet`), `test_seed_mergeconflict_surfaces_as_spec_bootstrap_error` (MergeConflict from a MODIFIED Requirement against the empty source-of-truth → wrapped in SpecBootstrapError with `__cause__` preserving the original MergeConflict; failed propose_merge MUST NOT leave a partial spec on disk).
- **EXTEND `tests/test_pv07_agents_md_slice.py`** (3 NEW test functions): `test_slice_yaml_disabled_returns_full_byte_identical` (operator YAML-override path — explicitly disabled YAML returns full AGENTS.md byte-identical), `test_agents_md_slice_default_on_in_v9_1_5` (canonical YAML pin — asserts `meta.agents_md_slice.enabled == True` in the shipped `context_profiles.yaml`), `test_agents_md_slice_env_flag_0_opts_out` (R5 strict byte-stable opt-out proof across 5 task types + 9 loose-variant rejections — `"0.0"` / `" 0 "` / `"false"` / `"0\n"` / `"00"` / `"1.0"` / `"true"` / `"yes"` / `"on"` ALL fall through to YAML default per §6 conjunction contract). Renamed the prior `test_slice_disabled_returns_full_byte_identical` → `test_slice_yaml_disabled_returns_full_byte_identical` because the YAML default is no longer disabled — the new name disambiguates.
- **EXTEND `workflow-system/agent/references/env-flags.md`** §2.11 with the `DEVOLAFLOW_AGENTS_MD_SLICE` entry (telegraphed v9.0.0 PV-07 ADR-007 D3; runtime read landed v9.1.5 PV-05; opt-out path explicit). Frontmatter `version:` bumped `8.5.1` → `9.1.5`; purpose paragraph gains the PV-05 wiring sentence. File grows 350 → 368 lines, well within the C-4 Large tier ≤ 1000 ceiling.
- **W-18 ghost-audit refresh discharged BEFORE this CHANGELOG entry** (per the W-18 PRECONDITION): `tests/test_no_ghost_features.py::test_v9_1_5_new_symbols_have_coverage` (~110 LOC, +1 NEW test function) asserts (1) every path in `_V9_1_5_NEW_FILES = ("src/devolaflow/agent_workspace/spec_bootstrap.py", "tests/test_spec_bootstrap.py")` is a regular file with size ≥ `_V9_1_5_FILE_MIN_BYTES = 50` bytes (no empty stubs); (2) `seed_initial_spec` + `SpecBootstrapError` are importable from BOTH the package facade `devolaflow.agent_workspace` AND the owning module `devolaflow.agent_workspace.spec_bootstrap`, with `facade is module` identity (catches the regression where one re-export path is dropped); (3) `SpecBootstrapError` subclasses `RuntimeError` (S-5 callers can catch via stdlib without agent_workspace import); (4) `seed_initial_spec(force=False)` default — the A-4 first-time-seed gate is the canonical entry path; (5) W-20 reuse-first lint: `_AGENTS_MD_SLICE_ENV_FLAG == "DEVOLAFLOW_AGENTS_MD_SLICE"` AND `_agents_md_slice_env_override({"DEVOLAFLOW_AGENTS_MD_SLICE": "0"}) is False` AND `... "1"}) is True`. PLUS `test_v9_1_5_agents_md_slice_default_on` (~25 LOC, +1 NEW test function) — the **headline** operator-visible-behaviour-change pin: `meta.agents_md_slice.enabled is True` AND `meta.agents_md_slice.fallback == "full"` (catches a regression where the canonical YAML is reverted without bumping the CHANGELOG).
- **W-2 NineS gate (planning + close)**: planning artifact at `.local/research/v9.1.5_nines_planning.json` from `nines analyze --target-path src/devolaflow/agent_workspace/ --depth deep --agent-impact --keypoints` (covers the new `spec_bootstrap.py` + `archive.py` docstring extensions). Close artifact at `.local/research/v9.1.5_nines.json` from `nines self-eval --baseline-version 9.1.4 -c nines.toml --golden-dir data/golden_test_set --samples-dir data/golden_test_set --src-dir src/devolaflow --test-dir tests --project-root .`; Markdown rendering at `.local/research/v9.1.5_nines.md`. The PV-05 explicit gate floors per the dispatch instructions: `byte_stability` (structure_recognition) ≥ 0.95 (the env-flag OFF path MUST produce byte-identical AGENTS.md) AND `architecture` ≥ 0.85 (the default-ON flip is an architectural decision).
- **W-9 / SI-10 6 / 6 PASS**: `pytest tests/ -q` 3549 passed / 20 skipped / 2 xfailed (was 3539; +10 NEW tests via 6 spec_bootstrap + 3 agents_md_slice + 2 ghost-audit but the prior `test_slice_disabled_returns_full_byte_identical` was renamed not added → +9 NEW collected); `ruff check src/ tests/` clean; `ruff format --check src/ tests/` clean; `pytest tests/test_version.py -v` 12 passed + 19 skipped (mirror absent self-skip per SF-3); `pytest tests/test_benchmarks.py -v` clean (0 regressions; agents_md_slice default-ON does not affect dispatch-side compaction baselines); `make check-cursor-skill` exit 0 (mirror absent — opt-in per SF-3).
- **Per-PV test cap (W-17)**: **+10 NEW test functions in PV-05** (6 in `tests/test_spec_bootstrap.py` + 2 NEW in `tests/test_pv07_agents_md_slice.py` (the 3rd is a rename) + 2 ghost-audit refresh in `tests/test_no_ghost_features.py`). Within the +30 W-17 per-PV cap (margin +20). Cumulative cycle delta from v9.1.0 → v9.1.5: **+83 NEW** (PV-01 +8 + PV-02 +26 + PV-03 +25 + PV-04 +14 + PV-05 +10) of +150 cycle cap (+67 headroom).
- **Soul-set count 10 (frozen by W-21)**; Architecture 6; Conventions 9; Workflow 21; Style 13. Total rule count unchanged at **59 / 60** (+1 headroom); PV-05 ships a YAML default flip + a bootstrap module + an env-flag wiring — none is a new layered rule. `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN.
- **A-2 layout invariant intact (the headline I-8 invariant)**: PASS — `schemas/lean-dispatch.yaml#layout_invariant.canonical_order` length **16** / version **5** byte-identical (PV-05 makes no schema changes; the 6 historical baselines + the v9.2.0 witness baseline pinned in PV-04 continue to PASS without modification).
- **A-4 invariant honoured**: source-of-truth files at `.local/memory/specs/<domain>/spec.md` are mutated ONLY at archive time AFTER the gate has PASSED (the v8.4.4 PV-04 `apply_merge` ≥ 8.5 PATCH/MINOR / ≥ 9.0 MAJOR contract). The new `seed_initial_spec` is the FIRST-TIME bootstrap path — gates on filesystem absence (refuse-overwrite without `force=True`) instead of gate-score, because there is no existing spec to gate against. Subsequent updates of an existing spec MUST go through `propose_merge → apply_merge` (the gate-score-checked update path).
- **A-5 SSOT registry pattern**: PASS — `seed_initial_spec` is NOT a registry symbol (per A-5 ADR §A-5.2 hygiene clause; pure pathlib + archive-engine glue with zero module-level state). The function is allowlisted in `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST` because its in-repo Python production caller lands in v9.2.0 PV-06 (the e2e capability test); `_check_allowlist_domain_overlap()` confirms non-overlap with `SSOT_REGISTRY_QUALIFIED_NAMES`.
- **W-20 reuse-first**: PASS — `DEVOLAFLOW_AGENTS_MD_SLICE` was telegraphed in v9.0.0 PV-07 ADR-007 D3 (the design-time landing) and the runtime read landed in v9.1.5 PV-05 (the wiring landing). NO new env-flag introduced — the existing-but-unwired flag is the W-20 reuse target. The v9.2.0 cycle plan §"Self-iteration constraint compliance matrix" pin "0 new flags across the entire 7-PV cycle" continues to hold.

### A-4 + W-20 audit table

| Audit | Result | Where |
|---|---|---|
| `seed_initial_spec(force=False)` refuses to overwrite | **PASS** | `tests/test_spec_bootstrap.py::test_seed_refuses_overwrite_without_force` |
| `seed_initial_spec(force=True)` logs WARNING + overwrites | **PASS** | `tests/test_spec_bootstrap.py::test_seed_force_overwrites_with_warning` |
| Bootstrap bypasses `apply_merge` gate-score check | **PASS** (`gate_score=5.0` does not block) | `tests/test_spec_bootstrap.py::test_seed_proposes_merge_does_not_auto_apply` |
| `_AGENTS_MD_SLICE_ENV_FLAG == "DEVOLAFLOW_AGENTS_MD_SLICE"` | **PASS** (W-20 reuse) | `tests/test_no_ghost_features.py::test_v9_1_5_new_symbols_have_coverage` |
| R5 strict env-flag literal-only matching | **PASS** (9 loose variants reject) | `tests/test_pv07_agents_md_slice.py::test_agents_md_slice_env_flag_0_opts_out` |
| Canonical YAML default `enabled: true` | **PASS** | `tests/test_no_ghost_features.py::test_v9_1_5_agents_md_slice_default_on` |
| W-18 ghost-audit refresh BEFORE CHANGELOG | **PASS** | `tests/test_no_ghost_features.py::test_v9_1_5_*` |

### Files Changed

**Source / tracked:**

- `src/devolaflow/__init__.py` — `9.1.4` → `9.1.5` (SSOT)
- `pyproject.toml` — `9.1.4` → `9.1.5`
- `workflow-system/agent/SKILL.md` — `9.1.4` → `9.1.5` frontmatter + banner + body "Current version:" (line count unchanged at 458 — preserves byte layout for EvoBench compression scenarios + Windsurf 8000-char budget)
- `workflow-system/agent/workflow-skill.yaml` — `9.1.4` → `9.1.5`
- `src/devolaflow/agent_workspace/spec_bootstrap.py` — **NEW** (~150 LOC; `seed_initial_spec` + `SpecBootstrapError`; 88% coverage)
- `src/devolaflow/agent_workspace/__init__.py` — re-exports `seed_initial_spec` + `SpecBootstrapError` from the package facade (added to `__all__` + module docstring updated)
- `src/devolaflow/agent_workspace/archive.py` — module docstring + `ArchiveManager.archive(propose_merge=True)` kwarg docstring extended with v9.1.5 PV-05 wiring clarification (no behavioural code change)
- `src/devolaflow/task_adaptive_selector.py` — NEW `_AGENTS_MD_SLICE_ENV_FLAG` + `_agents_md_slice_env_override()` helper; `select_agents_md_slice()` gains `env` kwarg + R5 strict env-override precedence; comment block above `_AGENTS_MD_PATH` updated for v9.1.5 PV-05 default-ON semantics
- `workflow-system/agent/context_profiles.yaml` — `meta.agents_md_slice.enabled: false` → `true` (line 274); comment block (lines 240-272) gains v9.1.5 PV-05 paragraph documenting the default-ON flip + the `DEVOLAFLOW_AGENTS_MD_SLICE=0` opt-out hatch
- `workflow-system/agent/references/env-flags.md` — NEW §2.11 entry for `DEVOLAFLOW_AGENTS_MD_SLICE`; frontmatter `version:` `8.5.1` → `9.1.5`; purpose paragraph gains PV-05 wiring sentence (350 → 368 lines, within C-4 Large tier ≤ 1000)
- `scripts/detect_dead_apis.py` — `DEFAULT_ALLOWLIST` gains `"devolaflow.agent_workspace.spec_bootstrap:seed_initial_spec"` with multi-paragraph rationale comment
- `scripts/generate_human_docs.py` — SOURCE_VERSION `9.1.4` → `9.1.5`
- `tests/test_smoke.py` — version assertion `9.1.4` → `9.1.5`
- `README.md` — badge + example version `9.1.4` → `9.1.5`
- `workflow-system/human/demo/benchmark-results/index.html` — SAMPLE_DATA `9.1.4` → `9.1.5`
- `workflow-system/human/demo/version-timeline/versions.json` — v9.1.5 entry appended per ST-7 (48 → 49 entries)
- `workflow-system/human/{en,zh}/*.md` — 16 EN/ZH guides regenerated by `make sync-human-docs` for ST-4 version propagation
- `CHANGELOG.md` — this entry

**Tests (all NEW unless noted):**

- `tests/test_spec_bootstrap.py` — **NEW** (~390 LOC, 6 NEW test functions), `seed_initial_spec` contract pin (happy path / A-4 overwrite-refusal / force=True wholesale-replace + WARNING / missing-archive error / gate-score independence / MergeConflict surface preserving __cause__)
- `tests/test_pv07_agents_md_slice.py` — **EXTENDED** (+3 NEW test functions; `test_slice_disabled_returns_full_byte_identical` renamed to `test_slice_yaml_disabled_returns_full_byte_identical` for clarity post-default-flip; new `test_agents_md_slice_default_on_in_v9_1_5` + `test_agents_md_slice_env_flag_0_opts_out`)
- `tests/test_no_ghost_features.py` — **EXTENDED** (~200 LOC, +2 NEW test functions: `test_v9_1_5_new_symbols_have_coverage` + `test_v9_1_5_agents_md_slice_default_on`) — W-18 refresh + headline operator-visible behaviour change pin

**Research (.local/, gitignored):**

- `.local/research/v9.1.5_nines_planning.json` — W-2 SI-2 planning artifact (`nines analyze --target-path src/devolaflow/agent_workspace/ --depth deep --agent-impact --keypoints`)
- `.local/research/v9.1.5_nines.json` — W-2 SI-2 close artifact (`nines self-eval --baseline-version 9.1.4 -c nines.toml --golden-dir data/golden_test_set --samples-dir data/golden_test_set --src-dir src/devolaflow --test-dir tests --project-root .`)
- `.local/research/v9.1.5_nines.md` — Markdown rendering of close composite + SI-3 weighted scorecard

### Verification Checklist

- W-1 SI-1 planning gate artifact: **PASS** (cycle plan at `.cursor/plans/workspace-capability-activation_ec560bc8.plan.md` §"PV-05 — `v9.1.5` PATCH: Source-of-Truth Spec Bootstrap + agents_md_slice Default-ON" provides the planning gate)
- W-2 SI-2 NineS gate (planning + close): **PASS** (raw + Markdown artifacts in `.local/research/v9.1.5_nines{,.md,_planning.json}`; explicit byte_stability ≥ 0.95 AND architecture ≥ 0.85 verified per the dispatch instruction's most-behaviour-flipping-PV gate)
- W-3 SI-3 composite ≥ 8.5: **PASS** (NineS composition per §"NineS composition into SI-3"; PATCH floor 8.5 — this PV is the most behaviour-flipping; composite holds)
- W-9 SI-10 all 6 steps: **PASS** (tests / lint / format / version / benchmarks / check-cursor-skill all green)
- W-11 gate test suite: **N/A** (PV-05 does not modify `src/devolaflow/gate/`)
- W-13 context optimization benchmark: **N/A for benchmark suite** (PV-05 modifies `task_adaptive_selector.py` for the env-flag wiring + flips the YAML default, but the benchmark scenarios target dispatch-side compaction not AGENTS.md slicing — `test_benchmarks.py` continues PASS)
- W-17 per-PV test cap ≤ +30: **PASS** (+10 NEW functions in PV-05; cumulative cycle delta +83 of +150 cap with +67 headroom)
- W-18 ghost-audit refresh precondition: **PASS** (2 new test functions authored BEFORE this CHANGELOG entry; presence + import-smoke + facade-vs-module identity + force kwarg default + W-20 env-flag pin + canonical YAML default-ON pin + fallback='full' invariant)
- W-20 env-flag reuse-first: **PASS** (`DEVOLAFLOW_AGENTS_MD_SLICE` was telegraphed in v9.0.0 PV-07 ADR-007 D3; the runtime read landed in v9.1.5 PV-05 — REUSE of the existing-but-unwired flag, NOT a new flag; v9.2.0 cycle plan §"Self-iteration constraint compliance matrix" "0 new flags across the entire 7-PV cycle" continues to hold)
- W-21 Soul-set freeze (cap 10): **PASS** (10 Soul rules unchanged; PV-05 ships a YAML flip + a bootstrap module + an env-flag wiring — none is a Soul invariant candidate)
- 60-rule cap: **PASS** (59/60 unchanged with +1 headroom; PV-05 introduces neither a layered rule nor a lifecycle event)
- A-2 layout invariant intact (the headline I-8 invariant): **PASS** (`canonical_order` length 16 / `version` 5 byte-identical; `tests/test_layout_invariant_multi_baseline.py` 6 historical baselines + the v9.2.0 baseline pinned in PV-04 continue to PASS without modification — PV-05 makes no schema changes)
- A-4 invariant honoured: **PASS** (`seed_initial_spec(force=False)` refuses overwrite; subsequent updates use `apply_merge` ≥ 8.5 PATCH/MINOR / ≥ 9.0 MAJOR threshold; force=True is for repo-init / disaster-recovery only and logs WARNING per S-5)
- A-5 SSOT registry pattern: **PASS** (`seed_initial_spec` is NOT a registry symbol; pure pathlib + archive-engine glue with zero module-level state; allowlisted in `DEFAULT_ALLOWLIST` because the in-repo Python production caller lands in v9.2.0 PV-06)
- S-3 coverage floor on spec_bootstrap.py: **PASS** (88% module coverage with 6 NEW tests — well above the ≥ 80% S-3 floor)
- ST-4 version propagation: **PASS** (canonical 7 sync locations all updated by `scripts/bump_version.py`; `make sync-human-docs` regenerated 16 EN/ZH files)
- ST-7 version-timeline entry: **PASS** (v9.1.5 appended; 48 → 49 entries; cites the operator-visible behaviour change as the headline per ST-7)
- C-4 tiered line budget: **PASS** (SKILL.md 458/500 unchanged; `references/env-flags.md` 350 → 368 within Large tier ≤ 1000)

## [9.1.4] — 2026-05-01

**PATCH — PV-04 of the v9.2.0 MINOR cycle: `memory_router.consult_for_dispatch()` + automatic feedback ingestion at plan-mode entry + `change_context` NEST extension (3 new OPTIONAL sub-fields).** Fourth PV of the v9.2.0 cycle. Composes `.local/memory/cases/index.yaml` + `.local/feedbacks/` into the dispatch payload's `change_context` block via 3 NEW OPTIONAL sub-fields (`prior_feedback_themes` / `memory_case_hits` / `source_of_truth_excerpt`) added per A-2.3 (NEST decision rule — fields are independently optional + modify how `change_context` is interpreted → NEST). **`canonical_order` length STAYS at 16** AND **schema version STAYS at 5** — zero cache-prefix disturbance (the I-8 invariant intact through v9.1.4). All 6 historical baselines (v7.0.0 → v8.4.0) continue to PASS without modification, plus a NEW v9.2.0 witness baseline pinned at `benchmarks/devolaflow_context/baselines/layout_invariant_v9.2.0.yaml` (byte-identical to v8.4.0). New `consult_for_dispatch()` is the keyword-scored hint companion to the existing `MemoryRouter.lookup_case` fast-path; gated by `DEVOLAFLOW_MEMORY_ROUTER` (REUSED per W-20 — 0 new env flags introduced, matching the v9.2.0 cycle plan §"Self-iteration constraint compliance matrix"). Feedback ingestion at plan-mode entry is documented in `references/plan-mode-enforcement.md` §5.5 "Automatic Ingestion at Plan-Mode Entry (v9.1.4+)" as normative L0 guidance (no new Python module — the L0 LLM extracts ≤ 5 themes from the latest 3 feedbacks). Soul-set frozen at 10 per W-21; rule count unchanged at 59 / 60 (+1 headroom).

### Highlights

- **NEST decision per A-2.3**: `change_context` (position 16) extended with 3 OPTIONAL sub-fields. NEST chosen over APPEND because the new fields modify how an existing block is interpreted (the L3 task agent treats `change_context` as the binding for in-flight workspace state PLUS, when present, the user-voice anchors). Per A-2.3 decision matrix, "new fields are independently optional and modify how an existing block is interpreted — NEST". Schema version STAYS at 5; canonical_order length STAYS at 16.
- **NEW `src/devolaflow/memory_router/cache.py::consult_for_dispatch()`** (~250 LOC including docstring; pure function, zero module state; **84% joint coverage** on `cache.py` with `test_memory_consult_for_dispatch.py` + `test_memory_router.py` — well above the S-3 ≥ 80% floor). Public surface: `consult_for_dispatch(payload, repo_root, *, max_hits=3, env=None, current_version=None, today=None) -> list[MemoryCase]`. Module constants: `_CONSULT_ENV_FLAG = "DEVOLAFLOW_MEMORY_ROUTER"` (REUSED — see W-20 below), `_CONSULT_ENV_TRUTHY = "1"` (R5 strict literal-only), `_DEFAULT_CONSULT_INDEX_PATH = Path(".local/memory/cases/index.yaml")` (resolved at CALL time, not import time, so `monkeypatch.chdir(tmp_path)` works), `_CONSULT_STOPWORDS = frozenset({"the", "and", "for", "with", "from", "into", "this", "that", "have", "will"})` (≤ 10 hardcoded; intentionally small), `_CONSULT_KEYWORD_SPLIT_RE` (lowercase tokenizer; non-alnum word-boundaries; tokens < 3 chars dropped), `_CONSULT_DEFAULT_MAX_HITS = 3` (matches the documented schema cap on `memory_case_hits`). Decision tree (cache-miss is ALWAYS the safe path per R5 strict): (1) env-flag NOT literal "1" → `[]` immediately + zero IO; (2) `.local/memory/cases/index.yaml` missing → `[]` + DEBUG log only (legitimate new-repo state — NOT WARNING); (3) malformed YAML → `[]` + WARNING (S-5 explicit error state); (4) well-formed → score every fresh + version-current case against payload keywords (extracted from `payload['task']['description']` + `payload['task']['title']` + `payload['task']['goal']` + `payload['goal']`; defensive against missing keys), score = `len(payload_keywords ∩ case_corpus)` where `case_corpus = summary + tags + workflow_type + task_type` (case_id and recipe_path intentionally excluded — they leak filename noise); sort by `(-score, _negate_iso_date(last_accessed), case_id)` ascending; return first `max_hits`. Lazy-imports `devolaflow.__version__` inside the function body (avoids the cache.py import-cycle hazard documented in `router.py::_resolve_current_version`).
- **W-20 reuse-first**: `consult_for_dispatch` is gated by the SAME env-flag as the existing `MemoryRouter.lookup_case` fast-path (`DEVOLAFLOW_MEMORY_ROUTER`). Per the v9.2.0 cycle plan §"Self-iteration constraint compliance matrix" W-20 row: "DEVOLAFLOW_MEMORY_CONSULT REUSED with DEVOLAFLOW_MEMORY_ROUTER (PV-04); 0 new flags across the entire 7-PV cycle." The two surfaces share the same operator opt-in, the same `.local/memory/cases/index.yaml` source-of-truth, and the same R5 strict zero-IO contract when disabled — introducing a separate `DEVOLAFLOW_MEMORY_CONSULT` flag would have failed the behavioural-orthogonality test in `references/env-flags.md` §7. The reuse rationale is documented in the new `_CONSULT_ENV_FLAG` docstring + the `consult_for_dispatch` function docstring + this CHANGELOG entry; the W-18 ghost-audit lint pins `_CONSULT_ENV_FLAG == "DEVOLAFLOW_MEMORY_ROUTER"` (regression-proof).
- **EXTEND `schemas/lean-dispatch.yaml`** `change_context` (position 16) with 3 NEW OPTIONAL sub-fields per A-2.3 NEST decision: `prior_feedback_themes: list[str]` (≤ 5 items, ≤ 30 chars each — extracted themes from `.local/feedbacks/` latest 3 files per `scan_workspace.recent_feedbacks`); `memory_case_hits: list[str]` (≤ 3 items, MemoryCase.case_id values surfaced by `consult_for_dispatch()`); `source_of_truth_excerpt: str | None` (≤ 200 tokens; excerpt from `.local/memory/specs/<domain>/spec.md` named by `spec_delta_target`). The caps are SCHEMA-DOCUMENTED only; PV-04 does NOT add runtime validation (a future PV may). The schema body comment block above `layout_invariant` gains a new "v9.1.4 PV-04" paragraph citing A-2.3 + the I-8 invariant; the `layout_invariant.enforcement.backward_compat` block gains a `v9_2_0_baseline_passes: true` row.
- **NEW `benchmarks/devolaflow_context/baselines/layout_invariant_v9.2.0.yaml`** — byte-identical to `layout_invariant_v8.4.0.yaml`. The witness baseline proves that callers who do NOT yet emit the new sub-fields continue to see the same cache-prefix bytes (the headline I-8 invariant proof for v9.1.4 PV-04). Pinned in `tests/test_layout_invariant_multi_baseline.py` via 2 NEW test methods: `test_v9_2_0_baseline_byte_identical` (renders the v9.2.0 payload through `_render` and byte-compares against the file) AND `test_v9_2_0_baseline_byte_identical_to_v8_4_0` (asserts the two YAML files are verbatim copies — catches the regression where one is updated and the other forgotten).
- **EXTEND `workflow-system/agent/references/plan-mode-enforcement.md`** §5.5 with a new "Automatic Ingestion at Plan-Mode Entry (v9.1.4+)" sub-section (~50 lines; file grows 586 → 647 within C-4 Large tier ≤ 1000 ceiling). Documents the 4-step contract: (1) call `scan_workspace(repo_root)` at plan-mode entry; (2) read each of the ≤ 3 feedback paths with the standard `Read` tool (no new tool permission needed — already allowed in §5.1); (3) extract ≤ 5 short noun/verb phrases (≤ 30 chars each; lowercase snake_case preferred, examples: `handoff_auto_write`, `slash_commands_cli`, `workspace_discovery`, `memory_consultation`); (4) populate `change_context.prior_feedback_themes`, citing source paths repo-relative per S-2. Also documents the rationale for NEST (vs APPEND) and the W-20 reuse for `memory_case_hits` activation.
- **NEW `tests/test_memory_consult_for_dispatch.py`** (~340 LOC, **6 NEW test functions**): `test_env_flag_off_returns_empty_list` (R5 strict — env-flag absent → `[]` with zero `Path.read_text` calls verified by a `monkeypatch.setattr(Path, "read_text", _watcher)` watcher; ALSO parametrized over 7 non-`"1"` variants `["0", "true", "yes", " 1 ", "1.0", "TRUE", ""]` — every variant must short-circuit), `test_missing_index_yaml_returns_empty_list` (env-flag ON + index absent → `[]` + DEBUG log only — NOT WARNING; legitimate new-repo state), `test_malformed_index_yaml_returns_empty_list_with_warning` (env-flag ON + 3 malformed shapes — YAML parse error / top-level non-mapping / `cases` non-list — each → `[]` + WARNING captured via `caplog.at_level(logging.WARNING, logger="devolaflow.memory_router.cache")`), `test_zero_max_hits_and_empty_keywords_short_circuit` (defensive cap=0 + empty payload + missing task block + non-dict task), `test_keyword_overlap_returns_top_hits` (5 cases with varying overlap → top-3 by score; mid-tier ties broken by `last_accessed` desc), `test_expired_and_stale_cases_are_filtered` (1 fresh + 1 version-stale + 1 TTL-expired + 1 fresh → only the 2 fresh cases returned).
- **NEW `tests/test_feedback_ingestion_plan_mode.py`** (~210 LOC, **4 NEW test functions**): `test_no_feedbacks_dir_returns_empty_recent_feedbacks` (empty `.local/feedbacks/` AND README-only directory both → empty tuple), `test_recent_feedbacks_paths_relative_via_to_summary_dict` (S-2 invariant — every path in `WorkspaceContext.to_summary_dict()['recent_feedbacks']` starts with `.local/feedbacks/` and never with `/`; JSON round-trip preserves the contract), `test_cap_at_three_feedbacks_newest_first_by_mtime` (5 staggered-mtime files → exactly 3 returned; `MAX_FEEDBACKS_RETURNED == 3` pinned; mtime-desc order verified), `test_plan_mode_enforcement_doc_documents_v9_1_4_ingestion_contract` (W-18 contract pin: doc contains the v9.1.4+ section anchor + ≤ 5 / ≤ 30 caps + `change_context.prior_feedback_themes` schema cite + `Read` tool reference + S-2 cite + `MAX_FEEDBACKS_RETURNED` value 3). Per the dispatch instructions §"Alternative" preference, NO new Python helper module was added — the theme extraction is normative L0 guidance documented in `plan-mode-enforcement.md` §5.5; the tests pin the discovery API contract + the doc content.
- **EXTEND `tests/test_layout_invariant_multi_baseline.py`** with `_v9_2_0_payload()` constructor (delegates to `_v8_4_0_payload()` to make the byte-identical contract obvious to any reader) + 2 NEW test methods (`test_v9_2_0_baseline_byte_identical` and `test_v9_2_0_baseline_byte_identical_to_v8_4_0`). Module docstring updated to enumerate v9.2.0 as the 7th covered baseline. The 6 prior baselines (v7.0.0 / v7.3.0 / v8.0.0 P-08 / v8.0.0 P-10 / v8.3.0 PV-05 / v8.4.0) continue to PASS without modification — the headline I-8 invariant proof for the NEST extension.
- **EXTEND `src/devolaflow/memory_router/__init__.py`** to re-export `consult_for_dispatch` from the package facade (added to `__all__` in alphabetical position).
- **EXTEND `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST`** with `"devolaflow.memory_router.cache:consult_for_dispatch"` + a multi-paragraph comment citing the v9.2.0 PV-06 production-caller landing date (mirrors the `lifecycle.run_hooks` / `directed_compact` / `workspace_context.scan_workspace` allowlist precedent — public surface advertised in `references/plan-mode-enforcement.md` §5.5, in-repo Python caller lands in PV-06 `tests/test_capability_e2e.py`). NOT a domain-SSOT registry symbol per A-5.2 — pure read-only YAML-parser + keyword-overlap scorer with no module-level state. The `_check_allowlist_domain_overlap()` lint at script import time verifies non-overlap; `tests/test_dead_apis.py::test_default_allowlist_no_ssot_overlap` continues GREEN.
- **EXTEND `workflow-system/human/demo/index.html`** with a NEW "What's New in v9.1.4" `<section class="fade-in">` inserted immediately above the existing v9.1.2 entry (4 cards: CONSULTATION / NEST / INGESTION / METRICS). `tests/test_doc_consistency.py::test_demo_index_version_matches_package` continues GREEN (demo version 9.1.4 matches package); `test_demo_index_gate_types` continues GREEN (avoided the stale "advisory" gate-type term — used "hint companion" instead).
- **W-18 ghost-audit refresh discharged BEFORE this CHANGELOG entry** (per the W-18 PRECONDITION): `tests/test_no_ghost_features.py::test_v9_1_4_new_symbols_have_coverage` (+165 LOC, +1 NEW test function) asserts (1) every path in `_V9_1_4_NEW_FILES = ("tests/test_memory_consult_for_dispatch.py", "tests/test_feedback_ingestion_plan_mode.py", "benchmarks/devolaflow_context/baselines/layout_invariant_v9.2.0.yaml")` is a regular file with size ≥ `_V9_1_4_FILE_MIN_BYTES = 50` bytes (no empty stubs); (2) `consult_for_dispatch` is importable from BOTH the package facade `devolaflow.memory_router` AND the owning module `devolaflow.memory_router.cache`, both are callable, AND `facade is module` (catches the regression where one re-export path is dropped); (3) W-20 reuse-first: `_CONSULT_ENV_FLAG == "DEVOLAFLOW_MEMORY_ROUTER"` AND `_CONSULT_ENV_TRUTHY == "1"` (R5 strict pin); (4) schema NEST extension lint: `change_context.fields` in `schemas/lean-dispatch.yaml` contains all 3 new sub-fields (`prior_feedback_themes` / `memory_case_hits` / `source_of_truth_excerpt`). PLUS `test_v9_1_4_nest_preserves_canonical_order_length` (+50 LOC, +1 NEW test function) — the **headline I-8 invariant proof**: `len(layout_invariant.canonical_order) == 16` AND `layout_invariant.version == 5` AND the v9.2.0 baseline is byte-identical to the v8.4.0 baseline (catches a future regression where one is updated and the other forgotten).
- **W-2 NineS gate (planning + close)**: planning artifact at `.local/research/v9.1.4_nines_planning.json` from `nines analyze --target-path src/devolaflow/memory_router/ --depth deep --agent-impact --keypoints` (covers the new `consult_for_dispatch` surface). Close artifact at `.local/research/v9.1.4_nines.json` from `nines self-eval --baseline-version 9.1.3 -c nines.toml --golden-dir data/golden_test_set --samples-dir data/golden_test_set --src-dir src/devolaflow --test-dir tests --project-root .` plus Markdown rendering at `.local/research/v9.1.4_nines.md`. **Composite `overall: 0.9073` ≥ 0.85 (margin +0.0573)**, byte-identical to the v9.1.1 PV-01 cycle-start baseline (PV-04 is byte-stable additive). Headline axis `index_recall: 0.8000` is at the persistent upstream A3 ceiling per the dispatch instructions ("NineS upstream A3 blocker may cap at 0.8; document if so"); `code_coverage: 0.0000` is the persistent upstream A1 blocker (subprocess timeout — NOT a real coverage shortfall; documented in v9.0.0 retrospective). SI-3 weighted composite **9.89 / 10** (composed per cycle plan §"NineS composition into SI-3" mapping; ≥ 8.5 PATCH floor with margin +1.39).
- **W-9 / SI-10 6 / 6 PASS**: `pytest tests/ -q` 3539 passed / 20 skipped / 2 xfailed; `ruff check src/ tests/` clean (5 line-length errors fixed in-scope); `ruff format --check src/ tests/` clean; `pytest tests/test_version.py -v` 12 passed + 19 skipped (mirror absent self-skip per SF-3); `pytest tests/test_benchmarks.py -v` 36 passed (0 regressions); `make check-cursor-skill` exit 0 (mirror absent — opt-in per SF-3).
- **Per-PV test cap (W-17)**: **+14 NEW test functions in PV-04** (6 in `tests/test_memory_consult_for_dispatch.py` + 4 in `tests/test_feedback_ingestion_plan_mode.py` + 2 ghost-audit refresh in `tests/test_no_ghost_features.py` + 2 baseline-byte tests in `tests/test_layout_invariant_multi_baseline.py`). Within the +30 W-17 per-PV cap (margin +16). Cumulative cycle delta from v9.1.0 → v9.1.4: **+73 NEW** (PV-01 +8 + PV-02 +26 + PV-03 +25 + PV-04 +14) of +150 cycle cap (+77 headroom).
- **Soul-set count 10 (frozen by W-21)**; Architecture 6; Conventions 9; Workflow 21; Style 13. Total rule count unchanged at **59 / 60** (+1 headroom); PV-04 ships a NEST schema extension + a new pure function — not a new layered rule. `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN.
- **A-2.3 NEST-vs-APPEND decision (PV-04)**: NEST. The 3 new sub-fields are independently OPTIONAL and modify how the existing `change_context` block is interpreted (the L3 task agent reads `change_context` as the binding for in-flight workspace state PLUS, when present, the user-voice anchors). Per A-2.3 decision matrix: "Does the behaviour modify how an existing block is interpreted? → NEST" + "Are the new fields independently optional? Either is acceptable; prefer NEST when the data shape allows" → both rows favor NEST. APPEND would have bumped `canonical_order` to length 17 + `version` to 6 — invalidating ALL 6 historical baselines + the cache prefix (the very disturbance A-2 protects against).
- **A-2.4 multi-baseline byte test**: PASS. All 6 historical baselines (v7.0.0 / v7.3.0 / v8.0.0 P-08 / v8.0.0 P-10 / v8.3.0 PV-05 / v8.4.0) PASS unchanged + the NEW v9.2.0 baseline pin PASSES. `tests/test_layout_invariant_multi_baseline.py` 23 → 25 tests (+2 new).
- **A-5 SSOT registry pattern**: PASS — `consult_for_dispatch` is NOT a registry symbol (per A-5 ADR §A-5.2 hygiene clause; pure function with zero module state). The function is allowlisted in `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST` because its in-repo Python production caller lands in v9.2.0 PV-06 (the e2e test); `_check_allowlist_domain_overlap()` confirms non-overlap with `SSOT_REGISTRY_QUALIFIED_NAMES`.

### NEST decision evidence (the I-8 invariant proof)

| Audit | Result | Where |
|---|---|---|
| `len(layout_invariant.canonical_order)` | **16** (unchanged from v8.4.0) | `schemas/lean-dispatch.yaml` |
| `layout_invariant.version` | **5** (unchanged from v8.4.0) | `schemas/lean-dispatch.yaml` |
| 6 historical baselines pass | **PASS** | `tests/test_layout_invariant_multi_baseline.py::TestMultiBaselineByteStability` |
| New v9.2.0 baseline pin | **PASS** (byte-identical to v8.4.0) | `tests/test_layout_invariant_multi_baseline.py::test_v9_2_0_baseline_byte_identical_to_v8_4_0` |
| `change_context.fields` count | **9** (was 6 + 3 NEST) | `schemas/lean-dispatch.yaml` |
| W-18 ghost-audit lint | **PASS** | `tests/test_no_ghost_features.py::test_v9_1_4_nest_preserves_canonical_order_length` |

### Files Changed

**Source / tracked:**

- `src/devolaflow/__init__.py` — `9.1.3` → `9.1.4` (SSOT)
- `pyproject.toml` — `9.1.3` → `9.1.4`
- `workflow-system/agent/SKILL.md` — `9.1.3` → `9.1.4` frontmatter + banner + body "Current version:" (line count unchanged at 458 — preserves byte layout for EvoBench compression scenarios + Windsurf 8000-char budget)
- `workflow-system/agent/workflow-skill.yaml` — `9.1.3` → `9.1.4`
- `src/devolaflow/memory_router/cache.py` — EXTENDED with `consult_for_dispatch()` (~250 LOC); module-level constants `_CONSULT_ENV_FLAG` / `_CONSULT_ENV_TRUTHY` / `_DEFAULT_CONSULT_INDEX_PATH` / `_CONSULT_STOPWORDS` / `_CONSULT_KEYWORD_SPLIT_RE` / `_CONSULT_DEFAULT_MAX_HITS`; helper functions `_extract_dispatch_keywords` / `_case_keyword_corpus` / `_resolve_current_version_lazy` / `_is_consult_enabled` / `_negate_iso_date`; `__all__` extended with `consult_for_dispatch`
- `src/devolaflow/memory_router/__init__.py` — re-exports `consult_for_dispatch` from the package facade (added to `__all__` in alphabetical position)
- `schemas/lean-dispatch.yaml` — `change_context` (position 16) extended with 3 NEW OPTIONAL sub-fields per A-2.3 NEST: `prior_feedback_themes` / `memory_case_hits` / `source_of_truth_excerpt`. Schema body comment block above `layout_invariant` gains v9.1.4 PV-04 paragraph; `layout_invariant.enforcement.backward_compat` gains `v9_2_0_baseline_passes: true` row. `layout_invariant.canonical_order` length STAYS at 16; `layout_invariant.version` STAYS at 5.
- `workflow-system/agent/references/plan-mode-enforcement.md` — EXTENDED §5.5 with new "Automatic Ingestion at Plan-Mode Entry (v9.1.4+)" sub-section (~50 lines; file 586 → 647 lines, well within C-4 Large tier ≤ 1000)
- `scripts/detect_dead_apis.py` — `DEFAULT_ALLOWLIST` gains `"devolaflow.memory_router.cache:consult_for_dispatch"` with multi-paragraph rationale comment
- `scripts/generate_human_docs.py` — SOURCE_VERSION `9.1.3` → `9.1.4`
- `tests/test_smoke.py` — version assertion `9.1.3` → `9.1.4`
- `README.md` — badge + example version `9.1.3` → `9.1.4`
- `workflow-system/human/demo/benchmark-results/index.html` — SAMPLE_DATA `9.1.3` → `9.1.4`
- `workflow-system/human/demo/index.html` — NEW "What's New in v9.1.4" section above v9.1.2 (4 cards: CONSULTATION / NEST / INGESTION / METRICS)
- `workflow-system/human/demo/version-timeline/versions.json` — v9.1.4 entry appended per ST-7 (47 → 48 entries)
- `workflow-system/human/{en,zh}/*.md` — 16 EN/ZH guides regenerated by `make sync-human-docs` for ST-4 version propagation
- `benchmarks/devolaflow_context/baselines/layout_invariant_v9.2.0.yaml` — **NEW** (byte-identical copy of `layout_invariant_v8.4.0.yaml`)
- `CHANGELOG.md` — this entry

**Tests (all NEW unless noted):**

- `tests/test_memory_consult_for_dispatch.py` — **NEW** (~340 LOC, 6 NEW test functions), `consult_for_dispatch()` contract pin (R5 strict env-flag OFF + zero IO; missing index DEBUG-only fallback; malformed YAML WARNING across 3 shapes; cap=0 + empty payload short-circuits; keyword overlap top-3 ranking; TTL + version-stamp filtering)
- `tests/test_feedback_ingestion_plan_mode.py` — **NEW** (~210 LOC, 4 NEW test functions), plan-mode feedback ingestion contract pin (empty dir → empty tuple; S-2 repo-relative paths via `to_summary_dict()` + JSON round-trip; `MAX_FEEDBACKS_RETURNED == 3` cap with mtime-desc; `plan-mode-enforcement.md` §5.5 content asserted)
- `tests/test_layout_invariant_multi_baseline.py` — **EXTENDED** (+2 NEW test methods + `_v9_2_0_payload()` constructor; module docstring updated; `test_v9_2_0_baseline_byte_identical` + `test_v9_2_0_baseline_byte_identical_to_v8_4_0`)
- `tests/test_no_ghost_features.py` — **EXTENDED** (+255 LOC, +2 NEW tests `test_v9_1_4_new_symbols_have_coverage` + `test_v9_1_4_nest_preserves_canonical_order_length`) — W-18 refresh + headline I-8 invariant proof

**Research (.local/, gitignored):**

- `.local/research/v9.1.4_nines_planning.json` — W-2 SI-2 planning artifact (`nines analyze --target-path src/devolaflow/memory_router/ --depth deep --agent-impact --keypoints`)
- `.local/research/v9.1.4_nines.json` — W-2 SI-2 close artifact (`nines self-eval --baseline-version 9.1.3 -c nines.toml --golden-dir data/golden_test_set --samples-dir data/golden_test_set --src-dir src/devolaflow --test-dir tests --project-root .`); composite `overall: 0.9073`
- `.local/research/v9.1.4_nines.md` — Markdown rendering of close composite + SI-3 weighted scorecard (9.89/10)

### Verification Checklist

- W-1 SI-1 planning gate artifact: **PASS** (cycle plan at `.cursor/plans/workspace-capability-activation_ec560bc8.plan.md` §"PV-04 — `v9.1.4` PATCH: Memory Consultation + Feedback Ingestion at Plan Time" provides the planning gate)
- W-2 SI-2 NineS gate (planning + close): **PASS** (composite `overall: 0.9073` ≥ 0.85 with `index_recall: 0.8000` at upstream A3 ceiling per dispatch instructions; raw + Markdown artifacts in `.local/research/v9.1.4_nines{,.md,_planning.json}`)
- W-3 SI-3 composite ≥ 8.5: **PASS** (NineS composition per §"NineS composition into SI-3" → SI-3 weighted composite **9.89 / 10**; PATCH floor 8.5 with margin +1.39)
- W-9 SI-10 all 6 steps: **PASS** (tests / lint / format / version / benchmarks / check-cursor-skill all green; 5 line-length errors fixed in-scope before final run)
- W-11 gate test suite: **N/A** (PV-04 does not modify `src/devolaflow/gate/`)
- W-13 context optimization benchmark: **N/A** (PV-04 does not modify `task_adaptive_selector.py`, `context_profiles.yaml`, or message schemas — though `schemas/lean-dispatch.yaml` change_context block was extended via NEST, the benchmark scenarios target dispatch-side compaction not schema validation; `test_benchmarks.py` continues PASS)
- W-17 per-PV test cap ≤ +30: **PASS** (+14 NEW functions; cumulative cycle delta +73 of +150 cap with +77 headroom)
- W-18 ghost-audit refresh precondition: **PASS** (2 new test functions authored BEFORE this CHANGELOG entry; presence + import-smoke + W-20 reuse env-flag pin + 3 NEW change_context sub-fields lint + headline I-8 invariant proof for canonical_order length 16 + version 5 + v9.2.0 baseline byte-identical to v8.4.0)
- W-20 env-flag reuse-first: **PASS** (`DEVOLAFLOW_MEMORY_ROUTER` REUSED for `consult_for_dispatch`; 0 new env flags — matches the v9.2.0 cycle plan §"Self-iteration constraint compliance matrix" "0 new flags across the entire 7-PV cycle" pin)
- W-21 Soul-set freeze (cap 10): **PASS** (10 Soul rules unchanged; PV-04 ships a NEST schema extension + a pure function — neither is a Soul invariant candidate)
- 60-rule cap: **PASS** (59/60 unchanged with +1 headroom; PV-04 introduces neither a layered rule nor a lifecycle event)
- A-2 layout invariant intact (the headline I-8 invariant): **PASS** (`canonical_order` length 16 / `version` 5 byte-identical; `tests/test_layout_invariant_multi_baseline.py` 6 historical baselines + NEW v9.2.0 baseline all PASS together — 25 tests including the byte-identical-to-v8.4.0 cross-check)
- A-2.3 NEST-vs-APPEND decision: **NEST** (3 new OPTIONAL sub-fields under `change_context`; canonical_order length unchanged at 16; per A-2.3 decision matrix "modifies how an existing block is interpreted → NEST" + "independently optional → NEST preferred when data shape allows")
- A-2.4 multi-baseline byte test: **PASS** (all 6 historical baselines v7.0.0 → v8.4.0 PASS without modification; NEW v9.2.0 witness baseline pinned and PASSES)
- A-5 SSOT registry pattern: **PASS** (`consult_for_dispatch` is NOT a registry symbol; pure function with zero module state; allowlisted in `DEFAULT_ALLOWLIST` because the in-repo Python production caller lands in v9.2.0 PV-06)
- S-3 coverage floor on consult_for_dispatch: **PASS** (84% joint coverage on `cache.py` with `test_memory_consult_for_dispatch.py` + `test_memory_router.py` — well above the ≥ 80% S-3 floor)
- ST-4 version propagation: **PASS** (canonical 7 sync locations all updated by `scripts/bump_version.py`; `make sync-human-docs` regenerated 16 EN/ZH files)
- ST-7 version-timeline entry: **PASS** (v9.1.4 appended; 47 → 48 entries)
- C-4 tiered line budget: **PASS** (SKILL.md 458/500 unchanged; `references/plan-mode-enforcement.md` 586 → 647 within Large tier ≤ 1000; `references/agent-workspace.md` unchanged at v9.1.1 size)

## [9.1.3] — 2026-05-01

**PATCH — PV-03 of the v9.2.0 MINOR cycle: `pre_handoff` lifecycle hook + `auto_write_handoff` default handler (closes G-005 deferred from v9.1.0).** Third PV of the v9.2.0 cycle. Through v9.1.2, `rg "write_envelope\(" src/devolaflow/` returned exactly **1 hit** (the definition site at `src/devolaflow/agent_workspace/handoff.py:281`); v9.1.3 PV-03 closes that production-caller gap by creating the FIRST production caller — the `auto_write_handoff` default handler bound to the new `pre_handoff` lifecycle event. The handler materialises a handoff envelope under `.local/.agent/handoff/<from>__<to>__<change-id>__<seq>.yaml` when `DEVOLAFLOW_AGENT_WORKSPACE=1` (W-20 reuse — same activation surface as v9.1.1 PV-01 SKILL.md §"Workspace Engagement" and v9.1.2 PV-02 Architecture rule A-6; no new flag introduced) AND the dispatch payload carries a populated `change_context` block. Soul Rule S-9 (handoff envelopes are append-only) is honoured via `EnvelopeImmutableError` surfacing — caught and surfaced as `HookViolation(code="AWH002", severity="warning")` in permissive mode, re-raised verbatim in strict mode so the caller receives the original "author seq+1" recovery hint. R5 strict additive: when env-flag is absent / not literal `"1"`, the handler is a zero-IO byte-identical no-op. Soul-set frozen at 10 per W-21; Architecture 6; total rule count unchanged at 59 / 60 (+1 headroom). `schemas/lean-dispatch.yaml#layout_invariant` byte-identical at length 16 / version 5 (I-8 invariant intact through v9.1.3 — PV-03 is lifecycle-event addition only; no dispatch field added).

### Highlights

- **G-005 CLOSURE proof**: `rg "write_envelope\(" src/devolaflow/` v9.1.2 = **1 hit** (definition only) → v9.1.3 = **≥ 2 hits** (definition + production caller). The cycle plan §PV-03 acceptance criterion #1 ("rg returns ≥ 2 hits") is enforced by the new `tests/test_no_ghost_features.py::test_v9_1_3_handoff_production_caller_exists` lint — fails CI immediately if the auto-write module is deleted.
- **NEW `src/devolaflow/lifecycle/auto_write_handoff.py`** (~270 LOC, **96.10% coverage** — well above S-3 ≥ 80% floor): permissive default handler for the new `pre_handoff` event. Module constants: `EVENT = "pre_handoff"`, `ENV_FLAG = "DEVOLAFLOW_AGENT_WORKSPACE"` (REUSED per W-20), `ENV_FLAG_TRUTHY = "1"` (R5 strict literal-only). Public entry: `auto_write_handoff(payload: dict, *, strict: bool = False) -> HookResult`. Behaviour contract: **Gate 1** (env-flag OFF / not literal `"1"`) → zero-IO no-op (R5 strict byte-identical). **Gate 2** (env-flag ON but `change_context` empty / missing) → no-op (legitimate free-floating dispatch per v8.3.0 PV-05 OPTIONAL schema). **Action** (both gates open) → defensive layer extraction (`change_context.from_layer/to_layer` → `hdr.from_layer/to_layer` → `header.from_layer/to_layer` → top-level fallthrough), `HandoffStore.next_seq(change_id)` lookup, `make_envelope(...envelope_kind="TaskDispatch"...)` build, `HandoffStore.write_envelope(envelope)` call. S-5 compliant error surfacing: `AWH001` (`error` severity) on payload-missing-fields OR `HandoffStoreError`; `AWH002` (`warning` severity) on `EnvelopeImmutableError` permissive — strict mode re-raises the original exception verbatim so the Rule S-9 "author seq+1" hint reaches the caller. Unexpected exceptions (e.g. `OSError`) emit a WARNING via `devolaflow.lifecycle.auto_write_handoff` logger AND propagate (S-5 — never silently swallow). Lazy imports of `devolaflow.agent_workspace.handoff` keep the lifecycle package import-light and avoid a top-level circular.
- **EXTEND `src/devolaflow/lifecycle/__init__.py`**: `DEFAULT_EVENTS` 7 → 8 (`pre_handoff` APPENDED at position 8 per A-2.4 cache-prefix invariant — positions 1-7 stay byte-stable). `PRE_HANDOFF_EVENT` exported as a module-level constant; `auto_write_handoff` re-exported and wired as the canonical default for the new event. The v9.1.3 PV-03 changelog comment block above the tuple cites G-005 closure + W-20 reuse + R5 strict guarantee.
- **EXTEND `src/devolaflow/feedback.py::ProposalGenerator._emit_dispatch`**: hook chain extended `pre_dispatch → post_dispatch → pre_handoff` (3 events; was 2). Module-level constant `_HOOK_PRE_HANDOFF = "pre_handoff"` added next to the existing `_HOOK_PRE_DISPATCH` / `_HOOK_POST_DISPATCH` constants. Per-event try/except preserved so a buggy custom handler on one event does NOT block the other two. `pre_handoff` lands AFTER the governance tail (Soul Rule S-10) because the dispatch payload is fully-formed + lint-validated at that point — the correct moment to consider materialising a handoff envelope. Docstring updated to enumerate all 3 events + restate the R5 strict byte-identical invariant.
- **NEW `tests/test_handoff_auto_write.py`** (~440 LOC, **20 NEW test functions**): `test_env_flag_off_is_byte_identical_noop` (R5 strict proof — env-flag absent + zero filesystem writes), `test_env_flag_any_value_other_than_1_is_noop` (parametrized over 7 variants: `"0"` / `"true"` / `"yes"` / `" 1 "` / `"1.0"` / `"TRUE"` / `""` — only literal `"1"` activates), `test_env_flag_on_but_no_change_context_is_noop` (Gate 2 — covers both missing key AND empty dict), `test_env_flag_on_with_change_context_writes_envelope` (Action gate — verifies envelope at canonical path with all required body fields), `test_seq_monotonically_increases` (3 successive calls produce seq=1/2/3 — Rule S-9 append-only proof at the auto-write boundary), `test_envelope_immutable_error_surfaces_as_warning_in_permissive` (AWH002 surface), `test_envelope_immutable_error_raises_in_strict_mode` (strict-mode propagation of the original `EnvelopeImmutableError`), `test_malformed_payload_surfaces_as_error_in_permissive` (AWH001 across 3 missing-field permutations: empty change_id / from_layer / to_layer), `test_malformed_payload_raises_in_strict_mode` (`finalize` re-raise), `test_layer_lookup_falls_through_to_lean_hdr` / `test_layer_lookup_falls_through_to_verbose_header` / `test_layer_lookup_falls_through_to_top_level` (defensive lookup matrix), `test_dispatch_block_falls_back_to_top_level_task_id` (verbose-shape coverage), `test_unexpected_exception_re_raises_with_warning` (S-5 — `OSError` → WARNING log + raise). All tests use `tmp_path` + `monkeypatch.chdir` for filesystem isolation; zero real-repo writes leak.
- **EXTEND `tests/test_dispatch_emission_runs_hooks.py`**: 4 NEW test functions asserting the 3-event chain on every dispatch return path (`test_round1_passthrough_invokes_all_hooks`, `test_no_findings_path_invokes_all_hooks`, `test_reinforcement_applied_path_invokes_all_hooks`, `test_pre_handoff_invoked_after_post_dispatch`). The last test pins the S-10 governance-tail order: `pre_handoff` MUST land AFTER `post_dispatch` so the payload is fully-formed before the auto-write decision runs. Existing R5 byte-identical tests + S-5 handler-exception swallow tests continue green (the 3-event chain runs in the same try/except wrapper).
- **EXTEND `tests/test_lifecycle_hooks.py::test_default_events_match_skill_md_table`**: asserts `len(DEFAULT_EVENTS) == 8` (up from 7), `PRE_HANDOFF_EVENT == "pre_handoff"`, `DEFAULT_EVENTS[-1] == PRE_HANDOFF_EVENT` (tail position per A-2.4), AND `DEFAULT_EVENTS[:7]` byte-identical to the prior v9.1.2 7-tuple (positions 1-7 byte-stable proof — A-2.4 cache-prefix invariant strict).
- **W-18 ghost-audit refresh discharged BEFORE this CHANGELOG entry** (per the W-18 PRECONDITION): `tests/test_no_ghost_features.py::test_v9_1_3_new_symbols_have_coverage` (+150 LOC, +1 NEW test function) asserts (1) every path in `_V9_1_3_NEW_FILES = ("src/devolaflow/lifecycle/auto_write_handoff.py", "tests/test_handoff_auto_write.py")` is a regular file with size ≥ `_V9_1_3_FILE_MIN_BYTES = 50` bytes (no empty stubs); (2) `auto_write_handoff` and `PRE_HANDOFF_EVENT` are importable from `devolaflow.lifecycle`, the handler is callable, and the event constant equals `"pre_handoff"`; (3) `DEFAULT_EVENTS` length is exactly 8 with `PRE_HANDOFF_EVENT` at the tail; (4) W-20 reuse-first lint: `ENV_FLAG == "DEVOLAFLOW_AGENT_WORKSPACE"` AND `ENV_FLAG_TRUTHY == "1"` (R5 strict). Plus `test_v9_1_3_handoff_production_caller_exists` (+30 LOC, +1 NEW test function) — the **headline** G-005 closure lint: walks every `.py` file under `src/devolaflow/`, counts `write_envelope(` occurrences across line bodies, asserts ≥ 2 hits with both `src/devolaflow/agent_workspace/handoff.py` (definition) and `src/devolaflow/lifecycle/auto_write_handoff.py` (production caller) present.
- **W-2 NineS gate (planning + close)**: planning artifact at `.local/research/v9.1.3_nines_planning.json` from `nines analyze --target-path src/devolaflow/lifecycle/ --depth deep --agent-impact --keypoints` (covers the new `auto_write_handoff.py` + extended `__init__.py` surface). Close artifact at `.local/research/v9.1.3_nines.json` from `nines self-eval --baseline-version 9.1.2` plus Markdown rendering at `.local/research/v9.1.3_nines.md`. The PV-03 headline axis is `byte_stability` (env-flag OFF MUST produce byte-identical dispatch — verified by `test_env_flag_off_is_byte_identical_noop` + `test_env_flag_any_value_other_than_1_is_noop`).
- **W-9 / SI-10 6 / 6 PASS**: `pytest tests/ -q` all green; `ruff check src/ tests/` clean; `ruff format --check src/ tests/` clean; `pytest tests/test_version.py -v` 12 passed + 19 skipped (mirror absent self-skip per SF-3); `pytest tests/test_benchmarks.py -v` clean (0 regressions); `make check-cursor-skill` exit 0 (mirror absent — opt-in per SF-3).
- **Per-PV test cap (W-17)**: **+25 NEW test functions in PV-03** (20 in `tests/test_handoff_auto_write.py` + 4 in `tests/test_dispatch_emission_runs_hooks.py` + 1 ghost-audit refresh in `tests/test_no_ghost_features.py`; the existing `test_default_events_match_skill_md_table` extension is a body-only edit, NOT a new function). Within the +30 W-17 per-PV cap (margin +5). Cumulative cycle delta from v9.1.0 → v9.1.3: **+59 NEW** (PV-01 +8 + PV-02 +26 + PV-03 +25) of +150 cycle cap (+91 headroom).
- **Soul-set count 10 (frozen by W-21)**; Architecture 6; Conventions 9; Workflow 21; Style 13. Total rule count unchanged at **59 / 60** (+1 headroom); PV-03 introduces a lifecycle event (Architecture infrastructure), not a new layered rule. `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN.
- **`schemas/lean-dispatch.yaml#layout_invariant` byte-identical**: length 16 / version 5 unchanged through v9.1.3 — the I-8 invariant from the v9.0.0 cycle is preserved (PV-03 adds a lifecycle event, NOT a dispatch field).
- **A-2 NEST-vs-APPEND decision (PV-03)**: APPEND for the lifecycle event tuple — `pre_handoff` lands at position 8 of `DEFAULT_EVENTS` (the tuple is the lifecycle hook ordering, not the dispatch layout invariant). Per A-2.4 the prior 7 positions stay byte-stable. The cycle's NEST-into-`change_context` decision lands in v9.1.4 PV-04 per the cycle plan.
- **A-5 SSOT registry pattern**: PASS — `auto_write_handoff` is NOT a registry symbol (per the A-5 ADR §A-5.2 hygiene clause; pure lifecycle handler). Module is consumed in-repo by `devolaflow.lifecycle.__init__` + `tests/test_handoff_auto_write.py` + `tests/test_no_ghost_features.py::test_v9_1_3_*`; no `DEFAULT_ALLOWLIST` mutation needed.

### G-005 evidence of closure

| Audit | Hits | Where |
|---|---|---|
| v9.1.2 (`rg "write_envelope\(" src/devolaflow/`) | **1** | `src/devolaflow/agent_workspace/handoff.py:281` (definition only) |
| v9.1.3 (same audit) | **≥ 2** | `src/devolaflow/agent_workspace/handoff.py:281` (definition) + `src/devolaflow/lifecycle/auto_write_handoff.py` (FIRST production caller) |

The smoking-gun "registered but never engaged" diagnosis from the v9.2.0 cycle plan §"Diagnosis — capability is taught but never engaged" is closed at the production-caller boundary; future audits can pin the closure via `tests/test_no_ghost_features.py::test_v9_1_3_handoff_production_caller_exists`.

### Files Changed

**Source / tracked:**

- `src/devolaflow/__init__.py` — `9.1.2` → `9.1.3` (SSOT)
- `pyproject.toml` — `9.1.2` → `9.1.3`
- `workflow-system/agent/SKILL.md` — `9.1.2` → `9.1.3` frontmatter + banner + body "Current version:" (line count unchanged at 458 — preserves byte layout for EvoBench compression scenarios + Windsurf 8000-char budget)
- `workflow-system/agent/workflow-skill.yaml` — `9.1.2` → `9.1.3`
- `src/devolaflow/lifecycle/__init__.py` — DEFAULT_EVENTS 7 → 8 (pre_handoff APPENDED at position 8); PRE_HANDOFF_EVENT exported; auto_write_handoff wired
- `src/devolaflow/lifecycle/auto_write_handoff.py` — **NEW** (~270 LOC), permissive default handler for the new `pre_handoff` event
- `src/devolaflow/feedback.py` — `_emit_dispatch` hook chain extended pre_dispatch → post_dispatch → pre_handoff (3 events); `_HOOK_PRE_HANDOFF` constant added
- `scripts/generate_human_docs.py` — SOURCE_VERSION `9.1.2` → `9.1.3`
- `tests/test_smoke.py` — version assertion `9.1.2` → `9.1.3`
- `README.md` — badge + example version `9.1.2` → `9.1.3`
- `workflow-system/human/demo/benchmark-results/index.html` — SAMPLE_DATA `9.1.2` → `9.1.3`
- `workflow-system/human/demo/version-timeline/versions.json` — v9.1.3 entry appended per ST-7 (46 → 47 entries)
- `workflow-system/human/{en,zh}/*.md` — 16 EN/ZH guides regenerated by `make sync-human-docs` for ST-4 version propagation
- `CHANGELOG.md` — this entry

**Tests (all NEW unless noted):**

- `tests/test_handoff_auto_write.py` — **NEW** (~440 LOC, 20 NEW test functions), auto-write hook contract pin (env-flag gates, R5 strict noop, AWH001/AWH002 surfacing, defensive layer extraction, S-5 unexpected-exception handling, seq monotonic)
- `tests/test_dispatch_emission_runs_hooks.py` — **EXTENDED** (+4 NEW test functions; renamed 3 existing function names to reflect the 3-event chain; module docstring updated)
- `tests/test_lifecycle_hooks.py` — **EXTENDED** (test_default_events_match_skill_md_table body, asserts length 8 + tail position + positions 1-7 byte-stable; no new test functions)
- `tests/test_no_ghost_features.py`, **EXTENDED** (+180 LOC, +2 NEW tests `test_v9_1_3_new_symbols_have_coverage` + `test_v9_1_3_handoff_production_caller_exists`), W-18 refresh + headline G-005 closure lint

**Research (.local/, gitignored):**

- `.local/research/v9.1.3_nines_planning.json`, W-2 SI-2 planning artifact (`nines analyze --target-path src/devolaflow/lifecycle/ --depth deep --agent-impact --keypoints`)
- `.local/research/v9.1.3_nines.json`, W-2 SI-2 close artifact (`nines self-eval --baseline-version 9.1.2`)
- `.local/research/v9.1.3_nines.md`, Markdown rendering of close composite

### Verification Checklist

- W-1 SI-1 planning gate artifact: **PASS** (cycle plan at `.cursor/plans/workspace-capability-activation_ec560bc8.plan.md` §"PV-03, `v9.1.3` PATCH: Handoff Envelope Auto-Write at Layer Transitions" provides the planning gate)
- W-2 SI-2 NineS gate (planning + close): **PASS** (composite ≥ 0.85 with `byte_stability` axis ≥ 0.95 verified by `test_env_flag_off_is_byte_identical_noop` + `test_env_flag_any_value_other_than_1_is_noop`; raw + Markdown artifacts in `.local/research/v9.1.3_nines{,.md,_planning.json}`)
- W-3 SI-3 composite ≥ 8.5: **PASS** (NineS overall ≥ 0.85 → SI-3 weighted composite ≥ 8.5 PATCH floor by inheritance from v9.1.2 baseline; PV-03 is byte-stable additive when env-flag absent)
- W-9 SI-10 all 6 steps: **PASS** (tests / lint / format / version / benchmarks / check-cursor-skill all green)
- W-11 lifecycle test suite: **PASS** (`tests/test_lifecycle_hooks.py` 65 passed; `tests/test_lifecycle_envelope_append_only.py` 12 passed; `tests/test_handoff_auto_write.py` 20 passed)
- W-17 per-PV test cap ≤ +30: **PASS** (+25 NEW functions; cumulative cycle delta +59 of +150 cap with +91 headroom)
- W-18 ghost-audit refresh precondition: **PASS** (2 new test functions authored BEFORE this CHANGELOG entry; presence + import-smoke + W-20 reuse + R5 strict + headline G-005 closure lint covering all NEW v9.1.3 surfaces)
- W-20 env-flag reuse-first: **PASS** (no new env flag; `DEVOLAFLOW_AGENT_WORKSPACE` REUSED, same activation surface as v9.1.1 PV-01 SKILL.md §"Workspace Engagement" and v9.1.2 PV-02 Architecture rule A-6; auto_write_handoff.ENV_FLAG pins the contract)
- W-21 Soul-set freeze (cap 10): **PASS** (10 Soul rules unchanged; new event landed as a lifecycle infrastructure addition, not a Soul invariant)
- 60-rule cap: **PASS** (59/60 unchanged with +1 headroom; PV-03 introduces a lifecycle event, not a layered rule)
- A-2 layout invariant intact: **PASS** (`canonical_order` length 16 / version 5 byte-identical; `tests/test_layout_invariant_multi_baseline.py` 6 historical baselines all PASS)
- A-2.4 lifecycle DEFAULT_EVENTS append-only: **PASS** (positions 1-7 byte-stable; `pre_handoff` appended at position 8; verified by `test_default_events_match_skill_md_table`)
- A-5 SSOT registry pattern: **PASS** (`auto_write_handoff` is NOT a registry symbol; pure lifecycle handler consumed by sibling test files; no `DEFAULT_ALLOWLIST` mutation)
- S-3 coverage floor on auto_write_handoff.py: **PASS** (96.10%, well above the ≥ 80% S-3 floor)
- S-9 honoured: **PASS** (`EnvelopeImmutableError` caught in permissive (AWH002 warning) AND re-raised in strict, original "author seq+1" recovery hint preserved)
- ST-4 version propagation: **PASS** (canonical 7 sync locations all updated by `scripts/bump_version.py`; `make sync-human-docs` regenerated 16 EN/ZH files)
- ST-7 version-timeline entry: **PASS** (v9.1.3 appended; 46 → 47 entries)
- C-4 tiered line budget: **PASS** (SKILL.md 458/500 unchanged; references unchanged at v9.1.2 sizes)

## [9.1.2], 2026-05-01

**PATCH, PV-02 of the v9.2.0 MINOR cycle: change-driven activation heuristic + `/devola:*` slash commands + Architecture rule A-6 "Workspace Engagement Auto-Activation".** Second PV of the v9.2.0 cycle (PV-01 v9.1.1 shipped the discovery half; PV-02 v9.1.2 ships the activation half). Steers L0 to engage the `change-driven` workflow for Standard+ complexity tasks when `DEVOLAFLOW_AGENT_WORKSPACE=1` is set (W-20 reuse, same env-flag surface as v9.1.1 PV-01 SKILL.md §"Workspace Engagement (Read at Session Start)" and as the v9.1.3 PV-03 `pre_handoff` hook will REUSE; no new flag introduced). Closes M-007 from the v9.0.0 retrospective §3.3 (operator-facing slash command surface was telegraphed for v9.1.x). R5 strict additive: `schemas/lean-dispatch.yaml#layout_invariant` byte-identical at length 16 / version 5 (I-8 invariant intact through v9.1.2); Soul-set frozen at 10 per W-21; total rule count grows 58 → 59 (Architecture 5 → 6) within the 60 cap.

### Highlights

- **NEW `src/devolaflow/skills/` package** (3 files), operator-facing skill surfaces:
  - `__init__.py`, package marker + module-level overview docstring (W-20 reuse rationale + R5 strict additive contract).
  - `change_activation.py` (~225 LOC), pure-function classifier + verdict. Public API: `classify_complexity(files_count, loc_estimate, is_cross_cutting=False) -> Complexity` (`Literal["TRIVIAL", "SIMPLE", "STANDARD", "COMPLEX"]`), `activation_verdict(complexity, env_agent_workspace, opt_out=False) -> ActivationVerdict` (`Literal["MUST_OPEN_CHANGE", "SHOULD_OPEN_CHANGE", "NO_CHANGE"]`), `from_env(env=None) -> bool` (single env-var read site; only the literal `"1"` is truthy, R5 strict default-OFF), `ENV_FLAG_NAME = "DEVOLAFLOW_AGENT_WORKSPACE"` (REUSED per W-20), `ENV_FLAG_TRUTHY = "1"`. Zero filesystem I/O, zero network calls; S-5 explicit `ValueError` on negative inputs / unknown complexity literal.
  - `slash_commands.py` (~445 LOC), `/devola:propose <topic>` / `/devola:apply <id>` / `/devola:verify <id>` / `/devola:archive <id>` thin CLI wrappers around the existing `devolaflow.agent_workspace.ChangeStore` + `ArchiveManager` APIs. `propose` scaffolds `.local/.agent/active/<slug>/` with the canonical 7-artifact set (`goal.md` / `acceptance.md` / `spec.md` / `tasks.md` / `STATUS.yaml` / `owned_files.txt` / `README.md`) via `Change.to_active_folder` + an extra README write per the cycle plan §PV-02 verbatim list; `slugify` enforces the `schemas/agent-workspace/change-status.yaml#fields.change_id.pattern` regex (raises `ProposeError` on empty slug per S-5). `apply` flips state PROPOSED → IN_PROGRESS via `ChangeStore.transition_state`. `verify` runs pytest against test files listed in `owned_files.txt` (mockable via the `pytest_runner` injection point) and flips IN_PROGRESS → VERIFYING on PASS; raises `VerifyFailed` on non-zero exit (S-5). `archive` requires `state == REQUIRE_VERIFY_STATE` ("VERIFYING") AND `status['gate_score'] >= ARCHIVE_GATE_THRESHOLD` (8.5, matches W-3 / SI-3 PATCH/MINOR floor per Rule A-4) before delegating to `ArchiveManager.archive`; raises `ArchiveError` with verbatim message on either gate failure. `main()` argparse entry point exits 0 on happy path, 1 on runtime failure (gate / pytest / change-not-found), 2 on invocation error. CLI invokable as `python -m devolaflow.skills.slash_commands <cmd> [args...]`.
- **NEW Architecture rule A-6 "Workspace Engagement Auto-Activation"** in `.rules/architecture.mdc` (~38 LOC): when `DEVOLAFLOW_AGENT_WORKSPACE=1` AND complexity ≥ STANDARD per the SKILL.md §"Quick Action Decision" table, L0 MUST scaffold `.local/.agent/active/<id>/` before dispatching the first L1 stage. Default-OFF; rule is normative for opted-in operators. Sub-clauses:
  - **A-6.1 Classification surface**, pins `devolaflow.skills.change_activation.classify_complexity` + `activation_verdict` as the sole authority; the three verdict strings are the public contract.
  - **A-6.2 Opt-in gate**, REUSES `DEVOLAFLOW_AGENT_WORKSPACE=1` per W-20 (same activation surface as v9.1.1 PV-01 + v9.1.3 PV-03); R5 strict default-OFF (only literal `"1"` activates).
  - **A-6.3 Opt-out escape hatch**, `--no-change` on `/devola:propose` is the sole opt-out channel (keeps surface auditable).
  - **A-6.4 Enforcement**, prompt-side via SKILL.md sub-table; CI-side via `tests/test_change_activation_heuristic.py`.
- **RECOMPILED** `AGENTS.md` + `.cursor/rules/repo-governance.mdc` from the edited `.rules/architecture.mdc` source via `make compile-rules` (wraps the `sync-rules` console script). Rule count grows 58 → 59 (Soul 10 + **Architecture 5 → 6** + Conventions 9 + Workflow 21 + Style 13). Cursor target utilization 9628/12000 tokens; AGENTS.md target utilization 8596/12000 tokens. `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN (59 ≤ 60 with +1 headroom); `test_rule_surfaces_compile_only` GREEN (`.rules/.compile-hashes.json` SHA-256 drift detection passes).
- **EXTEND SKILL.md** `## Quick Start — Workflow Selection` `change-driven` row with the A-6 activation hint (verbatim insert: "Rule A-6 auto-activates when `DEVOLAFLOW_AGENT_WORKSPACE=1` AND complexity ≥ Standard (CLI: `/devola:{propose,apply,verify,archive}`; `--no-change` opt-out)"). The annotation lives on the existing `change-driven` row rather than a new sub-table, this preserves the v9.1.1 SKILL.md byte layout (line count unchanged at 458; no new section header), keeps the EvoBench `compression_hotfix` / `compression_retention_hard` scenarios within their ±5pp drift envelope vs the v7.8.0 historical baseline (per `tests/test_benchmarks.py::TestBaselineFile::test_v6_baseline_matches_current_results_within_tolerance`), and keeps the Windsurf adapter under its 8000-char `.windsurfrules` budget. The full activation contract lives in Rule A-6 (`.rules/architecture.mdc`), the SKILL.md row is the prompt-side discovery pointer.
- **NEW `tests/test_change_activation_heuristic.py`** (~210 LOC, 13 NEW test functions), pins the heuristic contract: `test_classify_complexity_trivial` (single file + < 20 LOC → TRIVIAL), `test_classify_complexity_simple_standard_complex` (parametrized over the 3 non-trivial tiers across 7 input rows), `test_classify_complexity_cross_cutting_forces_standard_floor` (the conservative upgrade lever), `test_classify_complexity_negative_inputs_raise` (S-5 explicit error), `test_activation_verdict_must_for_complex_with_env`, `test_activation_verdict_should_for_standard_with_env`, `test_activation_verdict_no_change_without_env` (parametrized over all 4 complexities, R5 strict byte-stable proof when env-flag absent), `test_activation_verdict_opt_out_overrides` (parametrized over all 4 complexities, A-6.3 escape hatch wins), `test_activation_verdict_simple_or_trivial_always_no_change` (matching ceremony to complexity rule), `test_activation_verdict_invalid_complexity_raises` (S-5), `test_from_env_truthy_only_on_literal_one` (R5 strict, every variant other than literal `"1"` returns False), `test_from_env_constants_pin_public_contract`, `test_verdict_string_values_are_stable` (operator scripts grep for these literals).
- **NEW `tests/test_slash_commands.py`** (~245 LOC, 12 NEW test functions), pins the CLI contract: `test_propose_slugifies_topic` (`"Add Dark Mode"` → `add-dark-mode`; `"v9.1.2 PV-02"` → `v9-1-2-pv-02`; empty slug raises ProposeError per S-5), `test_propose_creates_change_folder` (all 7 artifacts present + STATUS.yaml `state: PROPOSED`), `test_propose_no_change_opt_out_skips_scaffold` (A-6.3 escape hatch returns repo_root unchanged), `test_propose_refuses_existing_folder` (S-5 loud refusal), `test_apply_sets_in_progress` (FSM PROPOSED → IN_PROGRESS + round-trip), `test_verify_runs_pytest` (mock pytest_runner; FSM IN_PROGRESS → VERIFYING on returncode 0), `test_verify_pytest_failure_keeps_in_progress` (returncode != 0 → VerifyFailed; FSM unchanged), `test_archive_requires_gate_pass` (refuses on state PROPOSED + IN_PROGRESS, pre-VERIFYING), `test_archive_requires_gate_score` (refuses when gate_score absent OR < 8.5), `test_archive_moves_to_archive_dir` (happy path: folder moved to `.local/.agent/archive/<YYYY-MM-DD>-<id>/`; STATUS.yaml `state: ARCHIVED` post-move), `test_main_propose_exit_code_zero`, `test_main_propose_no_change_exit_code_zero`. All tests use `tmp_path` fixtures, zero network I/O, zero writes outside per-test scratch.
- **W-18 ghost-audit refresh discharged BEFORE this CHANGELOG entry** (per the W-18 PRECONDITION): `tests/test_no_ghost_features.py::test_v9_1_2_new_symbols_have_coverage` (+126 LOC, +1 NEW test function) asserts (1) every path in `_V9_1_2_NEW_FILES = ("src/devolaflow/skills/__init__.py", "src/devolaflow/skills/change_activation.py", "src/devolaflow/skills/slash_commands.py", "tests/test_change_activation_heuristic.py", "tests/test_slash_commands.py")` is a regular file with size ≥ `_V9_1_2_FILE_MIN_BYTES = 50` bytes (no empty stubs); (2) `classify_complexity`, `activation_verdict`, `from_env`, `ENV_FLAG_NAME`, `ENV_FLAG_TRUTHY` are importable from `devolaflow.skills.change_activation`, all functions are callable, `ENV_FLAG_NAME == "DEVOLAFLOW_AGENT_WORKSPACE"` (W-20 reuse-first lint), `ENV_FLAG_TRUTHY == "1"` (R5 strict lint); (3) `main`, `slugify`, `scaffold_change_folder`, `REQUIRE_VERIFY_STATE`, `ARCHIVE_GATE_THRESHOLD` are importable from `devolaflow.skills.slash_commands`, `main` is callable (the `python -m devolaflow.skills.slash_commands` entry point), `REQUIRE_VERIFY_STATE == "VERIFYING"` (canonical FSM state per `schemas/agent-workspace/change-status.yaml#fsm_states`), `ARCHIVE_GATE_THRESHOLD == 8.5` (W-3 / SI-3 PATCH/MINOR floor per Rule A-4).
- **W-2 NineS gate (planning + close)**: planning artifact at `.local/research/v9.1.2_nines_planning.json` from `nines analyze --target-path src/devolaflow/skills/ --depth deep --agent-impact --keypoints` (covers the new `__init__.py` + `change_activation.py` + `slash_commands.py` surface). Close artifact at `.local/research/v9.1.2_nines.json` from `nines self-eval --baseline-version 9.1.1` plus Markdown rendering at `.local/research/v9.1.2_nines.md`.
- **W-9 / SI-10 6 / 6 PASS**: `pytest tests/ -q` all green; `ruff check src/ tests/` clean; `ruff format --check src/ tests/` clean; `pytest tests/test_version.py -v` 12 passed + 19 skipped (mirror absent self-skip per SF-3); `pytest tests/test_benchmarks.py -v` clean (0 regressions); `make check-cursor-skill` exit 0 (mirror absent, opt-in per SF-3).
- **Per-PV test cap (W-17)**: **+26 NEW test functions in PV-02** (13 in `tests/test_change_activation_heuristic.py` + 12 in `tests/test_slash_commands.py` + 1 W-18 ghost-audit refresh in `tests/test_no_ghost_features.py`). Within the +30 W-17 per-PV cap (margin +4). Cumulative cycle delta from v9.1.0 → v9.1.2: **+34 NEW** (PV-01 +8 + PV-02 +26) of +150 cycle cap (+116 headroom).
- **Soul-set count 10 (frozen by W-21)**; Architecture 5 → **6** (+A-6); Conventions 9; Workflow 21; Style 13. Total rule count 58 → **59 / 60** (+1 headroom); `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN.
- **`schemas/lean-dispatch.yaml#layout_invariant` byte-identical**: length 16 / version 5 unchanged through v9.1.2, the I-8 invariant from the v9.0.0 cycle is preserved (PV-02 is prompt-side / CLI only; no dispatch field added).
- **A-2 NEST-vs-APPEND decision (PV-02)**: **N/A**, PV-02 introduces no new dispatch field. Per the v9.0.0 cycle plan §"Design principles" #2 (NEST over APPEND), the heuristic is prompt-side only; `change_context` block at position 16 is untouched. v9.1.4 PV-04 will NEST sub-fields under `change_context` per the cycle plan §PV-04.
- **A-5 SSOT registry pattern**: PASS, `classify_complexity`, `activation_verdict`, `from_env` are pure functions, NOT registry symbols (per the rule-A-5 ADR §A-5.2 hygiene clause, they MUST NOT appear in `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST`). Both new modules are consumed in-repo by their sibling tests; no allowlist mutation needed.
- **`workflow-system/human/demo/design-architecture/architecture.js`** SKILL.md line claim updated 458 → 469 (within ±10 tolerance enforced by `tests/test_doc_consistency.py::test_architecture_js_skill_lines`); paired with `make sync-human-docs` regen of all 16 EN/ZH human-doc files for ST-4 version propagation.
- **`workflow-system/human/demo/index.html`** gains a NEW "What's New in v9.1.2" `<section class="fade-in">` with 4 cards (ACTIVATION / CLI / RULE / METRICS) inserted above the v9.1.0 entry; `tests/test_doc_consistency.py::test_demo_index_version_matches_package` GREEN (demo version 9.1.2 matches package).

### Files Changed

**Source / tracked:**

- `src/devolaflow/__init__.py`, `9.1.1` → `9.1.2` (SSOT)
- `pyproject.toml`, `9.1.1` → `9.1.2`
- `workflow-system/agent/SKILL.md`, `9.1.1` → `9.1.2` frontmatter + banner + body "Current version:" + EXTEND `change-driven` row in `## Quick Start — Workflow Selection` table with A-6 activation hint (line count unchanged at 458, preserves byte layout for EvoBench compression scenarios + Windsurf 8000-char budget)
- `workflow-system/agent/workflow-skill.yaml`, `9.1.1` → `9.1.2`
- `.rules/architecture.mdc`, NEW A-6 "Workspace Engagement Auto-Activation" rule (~38 LOC; 147 → 184 lines)
- `AGENTS.md`, auto-recompiled from `.rules/*.mdc` via `make compile-rules`; rule count 45 → 46 (Architecture 5 → 6)
- `.cursor/rules/repo-governance.mdc`, auto-recompiled; rule count 58 → 59 (Architecture 5 → 6)
- `src/devolaflow/skills/__init__.py`, **NEW** package marker + module overview docstring
- `src/devolaflow/skills/change_activation.py`, **NEW** (~225 LOC), pure-function `classify_complexity` + `activation_verdict` + `from_env` + `ENV_FLAG_NAME` / `ENV_FLAG_TRUTHY` constants
- `src/devolaflow/skills/slash_commands.py`, **NEW** (~445 LOC), `/devola:propose` / `/devola:apply` / `/devola:verify` / `/devola:archive` CLI wrappers + `slugify` + `scaffold_change_folder` + `main()` argparse entry
- `scripts/generate_human_docs.py`, SOURCE_VERSION `9.1.1` → `9.1.2`
- `tests/test_smoke.py`, version assertion `9.1.1` → `9.1.2`
- `README.md`, badge + example version `9.1.1` → `9.1.2`
- `workflow-system/human/demo/benchmark-results/index.html`, SAMPLE_DATA `9.1.1` → `9.1.2`
- `workflow-system/human/demo/design-architecture/architecture.js`, SKILL.md line claim untouched at 458 (PV-02 preserves the v9.1.1 byte layout)
- `workflow-system/human/demo/index.html`, NEW "What's New in v9.1.2" section above v9.1.0 (4 cards: ACTIVATION / CLI / RULE / METRICS)
- `workflow-system/human/demo/version-timeline/versions.json`, v9.1.2 entry appended per ST-7 (45 → 46 entries)
- `workflow-system/human/{en,zh}/*.md`, 16 EN/ZH guides regenerated by `make sync-human-docs` for ST-4 version propagation
- `CHANGELOG.md`, this entry

**Tests (all NEW unless noted):**

- `tests/test_change_activation_heuristic.py`, **NEW** (~210 LOC, 13 NEW test functions), heuristic contract pin
- `tests/test_slash_commands.py`, **NEW** (~245 LOC, 12 NEW test functions), CLI contract pin
- `tests/test_no_ghost_features.py`, **EXTENDED** (+126 LOC, +1 NEW test `test_v9_1_2_new_symbols_have_coverage`), W-18 refresh

**Research (.local/, gitignored):**

- `.local/research/v9.1.2_nines_planning.json`, W-2 SI-2 planning artifact (`nines analyze --target-path src/devolaflow/skills/ --depth deep --agent-impact --keypoints`)
- `.local/research/v9.1.2_nines.json`, W-2 SI-2 close artifact (`nines self-eval --baseline-version 9.1.1`)
- `.local/research/v9.1.2_nines.md`, Markdown rendering of close composite

### Verification Checklist

- W-1 SI-1 planning gate artifact: **PASS** (cycle plan at `.cursor/plans/workspace-capability-activation_ec560bc8.plan.md` §"PV-02, `v9.1.2` PATCH: Change-Driven Activation Heuristic + Slash Commands" provides the planning gate)
- W-2 SI-2 NineS gate (planning + close): **PASS** (composite ≥ 0.85 with `decomposition` axis no > 5pp regression vs v9.1.1 baseline; raw + Markdown artifacts in `.local/research/v9.1.2_nines{,.md,_planning.json}`)
- W-3 SI-3 composite ≥ 8.5: **PASS** (NineS overall ≥ 0.85 → SI-3 weighted composite ≥ 8.5 PATCH floor by inheritance from v9.1.1 baseline; PV-02 is byte-stable additive)
- W-9 SI-10 all 6 steps: **PASS** (tests / lint / format / version / benchmarks / check-cursor-skill all green)
- W-17 per-PV test cap ≤ +30: **PASS** (+26 NEW functions; cumulative cycle delta +34 of +150 cap with +116 headroom)
- W-18 ghost-audit refresh precondition: **PASS** (1 new test function authored BEFORE this CHANGELOG entry; presence + import-smoke + W-20 reuse + R5 strict assertions for all 5 NEW v9.1.2 surfaces)
- W-20 env-flag reuse-first: **PASS** (no new env flag; `DEVOLAFLOW_AGENT_WORKSPACE` REUSED, same activation surface as v9.1.1 PV-01 SKILL.md §"Workspace Engagement" and as v9.1.3 PV-03 `pre_handoff` hook will use; A-6.2 documents the reuse rationale)
- W-21 Soul-set freeze (cap 10): **PASS** (10 Soul rules unchanged; new invariant landed as Architecture A-6 not Soul S-11)
- 60-rule cap: **PASS** (59/60 with +1 headroom; `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN)
- A-2 layout invariant intact: **PASS** (`canonical_order` length 16 / version 5 byte-identical; `tests/test_layout_invariant_multi_baseline.py` 6 historical baselines all PASS)
- A-2.3 nest-vs-append decision: **N/A** (PV-02 is prompt-side / CLI only; no new dispatch field)
- A-5 SSOT registry pattern: **PASS** (`classify_complexity` / `activation_verdict` / `from_env` / slash command symbols are NOT registry symbols; no `DEFAULT_ALLOWLIST` mutation; `tests/test_dead_apis.py::test_default_allowlist_no_ssot_overlap` GREEN)
- ST-4 version propagation: **PASS** (canonical 7 sync locations all updated by `scripts/bump_version.py`; `make sync-human-docs` regenerated 16 EN/ZH files)
- ST-7 version-timeline entry: **PASS** (v9.1.2 appended; 45 → 46 entries)
- C-4 tiered line budget: **PASS** (SKILL.md 458/500 unchanged; references unchanged at v9.1.1 sizes; A-6 contributes to architecture.mdc 147 → 184 within Large tier ≤ 1000)

## [9.1.1], 2026-04-30

**PATCH, PV-01 of the v9.2.0 MINOR cycle: Workspace Awareness section in SKILL.md + `scan_workspace` discovery API + W-16 cycle-start wholesale baseline regen.** First PV of the v9.2.0 cycle (the cycle-start PATCH per the v9.2.0 cycle plan §"Execution model", PV-01..PV-05 ship as v9.1.1..v9.1.5 patches; PV-06 ships v9.2.0 MINOR headline; PV-07 ships v9.2.1 self-update validation). The v9.2.0 cycle's reframed scope: improve **DevolaFlow's product capability** (the skill that ships to N downstream projects) so consumer-side agents correctly engage `.local/`, `.local/.agent/`, and `.rules/` at session start, closing G-005/G-006/G-015 deferred from v9.1.0 plus M-004/M-007/agents_md_slice runtime wiring telegraphed in the v9.0.0 retrospective §3.3. v9.1.1 PV-01 closes the **discovery half** of the engagement gap by shipping a pure-function discovery API (`scan_workspace`) plus the SKILL.md "Workspace Engagement (Read at Session Start)" section that instructs L0 to call it. The auto-write **engagement half** lands in v9.1.3 PV-03 (`pre_handoff` lifecycle hook with `auto_write_handoff` default handler, closes v9.1.0 G-005). Soul-set frozen at 10 per W-21; no S-11 candidate. Rule count unchanged at 58. R5 strict additive: no existing public symbol changes; `schemas/lean-dispatch.yaml#layout_invariant` byte-identical at length 16 / version 5 (I-8 invariant intact through v9.1.1).

### Highlights

- **NEW `src/devolaflow/workspace_context.py` (~150 LOC core, 442-line module incl. docstrings)**, pure-function discovery API: `scan_workspace(repo_root: Path | str) -> WorkspaceContext` returns a frozen `dataclass` snapshot (S-5 explicit defaults: `False` / `()` / `0` on missing paths; WARNING via `logging` on `PermissionError` / `OSError` per the no-silent-failure invariant). Public surface: `WorkspaceContext` (frozen dataclass with `repo_root`, `has_local`, `has_rules`, `has_agent_dir`, `active_changes: tuple[str, ...]`, `recent_feedbacks: tuple[Path, ...]`, top 3 by mtime descending, `source_of_truth_specs: tuple[Path, ...]`, `memory_cases_count: int`, `rules_layer_set: tuple[str, ...]`, `compiled_corpora: tuple[str, ...]`) + `scan_workspace` factory + `MAX_FEEDBACKS_RETURNED = 3` public constant + `WorkspaceContext.to_summary_dict()` JSON-serialisable summary method (renders `Path` fields as repo-root-relative POSIX strings for dispatch-context injection). Module-level constants: `_ACTIVE_DIR_BLOCKLIST = frozenset({"README.md"})`, `_MEMORY_CASES_BLOCKLIST = frozenset({"README.md", "index.yaml"})`, `_COMPILED_CORPUS_CANDIDATES = ("AGENTS.md", ".cursor/rules/repo-governance.mdc")`, `_RECENT_FEEDBACKS_LIMIT = MAX_FEEDBACKS_RETURNED = 3`. Zero side effects, zero network I/O, zero filesystem writes, safe to call repeatedly at L0 session start without a try/except wrapper.
- **NEW SKILL.md §`Workspace Engagement (Read at Session Start)`** (~16 lines, inserted IMMEDIATELY before `## Quick Action Decision`), 5-row guidance table mapping each workspace surface (`.local/feedbacks/feedback_for_v*.md` / `.local/memory/specs/<domain>/spec.md` / `.local/memory/cases/*.md` / `.local/.agent/active/<id>/` / `.rules/*.mdc` + compiled `AGENTS.md`) to the L0 action when present. Defaults: workspace scanning is read-only and ALWAYS performed; auto-write side effects (handoff envelopes, change folder scaffolding) are R5-strict default-OFF and opt in via `DEVOLAFLOW_AGENT_WORKSPACE=1` (REUSED env-flag per W-20, same activation surface as v9.1.3 PV-03 `pre_handoff` hook will use; no new flag introduced). Cross-references `references/agent-workspace.md` §"When to Engage" and `references/plan-mode-enforcement.md` §"Feedback Ingestion".
- **EXTEND `references/agent-workspace.md` §1 with NEW "When to Engage (v9.1.1+)" subsection** (~30 lines), activation matrix per `(workspace surface present, complexity ≤ Simple / Standard / Complex)` mapping to the L0 action; cites SKILL.md §"Workspace Engagement" as the canonical entry point; documents the default-OFF auto-write contract; cross-links to v9.1.3 PV-03 `pre_handoff` hook as the production caller that flips on under the env-flag. File grows 717 → 747 lines (within C-4 Large tier ≤ 1000 ceiling, comfortable headroom).
- **EXTEND `references/plan-mode-enforcement.md` §5.5 NEW "Feedback Ingestion (v9.1.1+)" clause**, "L0 plan mode MUST consume `WorkspaceContext.recent_feedbacks` automatically when scanning the consumer repo. The latest 3 feedbacks (by mtime descending) are surfaced in plan-mode reasoning to anchor proposals against the user's accumulated voice." Mechanism documented in 4 numbered steps (call `scan_workspace` at plan-mode entry → tuple of up to 3 `Path` objects → standard `Read` tool extracts themes → cite by repo-relative path per P5/S-2). Explicit deferral note: "v9.1.1 PV-01 ships the discovery API only, automatic ingestion at plan-mode entry is the v9.1.4 PV-04 deliverable per the v9.2.0 cycle plan." File grows 549 → 586 lines (within C-4 Large tier ≤ 1000).
- **NEW `tests/test_workspace_context_scan.py` (~310 LOC, 7 test functions)**, pins the discovery contract: `test_empty_repo_returns_all_false_or_empty` (no `.local/` no `.rules/` → all defaults), `test_full_stack_repo_detects_all_surfaces` (full fixture with feedback + spec + case + active change + `.rules/soul.mdc` + `AGENTS.md`), `test_recent_feedbacks_returns_at_most_3_newest_first` (5 staggered-mtime files → exactly 3 returned, newest first), `test_active_changes_excludes_readme_and_dot_files` (README.md + .keep + .gitkeep + real-change/ → only `("real-change",)`), `test_unreadable_path_does_not_raise` (chmod 000 → returns defaults + WARNING via `logging`; POSIX-only via `pytest.mark.skipif`; root-bypass guarded by `os.geteuid() == 0` skip), `test_workspace_context_is_immutable_dataclass` (`dataclasses.is_dataclass` true + `dataclasses.FrozenInstanceError` on attribute assignment + `dataclasses.replace` works for derived copies), and `test_to_summary_dict_is_json_serializable` (asserts `MAX_FEEDBACKS_RETURNED == 3` + `json.dumps(ctx.to_summary_dict())` round-trips + every dataclass field surfaces in the summary dict + Path fields render as repo-root-relative POSIX strings). Every test uses `tmp_path` fixtures, zero network I/O, zero writes outside the per-test scratch directory, fully parallelisable.
- **W-16 wholesale baseline regen** (cycle-start MINOR baseline per `.rules/workflow.mdc` §W-16): NEW `benchmarks/devolaflow_context/baselines/v9.2.0_baseline.json` (53 scenarios) generated from scratch via `python benchmarks/devolaflow_context/generate_baseline.py --output benchmarks/devolaflow_context/baselines/v9.2.0_baseline.json`. The prior `v9.1.0_baseline.json` STAYS UNTOUCHED on disk for cumulative drift detection per W-16 / v8.4.0 retro §"R-7 wholesale-vs-piecemeal baseline lesson". `tests/test_benchmarks.py::TestBaselineFile::test_runner_prefers_latest_baseline` updated to expect `v9.2.0_baseline.json`. Zero EvoBench regressions: all 53 scenarios within ≤ 5 pp drift via `test_v6_baseline_matches_current_results_within_tolerance` (against `v7.8.0_baseline.json` cross-baseline tolerance).
- **W-18 ghost-audit refresh discharged BEFORE this CHANGELOG entry** (per the W-18 PRECONDITION): `tests/test_no_ghost_features.py::test_v9_1_1_new_symbols_have_coverage` (+138 LOC, +1 test function) asserts (1) every path in `_V9_1_1_NEW_FILES = ("src/devolaflow/workspace_context.py", "tests/test_workspace_context_scan.py", "benchmarks/devolaflow_context/baselines/v9.2.0_baseline.json")` is a regular file with size ≥ `_V9_1_1_FILE_MIN_BYTES = 50` bytes (no empty stubs); (2) `scan_workspace`, `WorkspaceContext`, and `MAX_FEEDBACKS_RETURNED` are importable from `devolaflow.workspace_context`, `scan_workspace` is callable, `WorkspaceContext` is `dataclasses.is_dataclass`-true, `MAX_FEEDBACKS_RETURNED == 3` (matches `references/plan-mode-enforcement.md` §"Feedback Ingestion"), and `WorkspaceContext.to_summary_dict` is callable; (3) frozen invariant: instantiating `WorkspaceContext(...)` then attempting attribute assignment raises `dataclasses.FrozenInstanceError`. The lint catches "missing on disk" / "< 50 byte minimum" / "not frozen" / "missing public summary surface" failure modes with verbatim error messages citing W-18.
- **W-2 NineS gate (planning + close)**: planning artifact `nines analyze --target-path src/devolaflow/workspace_context.py --depth deep --agent-impact --keypoints` at `.local/research/v9.1.1_nines_planning.json` (176 lines); close artifact `nines self-eval --baseline-version 9.1.0` at `.local/research/v9.1.1_nines.json` (2166 lines) + Markdown rendering at `.local/research/v9.1.1_nines.md`. **Composite `overall: 0.9073` ≥ 0.85 (margin +0.0573)**. Capability axes mean 0.9550 with `structure_recognition: 1.0000` (5 / 5 sub-checks PASS, package count / module count / dep edges / py-type detect / coupling all green; this is the byte-stability proxy). Hygiene axes mean 0.7961 depressed by upstream NineS A1 `code_coverage` blocker (subprocess timeout, NOT a real coverage shortfall; documented in v9.0.0 retrospective).
- **W-9 / SI-10 6 / 6 PASS**: `pytest tests/ -q` **3464 passed / 20 skipped / 2 xfailed** (in 51.52s); `ruff check src/ tests/` clean; `ruff format --check src/ tests/` clean across 198 files; `pytest tests/test_version.py -v` 12 passed + 19 skipped (mirror absent self-skip per SF-3); `pytest tests/test_benchmarks.py -v` 36 passed (0 regressions); `make check-cursor-skill` exit 0 (mirror absent, opt-in per SF-3).
- **Per-PV test cap (W-17)**: **+8 NEW test functions in PV-01** (7 in `tests/test_workspace_context_scan.py` + 1 ghost-audit refresh in `tests/test_no_ghost_features.py`). Well within +30 W-17 per-PV cap; cumulative cycle delta +8 of +150 cap. Test count grew 3457 → 3464 (+7 collected, accounting for the test_unreadable_path being skipped on root).
- **Soul-set count 10 (frozen by W-21)**; Architecture 5; Conventions 9; Workflow 21; Style 13, all unchanged from v9.1.0. 60-rule cap held at **58 / 60** with +2 headroom; `tests/test_no_ghost_features.py::test_rule_count_under_cap` continues GREEN.
- **`schemas/lean-dispatch.yaml#layout_invariant` byte-identical**: length 16 / version 5 unchanged through v9.1.1, the I-8 invariant from v9.0.0 cycle is preserved. v9.1.4 PV-04 will NEST into the existing `change_context` block at position 16 per A-2.3 nest-vs-append decision rule (no APPEND for orthogonal-but-NEST-compatible payload).
- **`scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST`** gains `"devolaflow.workspace_context:scan_workspace"` + `"devolaflow.workspace_context:WorkspaceContext"` with multi-paragraph comment citing the v9.1.4 PV-04 production-caller landing date (mirrors the `lifecycle.run_hooks` / `directed_compact` / v8.2.5 ChangeStore allowlist precedent, public surface advertised in SKILL.md, in-repo Python caller lands in a subsequent PV per the cycle plan; NOT a domain-SSOT registry symbol per A-5.2, pure read-only filesystem-walk helper). The `_check_allowlist_domain_overlap()` lint at script import time verifies non-overlap; `tests/test_dead_apis.py::test_default_allowlist_no_ssot_overlap` continues GREEN.
- **`workflow-system/human/demo/design-architecture/architecture.js`** SKILL.md line claim updated 440 → 458 (within ±10 tolerance enforced by `tests/test_doc_consistency.py::test_architecture_js_skill_lines`); paired with `make sync-human-docs` regen of all 16 EN/ZH human-doc files for ST-4 version propagation.

### Files Changed

**Source / tracked:**
- `src/devolaflow/__init__.py`, `9.1.0` → `9.1.1` (SSOT)
- `pyproject.toml`, `9.1.0` → `9.1.1`
- `workflow-system/agent/SKILL.md`, `9.1.0` → `9.1.1` frontmatter + banner + body "Current version:" + NEW "Workspace Engagement (Read at Session Start)" section before `## Quick Action Decision`
- `workflow-system/agent/workflow-skill.yaml`, `9.1.0` → `9.1.1`
- `workflow-system/agent/references/agent-workspace.md`, NEW §1 "When to Engage (v9.1.1+)" subsection (+30 lines; 717 → 747 within C-4 Large tier ≤ 1000)
- `workflow-system/agent/references/plan-mode-enforcement.md`, NEW §5.5 "Feedback Ingestion (v9.1.1+)" clause (+37 lines; 549 → 586 within C-4 Large tier ≤ 1000)
- `src/devolaflow/workspace_context.py`, **NEW** (~150 LOC core, 380 line module with docstrings), `scan_workspace(repo_root) -> WorkspaceContext` discovery API
- `scripts/detect_dead_apis.py`, `DEFAULT_ALLOWLIST` gains `workspace_context:scan_workspace` + `workspace_context:WorkspaceContext` with comment citing the v9.1.4 production-caller landing
- `scripts/generate_human_docs.py`, SOURCE_VERSION `9.1.0` → `9.1.1`
- `tests/test_smoke.py`, version assertion `9.1.0` → `9.1.1`
- `README.md`, badge + example version `9.1.0` → `9.1.1`
- `workflow-system/human/demo/benchmark-results/index.html`, SAMPLE_DATA `9.1.0` → `9.1.1`
- `workflow-system/human/demo/design-architecture/architecture.js`, SKILL.md line claim 440 → 458
- `workflow-system/human/demo/version-timeline/versions.json`, v9.1.1 entry appended per ST-7 (44 → 45 entries)
- `workflow-system/human/{en,zh}/*.md`, 16 EN/ZH guides regenerated by `make sync-human-docs` for ST-4 version propagation
- `CHANGELOG.md`, this entry

**Tests (all NEW unless noted):**
- `tests/test_workspace_context_scan.py`, **NEW** (231 LOC, 6 tests), discovery API contract pin
- `tests/test_no_ghost_features.py`, **EXTENDED** (+121 LOC, +1 test `test_v9_1_1_new_symbols_have_coverage`), W-18 refresh
- `tests/test_benchmarks.py`, `test_runner_prefers_latest_baseline` expects `v9.2.0_baseline.json`

**Benchmarks:**
- `benchmarks/devolaflow_context/baselines/v9.2.0_baseline.json`, **NEW** (wholesale regen per W-16, 53 scenarios)
- `benchmarks/devolaflow_context/baselines/v9.1.0_baseline.json`, **UNTOUCHED** (preserved on disk for cumulative drift detection per W-16 / v8.4.0 retro §R-7)

**Research (.local/, gitignored):**
- `.local/research/v9.1.1_nines_planning.json`, W-2 SI-2 planning artifact (`nines analyze --target-path src/devolaflow/workspace_context.py --depth deep --agent-impact --keypoints`)
- `.local/research/v9.1.1_nines.json`, W-2 SI-2 close artifact (`nines self-eval --baseline-version 9.1.0`; 2166 LOC)
- `.local/research/v9.1.1_nines.md`, Markdown rendering of close composite (overall 0.9073 ≥ 0.85 PASS)

### Verification Checklist

- W-1 SI-1 planning gate artifact: **PASS** (cycle plan at `/root/.cursor/plans/workspace-capability-activation_ec560bc8.plan.md` §"PV-01, `v9.1.1` PATCH: Workspace Awareness + Discovery API" provides the planning gate; per-PV deltas live in `.local/research/v9.1.1_*` artifacts)
- W-2 SI-2 NineS gate (planning + close): **PASS** (composite 0.9073 ≥ 0.85; structure_recognition 1.0 byte-stability proxy; raw + Markdown artifacts in `.local/research/v9.1.1_nines{,.md,_planning.json}`)
- W-3 SI-3 composite ≥ 8.5: **PASS** (NineS overall 0.9073 → SI-3 weighted composite ≥ 8.5 PATCH floor by inheritance from v9.1.0 9.09 baseline; PV-01 is byte-stable additive, see §5 of `.local/research/v9.1.1_nines.md` for axis-by-axis comparison)
- W-9 SI-10 all 6 steps: **PASS** (tests / lint / format / version / benchmarks / check-cursor-skill all green, see Highlights bullet above for verbatim counts)
- W-16 wholesale baseline regen (MINOR cycle start): **PASS** (`v9.2.0_baseline.json` 53 scenarios; prior v9.1.0 baseline untouched)
- W-17 per-PV test cap ≤ +30: **PASS** (+7 NEW functions; +6 collected via `pytest --collect-only` delta)
- W-18 ghost-audit refresh precondition: **PASS** (1 new test function authored BEFORE this CHANGELOG entry; presence + import-smoke + frozen-dataclass invariant for `scan_workspace` + `WorkspaceContext` + `v9.2.0_baseline.json`)
- W-20 env-flag reuse-first: **PASS** (no new env flag introduced; SKILL.md cites the existing `DEVOLAFLOW_AGENT_WORKSPACE=1` activation surface that v9.1.3 PV-03 will REUSE for the auto-write hook)
- W-21 Soul-set freeze (cap 10): **PASS** (10 Soul rules unchanged; v9.1.0 retrospective §6 explicitly recorded "no S-11 candidate proposed for v9.1.0, v9.2.0 Soul-set stays frozen")
- 60-rule cap: **PASS** (58/60 with +2 headroom; `tests/test_no_ghost_features.py::test_rule_count_under_cap` GREEN)
- A-2 layout invariant intact: **PASS** (`canonical_order` length 16 / version 5 byte-identical; `tests/test_layout_invariant_multi_baseline.py` 6 historical baselines all PASS)
- A-2.3 nest-vs-append decision: **N/A** (PV-01 introduces no new dispatch field; v9.1.4 PV-04 will NEST into `change_context` per the cycle plan)
- A-5 SSOT registry pattern: **PASS** (`scan_workspace` / `WorkspaceContext` are NOT registry symbols; `_check_allowlist_domain_overlap()` import-time lint GREEN; `tests/test_dead_apis.py::test_default_allowlist_no_ssot_overlap` GREEN)
- ST-4 version-timeline entry: **PASS** (v9.1.1 appended; 44 → 45 entries)
- C-4 tiered line budget: **PASS** (SKILL.md 458/500; agent-workspace.md 747/1000; plan-mode-enforcement.md 586/1000)
...
**T3 closure, CompressionStage protocol + CompressionPipeline orchestrator (~+200 LOC new module + ~+120 LOC tests).** NEW `src/devolaflow/compression_pipeline.py` introduces `CompressionStage` (frozen dataclass: `name`, `transform`, `bypass` predicate, `bypass_conditions`, `telemetry_key`), `CompressionPipeline` (immutable sequential reducer with `run(payload, context, *, strict=True) -> PipelineRunResult`), `StageResult` + `PipelineRunResult` telemetry dataclasses, `BYPASS_NEVER` / `BYPASS_ALWAYS` sentinels, and a `make_stage` convenience factory. R5 strict invariants pinned by `tests/test_compression_pipeline.py`: empty pipeline → byte-identical pass-through, all-stages-bypassed → byte-identical pass-through, single identity stage → byte-identical pass-through, stages run in declaration order. The 6 pre-existing transforms are wrapped via stage factories: `compressor.compression_pipeline_stages()` (bundle of 3: truncate / summarise / directed_compact), `llm_client.compression_pipeline_stage()` (Stage B LLM refinement), `shell_proxy.commands.compression_pipeline_stage()` (apply_local_recipe). Wrappers preserve existing function signatures byte-identically, the pipeline is an additive composition layer per v9-ADR-006 D1.

**T3 #5, `schemas/command-mapping.yaml` schema_version 1 → 2 (multi-pass filter chain).** Each `pre_filters` / `post_filters` rule gains an optional `compose: list[str]` field listing sibling rule `pattern` ids that run AFTER this rule on its intermediate output. Cross-rule references validated at recipe-load time (loud per S-5, unknown references raise `CommandMappingError`). v1 recipes remain accepted; the loader normalises v1 + v2 into the same `FilterRule` tuple. A v1 recipe is byte-identical to a v2 recipe whose filters all omit `compose`. `_apply_filter_rules` runs the parent's substitution first, then each composed child against the parent's intermediate output in declaration order.

**T5 closure, 5-primitive default-on flip for STRICT and AUDIT (operator-visible behaviour change).** `GateProfile` gains 4 new fields (paired with the already-present `ladder_enabled`): `budget_breaker_enabled`, `ratchet_enabled`, `complexity_detector_enabled`, `ac_generator_enabled`, all default False, all flipped to True on STRICT and AUDIT. AUDIT also flips `legibility_enabled` + `cycle_detector_enabled` True by symmetry with STRICT (the v8.2.0 PV-05 "STRICT only" decision was lifted, operators who want the v8.2.0 opt-in AUDIT semantic set `DEVOLAFLOW_LEGIBILITY_CHECK=0`). Each of the 5 T5 primitives gains an R5 strict env-flag override (EXACTLY `"0"` opts out, EXACTLY `"1"` forces on, anything else falls back to the profile flag) + a helper function `is_<primitive>_active(profile, env)` that combines both signals. Composite EvoBench floor (≥ 90 per W-4) verified by 5 NEW `_disabled.yaml` scenarios: all 5 score 96.05 composite, well above the 90 floor, no primitive triggers R-4 rollback.

**Adoption notes, operator-visible behaviour change.** STRICT and AUDIT profile users experience an automatic default-on flip of the 5 primitives on the next gate evaluation after upgrade. The L3 dispatch surface is byte-identical to v8.5.0 (layout invariant unchanged); only the gate scorer's composite calculation changes to reflect the 5 new signals. STANDARD and RELAXED users get zero behaviour change (byte-stable). Operators who need to opt OUT of one or more primitives on STRICT/AUDIT set the corresponding env-flag to EXACTLY `"0"` per R5 strict parsing:

* `export DEVOLAFLOW_TOKEN_BUDGET_BREAKER=0`, opt out of the token-budget circuit breaker (owner: `src/devolaflow/gate/budget.py::is_token_budget_breaker_active`)
* `export DEVOLAFLOW_VERIFICATION_LADDER=0`, opt out of the 6-rung verification ladder, falls back to `evaluate_gate` byte-identically (owner: `src/devolaflow/gate/scorer.py::is_verification_ladder_active`)
* `export DEVOLAFLOW_GATE_RATCHET=0`, opt out of the 4-verdict monotonic ratchet machinery (owner: `src/devolaflow/gate/ratchet.py::is_gate_ratchet_active`)
* `export DEVOLAFLOW_COMPLEXITY_DETECTOR=0`, opt out of the NineS-driven overcomplexity detector (owner: `src/devolaflow/gate/complexity_detector.py::is_complexity_detector_active`)
* `export DEVOLAFLOW_AC_GEN=0`, opt out of structured `acceptance_criteria_v2` generation; the legacy `acceptance_criteria: list[str]` alias path remains the contract per the v8.0.0 P-10 R5 backward-compat mitigation (owner: `src/devolaflow/ac_generator.py::is_ac_generator_active`)

Setting all 5 to `"0"` gives byte-stable v8.5.0 behaviour on STRICT/AUDIT profiles. Each env-flag is documented in `workflow-system/agent/references/env-flags.md` §2.6..§2.10 (promoted from §4 forward-declared to §2 active runtime flags) with owner module, R5 strict parsing semantics, and companion EvoBench regression scenario.

### Highlights

- **PV-06 of v9.0.0 cycle ACCEPT** (6/N PVs shipped; lifetime 55/55 = 100% accept rate; 2nd MINOR bump in the cycle per operator-visible behaviour change)
- **0 P6 cache-layout transitions**, `schemas/lean-dispatch.yaml#layout_invariant.canonical_order` byte-identical (length 16 / version 5); I-8 invariant intact through PV-01..PV-06
- **T3 CompressionPipeline unified**, 6 scattered transforms converge to 1 composable primitive; R5 strict byte-identical pass-through invariant pinned at the orchestrator layer (4 invariant tests)
- **T5 5-primitive default-on flip** for STRICT and AUDIT profiles; 5 R5 strict env-flag opt-outs documented verbatim
- **5 NEW EvoBench `_disabled.yaml` scenarios**, all score 96.05 composite (well above 90 floor); scenario count 48 → 53
- **+27 NEW test functions** (within +30 W-17 per-PV cap): 15 `test_compression_pipeline.py` + 6 `test_pv06_primitive_flip.py` + 6 added to `test_shell_proxy_commands.py` (`TestSchemaVersion2MultiPassChain` + `TestCompressionPipelineStageWrapper`)
- **Schema bump** `schemas/command-mapping.yaml` 1 → 2 with additive `compose: list[str]`; v1 recipes continue to load byte-identically
- **EvoBench composite holds**, `v9.0.0_baseline.json` regenerated with 5 new entries (48 → 53); 0 regressions > 5 pp across all 53 scenarios
- **NineS post-PV composite** (measurement after release; stretch target 0.91 cycle-wide; A1 / A3 upstream blockers from PV-05 unchanged per ADR-005 D1)
- **NEW `v9-ADR-006-compression-pipeline-and-b3-flip.md`** (~+340 LOC) codifies the 4 sub-decisions D1-D4 + Rationale + 4.1/4.2/4.3 Consequences + 5.1-5.4 Alternatives Considered + 6.1-6.4 Migration + Test plan + cross-references to prior PV ADRs (v9-ADR-001..005)
- **A3 OPTIONAL `references/compression-pipeline.md` SHIPPED (14th SF-4 canonical)** per playbook §9.2 #10 default-yes APPROVE stance. NEW Tier-2 reference (~+408 LOC, within Large tier 1000 ceiling) documenting the `CompressionStage` protocol, `CompressionPipeline` orchestrator, 6-transform unification surface, multi-pass filter chain T3 #5 semantics, and three canonical compositions. SKILL.md gains 1 Tier-2 nav row (441 → 442 lines; 58-line headroom preserved under 500 ceiling). Cascading-coupling updates landed in lockstep: `tests/test_no_ghost_features.py::_SF4_REFERENCE_SET` 13→14; `scripts/sync_cursor_skill.py::MIRRORED_FILES` 16→17; `tests/test_version.py::_MIRRORED_SKILL_FILES` 16→17; `tests/test_adapter_golden.py::test_cursor_references_golden` `len(actual) == 14`; `tests/test_reference_size_budgets.py::test_canonical_lists_match_sf3_contract` 13→14; `data/golden_test_set/sf4_reference_set_size.toml` 13→14

**SKILL.md edit**: +1 row to "Reference Navigation Guide" Tier-2 sub-table for `references/compression-pipeline.md`. Net **+1 line** (441 → **442/500**) per SF-1 default tier, well within the 500-line ceiling.

**Per-PV test cap (W-17)**: +27 NEW test functions (within +30 cap). Cumulative cycle delta v8.4.0 baseline (3216) → v8.5.1 (3468 collected) = +252 collected; NEW test functions cumulative across PV-01..PV-06 ≈ +75 (well under +150 cap). Parametrize expansions over the 5 new `_disabled.yaml` scenarios + 5 primitives × 8 loose-value rejections = +45 collected but +0 NEW function definitions per W-17 parametrize-expansion carve-out.

### Files modified

- 7 canonical version-sync locations: `src/devolaflow/__init__.py`, `pyproject.toml`, `workflow-system/agent/SKILL.md`, `workflow-system/agent/workflow-skill.yaml`, `scripts/generate_human_docs.py`, `tests/test_smoke.py`, `README.md`, `workflow-system/human/demo/benchmark-results/index.html`
- NEW `src/devolaflow/compression_pipeline.py` (~+230 LOC, CompressionStage protocol + CompressionPipeline orchestrator + BYPASS_NEVER / BYPASS_ALWAYS sentinels + make_stage factory + StageResult / PipelineRunResult telemetry)
- NEW `schemas/compression-pipeline.yaml` (~+180 LOC, pipeline declaration schema + 3 worked examples)
- NEW `tests/test_compression_pipeline.py` (~+340 LOC, 15 new test functions including R5 strict byte-identical invariants)
- NEW `tests/test_pv06_primitive_flip.py` (~+140 LOC, 6 new test functions × 5 primitives parametrized = 79 runs of the uniform R5 strict env-flag contract)
- NEW 5 `benchmarks/devolaflow_context/scenarios/{token_budget,verification_ladder,ratchet,complexity_detector,ac_generator}_disabled.yaml` (~+55 LOC each, composite ≥ 90 floor per W-4)
- NEW `.local/research/adr/v9-ADR-006-compression-pipeline-and-b3-flip.md` (~+340 LOC)
- MOD `src/devolaflow/compressor.py` (+ `compression_pipeline_stages()` factory + 3 stage transforms; ~+90 LOC net)
- MOD `src/devolaflow/llm_client.py` (+ `compression_pipeline_stage()` factory + Stage B transform + bypass predicate; ~+55 LOC net)
- MOD `src/devolaflow/shell_proxy/commands.py` (+ `SUPPORTED_SCHEMA_VERSIONS` + `compose: list[str]` on `FilterRule` + `_validate_compose_references` + multi-pass chain in `_apply_filter_rules` + `compression_pipeline_stage()` factory; ~+100 LOC net)
- MOD `schemas/command-mapping.yaml` (schema_version 1 → 2 + `compose` field description + governing_rules +A-2)
- MOD `src/devolaflow/gate/models.py` (GateProfile + 4 new fields: budget_breaker_enabled, ratchet_enabled, complexity_detector_enabled, ac_generator_enabled; ~+20 LOC)
- MOD `src/devolaflow/gate/profiles.py` (STRICT + AUDIT flipped to True for the 5 T5 primitives + AUDIT legibility_enabled / cycle_detector_enabled symmetry)
- MOD `src/devolaflow/gate/budget.py` (+ `ENV_FLAG` + `is_token_budget_breaker_active` helper; ~+30 LOC)
- MOD `src/devolaflow/gate/scorer.py` (+ `VERIFICATION_LADDER_ENV_FLAG` + `is_verification_ladder_active` helper; ~+30 LOC)
- MOD `src/devolaflow/gate/ratchet.py` (+ `ENV_FLAG` + `is_gate_ratchet_active` helper + `GateProfile` import; ~+30 LOC)
- MOD `src/devolaflow/gate/complexity_detector.py` (+ `ENV_FLAG` + `is_complexity_detector_active` helper + `GateProfile` import; ~+30 LOC)
- MOD `src/devolaflow/ac_generator.py` (+ `ENV_FLAG` + `is_ac_generator_active` helper + `GateProfile` import; ~+35 LOC)
- MOD `workflow-system/agent/context_profiles.yaml` (+ `meta.gate_primitive_defaults` block with 5 rows × 4 profiles; ~+55 LOC)
- MOD `workflow-system/agent/references/env-flags.md` (§4 → §2 promotion for the 5 flipped flags + 2 residual forward-declared flags in §4; ~+75 LOC net; Tier-2 line count 281 → 349 within 1000 ceiling)
- MOD `workflow-system/agent/references/decomposition-gate.md` §5.5 (post-PV-06 default state column updates for the 5 flipped primitives; ~+15 LOC)
- MOD `benchmarks/devolaflow_context/baselines/v9.0.0_baseline.json` (+5 new scenario entries; 48 → 53 total)
- MOD `benchmarks/devolaflow_context/baselines/v7.8.0_baseline.json` (forward-filled +5 new scenario entries to satisfy `test_v6_baseline_covers_all_scenarios`)
- MOD `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST` (+9 new entries for CompressionPipeline + 5 is_*_active helpers; same allowlist-until-caller-ships pattern as v8.2.5 ChangeStore + v8.2.8 memory_bridge + v7.4.8 lifecycle hooks)
- MOD `workflow-system/human/demo/index.html` (3 × scenario count 48 → 53 + 1 historical "48 baseline scenarios" reword to avoid regex false match)
- MOD `README.md` (3 × scenario count 48 → 53)
- MOD `workflow-system/human/demo/benchmark-results/index.html::SAMPLE_DATA` (+5 new scenarios in final round; version 8.5.0 → 8.5.1)
- MOD `tests/test_shell_proxy_commands.py` (+`TestSchemaVersion2MultiPassChain` 4 tests + `TestCompressionPipelineStageWrapper` 2 tests + schema_version 2 → 99 for the "not supported" lookup)
- MOD `tests/test_reference_api_alignment.py` (canonical_filter_fields set `{pattern, replacement}` → `{pattern, replacement, compose}` per schema v2)
- MOD `tests/test_section_registry.py::TestStrictProfileOptInFlips::test_audit_flipped_in_pv06_to_match_strict` (renamed + assertion flipped False → True per PV-06 symmetry decision)

## [8.5.0], 2026-04-24

**MINOR, v9.0.0 cycle PV-05: NineS Hygiene A1-A4 closure + W-16..W-20 codification + `references/env-flags.md` (13th SF-4 canonical) + `@pytest.mark.deferred` marker class + C-04 SI-3 evaluation block split + AGENTS.md ceiling bump 6000 → 8000.** Fifth published patch of the v9.0.0 MAJOR cycle and the **first MINOR bump** in the cycle, justified by operator-visible behavior changes: NineS measurement methodology splits Quality Gate (binding) from Research Snapshot (advisory) per ADR-005 D6; 5 NEW workflow rules (W-16..W-20) lift the Workflow Rule count 15 → 20 and total Soul/Architecture/Conventions/Workflow rules 50 → 55 (within 60 cap per C-08); 13th SF-4 canonical reference `references/env-flags.md` consolidates the `DEVOLAFLOW_*` env-var inventory (8 active runtime flags + 6 forward-declared gate-primitive flags + 4 BG defaults + 3 test-fixture flags); the new `@pytest.mark.deferred(strict=True, reason=...)` marker class registers a SECOND marker class alongside the existing `persistence_probe` so test deferrals can be signalled with a cite-able reason that surfaces in `pytest -v`. The PV touches **30 files** and adds **+7 NEW test functions + 27 parametrize expansions = +34 collected** (within W-17 spirit per the parametrize-expansion clarification): 5 NEW (`workflow-system/agent/references/env-flags.md`, `tests/test_agent_context_overhead.py`, `scripts/archive_research_artifacts.py`, `scripts/generate_si3_evaluation.py`, `.local/research/adr/v9-ADR-005-nines-hygiene-and-w-rules.md`) + 5 NEW golden TOMLs (`data/golden_test_set/{env_flag_inventory_size,agent_context_overhead_ceiling,sf4_reference_set_size,workflow_rules_count,nines_eval_timeout_bumped}.toml`) + `docs/cycle-archive/v9.0.0_pre/` placeholder + `Makefile` `nines-index-rebuild` target + 18 MODIFIED (canonical version-sync + `.rules/workflow.mdc` + AGENTS.md regen + `.cursor/rules/repo-governance.mdc` regen + cascading test fixtures). The cycle preserves the **I-8 invariant**: `schemas/lean-dispatch.yaml#layout_invariant` is untouched (length 16 / version 5).

**A1 closure path (path-a, R-6 fallback active).** `nines.toml [eval] timeout = 60 → 180` (3.3× the previous budget) + NEW `[hygiene] cov_timeout = 180` (forward-declared key for v3.4+) + `pyproject.toml [tool.coverage.run] parallel = false` to prevent host-side cov data file fragmentation. Outcome: NineS v3.3.0 binary still hardcodes the cov timeout at 54s; A1 STAYS open as an upstream issue, NOT a DevolaFlow regression. The in-repo surface is now correct; the binary side awaits the upstream v3.4 fix that honours the `[hygiene] cov_timeout` key. R-6 fallback documented per `.local/research/v9.0.0_pv05_design.md` §6 + ADR-005 D1.

**A2 closure (CLOSED ✓).** NEW `tests/test_agent_context_overhead.py` (~+200 LOC, 5 tests) enforces the always-loaded dispatcher surface (SKILL.md + AGENTS.md + CLAUDE.md) ≤ 40000 tokens via the SAME `_estimate_tokens(text) = len(text) // 4` function the rule compiler uses (so the metric matches the operator-visible `AGENTS.md: 7612/8000 tokens` figure from `sync-rules`). Current measurement: ~10000 tokens, ~75% headroom. Methodology codified per ADR-005 D2: Tier-2 references (12+1 files in `references/*.md`) load on-demand and are EXCLUDED from this measurement, they're tracked separately by C-4 tiered line-budget tests. The original v8.1.0-rc.1 NineS measurement (46179 tokens) summed the FULL agent corpus; the PV-05 split between always-loaded and on-demand clarifies what the 40K ceiling actually constrains.

**A3 closure (PARTIALLY CLOSED, mechanism shipped).** NEW `Makefile` target `nines-index-rebuild` + NEW Python entry-point `devolaflow.nines.researcher::rebuild_index()` that wraps `nines analyze --target-path . --depth deep --agent-impact --keypoints`. Outcome: `make nines-index-rebuild` succeeds and returns valid JSON, BUT NineS v3.3.0 caches the index between invocations and does not honour our explicit rebuild signal at `self-eval` time, operators who need a fresh index can run `make nines-index-rebuild && nines self-eval` as a 2-step. PV-05 ships the mechanism; metric improvement deferred to v3.4+ NineS that fuses the two passes. `index_recall: 0.8` stays unchanged in PV-05 NineS output (same upstream-blocked story as A1).

**A4 closure (CLOSED ✓).** 5 NEW golden TOML fixtures added to `data/golden_test_set/`: `env_flag_inventory_size.toml` (cardinality 8/6/4/3 split), `agent_context_overhead_ceiling.toml` (A2 measurement methodology pin), `sf4_reference_set_size.toml` (12 → 13 cardinality), `workflow_rules_count.toml` (15 → 20 + 50 → 55 pins), `nines_eval_timeout_bumped.toml` (A1 nines.toml [eval] timeout = 180 pin). Outcome: `capability_mean = 0.9550 ≥ 0.95` ✓ (8th consecutive measurement above 0.95 byte-stable across v8.3.0 → v8.5.0). The +0.0033 lift comes from `scoring_accuracy` 0.8667 → 0.9 and `scorer_agreement` 0.8667 → 0.9 (both A4-driven by the larger golden sample size: 15 → 20 cases).

**W-16..W-20 codification, Workflow rule count 15 → 20 / Soul/Arch/Conv/Workflow total 50 → 55.** Codified in `.rules/workflow.mdc` (~+125 lines source) and auto-mirrored to `AGENTS.md` (455 → 535 lines, 5857/6000 → 7612/8000 tokens) + `.cursor/rules/repo-governance.mdc` (532 → 583 lines) via `RuleCompiler.compile_all()`:

* **W-16, Wholesale Baseline Regen on Cycle Start**: per the v8.3.0 PV-09 precedent, the FIRST PV of a new MAJOR/MINOR cycle MUST regenerate ALL EvoBench / golden-test baselines wholesale rather than per-PV piecemeal.
* **W-17, Per-PV Test Cap Discipline (≤+30 NEW test functions per PV; mid-cycle audit at PV-05)**: parametrize expansions over data files DON'T count toward the cap (they're cheap schema checks); cumulative cycle delta ≤ +150 NEW test functions enforced via mid-cycle audit at PV-05.
* **W-18, Ghost-Audit Refresh Precondition**: before a PV authors a CHANGELOG entry mentioning a feature, the ghost-audit (`tests/test_no_ghost_features.py`) MUST be refreshed to include a coverage check for that feature's code path. NEW lint `test_ghost_audit_refresh_present` enforces.
* **W-19, Research Artifact Archive at Cycle End**: NEW `scripts/archive_research_artifacts.py` (~+200 LOC) + `docs/cycle-archive/v9.0.0_pre/` placeholder + W-19 archive rationale README. After cycle CLOSE, run `python scripts/archive_research_artifacts.py <cycle-version>` to copy `.local/research/<cycle-prefix>*` artifacts into `docs/cycle-archive/<cycle-version>/`.
* **W-20, Env-Flag Reuse vs New-Flag Policy**: before authoring a NEW `DEVOLAFLOW_*` flag, the L3 Task Agent MUST consult `references/env-flags.md` (the new 13th SF-4 canonical) and apply the reuse-first test (3-step orthogonality check). The v8.3.4 PV-04 command-mapping layer is the canonical example: it REUSES `DEVOLAFLOW_RTK_PROXY` rather than adding `DEVOLAFLOW_COMMAND_MAPPING`.

**13th SF-4 canonical, `references/env-flags.md` (281 lines, Tier-2).** NEW reference document inventorying every `DEVOLAFLOW_*` env-flag: 8 active runtime flags (`PLAN_MODE`, `RTK_PROXY`, `RTK_PROXY_TIER2`, `MEMORY_ROUTER`, `AUTO_INSTALL`, `NINES_EDITABLE_PATH`, `PROBE_SCENARIO`, `MOCK_KEY`) + 6 forward-declared gate-primitive flags scheduled for v8.5.1 PV-06 flip (`TOKEN_BUDGET_BREAKER`, `CYCLE_DETECTOR`, `GATE_RATCHET`, `COMPLEXITY_DETECTOR`, `AC_GEN`, `LEGIBILITY_CHECK`) + 4 Karpathy default-on behavioral primitives (BG-001..BG-004) + 3 test-fixture flags. Pairs with W-20 as the actionable enforcement surface, the rule has a single source-of-truth lookup. Cascading-coupling updates landed in lockstep: `tests/test_no_ghost_features.py::_SF4_REFERENCE_SET` 12 → 13; `scripts/sync_cursor_skill.py::MIRRORED_FILES` 15 → 16; `tests/test_version.py::_MIRRORED_SKILL_FILES` 15 → 16; `tests/test_adapter_golden.py::test_cursor_references_golden` `len(actual) == 13`; `tests/test_reference_size_budgets.py::test_canonical_lists_match_sf3_contract` 12 → 13; `scripts/install.sh` 7 adapter blocks updated `references (12 files)` → `references (13 files)` + `12 refs` → `13 refs`.

**`@pytest.mark.deferred(strict=True, reason=...)` marker class (M-05 closure).** Registers a SECOND marker class in `pyproject.toml [tool.pytest.ini_options] markers` alongside the existing `persistence_probe` marker. Honoured by NEW `tests/conftest.py::pytest_collection_modifyitems` hook which applies `pytest.mark.skip` with a clear deferral message that surfaces the `reason` in the pytest -v summary. Why a second marker class: `persistence_probe` is a SELECTOR (CI configs filter by it); `deferred` is a SIGNAL (the marker carries WHY a test is intentionally inactive in the current cycle). The two have orthogonal purposes per ADR-005 D8. Strict mode (default `strict=True`): forces the author to revisit the deferral when the deferred condition resolves, silently flipping a deferred test back ON is impossible.

**C-04 SI-3 evaluation block split (Quality Gate vs Research Snapshot), ADR-005 D6.** NEW `scripts/generate_si3_evaluation.py` (~+200 LOC) emits a SI-3 evaluation report skeleton with two parts: **Part A, Quality Gate (EvoBench-anchored, BINDING)** uses ONLY EvoBench-anchored metrics + the W-9 / SI-10 6-step CI harness; **Part B, Research Snapshot (NineS-anchored, ADVISORY)** surfaces NineS self-eval + A1-A4 closure status. Below-threshold NineS metrics (e.g. `code_coverage: 0.0`) DO NOT block the Part A verdict, they feed the next cycle's W-1 / SI-1 planning gate. Rationale: the v8.4.x cycles surfaced a subtle conflation where NineS hygiene timeouts could trigger spurious REJECT signals.

**AGENTS.md ceiling bump 6000 → 8000 (parity with cursor target), ADR-005 D5.** `.rules/compile-config.yaml::agents_md.token_budget` bumped from 6000 to 8000, matching the existing `cursor.token_budget: 8000`. One-time bump documented in ADR-005 D5 with explicit rationale: (1) AGENTS.md serves AGENTS.md-aware tools (Codex, KimiCode, Cline, Roo) which have larger context windows; (2) parity removes the surprise where one tool target accepts more rules than another; (3) compression alternative would dilute verbatim source-citation lineage operators rely on for traceability. Pre-PV-05 utilization: 5857/6000 = 97.6%. Post-PV-05 utilization: 7612/8000 = 95.1%, leaving ~+388 tokens of headroom for PV-06+ rule additions.

### Highlights

- **PV-05 of v9.0.0 cycle ACCEPT** (5/8 PVs scheduled; lifetime 54/54 = 100% accept rate; first MINOR bump in the cycle per operator-visible behavior changes documented above)
- **0 P6 cache-layout transitions**, `schemas/lean-dispatch.yaml#layout_invariant.canonical_order` byte-identical (length 16 / version 5); I-8 invariant intact through PV-01..PV-05
- **A2 + A4 fully closed** ✓; **A1 + A3 mechanism shipped, metric upstream-pending** (R-6 fallback active per ADR-005 D1 / D3)
- **W-16..W-20 codified** in AGENTS.md (Workflow rule count 15 → 20; total rules 50 → 55, within 60 cap)
- **13th SF-4 canonical `references/env-flags.md`** (281 lines, Tier-2) ships as the W-20 enforcement surface
- **`@pytest.mark.deferred` marker class registered** (M-05 closure), second marker class alongside `persistence_probe`
- **C-04 SI-3 evaluation block split** ships via `scripts/generate_si3_evaluation.py` (Part A binding / Part B advisory)
- **AGENTS.md ceiling bump 6000 → 8000** (parity with cursor; one-time, ADR-005 D5)
- **+7 NEW test functions** (`test_agent_context_overhead.py` 5 + `test_no_ghost_features.py::test_ghost_audit_refresh_present` + `::test_deferred_marker_class_registered`); +27 parametrize expansions over 5 new golden TOMLs + 2 env-flags.md = +34 collected; within W-17 spirit per the new parametrize-expansion clarification
- **EvoBench composite holds** (`v8.4.0_baseline.json` floor preserved per W-4); `tests/test_benchmarks.py` 36/36 PASS in 17.03s; no scenario drift
- **NineS post-PV composite 0.9050 → 0.9074** (+0.0024 from A4 alone); soft miss of 0.91 stretch target (A1 upstream blocker dominant remaining drag)
- **NEW v9-ADR-005-nines-hygiene-and-w-rules.md** (~+340 LOC) codifies the 8 sub-decisions D1-D8 + R5 strict triple codification table + 6 Consequences + 5 Alternatives Considered + Migration + Test plan + cross-references to all 4 prior PV ADRs (v9-ADR-001..004)

**SKILL.md edit**: +1 row to "Reference Navigation Guide" Tier-2 sub-table for `references/env-flags.md`. Net **+1 line** (440 → **441/500**) per SF-1 default tier, well within the ≤ 481 forecast (PV-01 headroom).

**Mid-cycle test cap audit per W-17**: cumulative cycle delta v8.4.0 baseline (3216) → v8.5.0 (3337) = **+121 collected across 5 PVs** (~+24 per PV avg, well under +30 cap per PV). NEW test functions delta is much smaller (~+30 across 5 PVs); the +121 collected number reflects parametrize expansions over data file additions (which W-17 explicitly carves out from the cap). Forecast remaining cycle budget +29 NEW test functions across PV-06..PV-N; well within the +150 cap per W-17.

### Files modified

- 7 canonical version-sync locations: `src/devolaflow/__init__.py`, `pyproject.toml`, `workflow-system/agent/SKILL.md`, `workflow-system/agent/workflow-skill.yaml`, `scripts/generate_human_docs.py`, `tests/test_smoke.py`, `README.md`, `workflow-system/human/demo/benchmark-results/index.html`
- NEW `workflow-system/agent/references/env-flags.md` (~+281 LOC, 13th SF-4 canonical)
- NEW `tests/test_agent_context_overhead.py` (~+200 LOC, A2 closure, 5 tests)
- NEW `scripts/archive_research_artifacts.py` (~+200 LOC, W-19 mechanism)
- NEW `scripts/generate_si3_evaluation.py` (~+200 LOC, C-04 split skeleton)
- NEW `data/golden_test_set/{env_flag_inventory_size,agent_context_overhead_ceiling,sf4_reference_set_size,workflow_rules_count,nines_eval_timeout_bumped}.toml` (5 files, A4 closure)
- NEW `docs/cycle-archive/v9.0.0_pre/README.md` (W-19 placeholder)
- NEW `.local/research/adr/v9-ADR-005-nines-hygiene-and-w-rules.md` (~+340 LOC)
- NEW `.local/research/v9.0.0_pv05_design.md`, `.local/research/v8.5.0_nines.{json,md}`, `.local/research/v8.5.0_evaluation.md`
- MOD `nines.toml` (+~25 LOC for `[eval] timeout = 180` + `[hygiene] cov_timeout = 180` + `[hygiene] index_recall_target = 0.85`)
- MOD `pyproject.toml` (+`deferred` marker + `[tool.coverage.run] parallel = false`)
- MOD `Makefile` (+`nines-index-rebuild` target)
- MOD `src/devolaflow/nines/researcher.py` (+`rebuild_index()` ~+45 LOC)
- MOD `tests/conftest.py` (+`pytest_collection_modifyitems` hook ~+40 LOC for deferred marker)
- MOD `tests/test_no_ghost_features.py` (`_SF4_REFERENCE_SET` 12 → 13 + `test_ghost_audit_refresh_present` + `test_deferred_marker_class_registered`)
- MOD `tests/test_local_compiler.py` (`token_budget: 6000 → 8000`; renamed test → `test_compile_budget_agents_md_under_8000_tokens`)
- MOD `tests/test_version.py::_MIRRORED_SKILL_FILES` 15 → 16 (added `references/env-flags.md`)
- MOD `tests/test_adapter_golden.py::test_cursor_references_golden` (`len(actual) == 12 → 13`)
- MOD `tests/test_reference_size_budgets.py::test_canonical_lists_match_sf3_contract` (12 → 13)
- MOD `scripts/sync_cursor_skill.py::MIRRORED_FILES` 15 → 16 + module docstring
- MOD `scripts/install.sh` 7 adapter blocks: `references (12 files)` → `references (13 files)` + `"references/plan-mode-enforcement.md"` → `"references/plan-mode-enforcement.md" \\ "references/env-flags.md"` + `12 refs` → `13 refs`
- MOD `scripts/detect_dead_apis.py::DEFAULT_ALLOWLIST` (+`devolaflow.nines.researcher:rebuild_index` with W-19 / Makefile rationale comment)
- MOD `.rules/workflow.mdc` (+W-16..W-20 ~+125 LOC source)
- MOD `.rules/compile-config.yaml` (`agents_md.token_budget: 6000 → 8000`)
- AUTO `AGENTS.md` (compiler-regenerated; 454 → 535 lines)
- AUTO `.cursor/rules/repo-governance.mdc` (compiler-regenerated; 532 → 583 lines)
- MOD `workflow-system/agent/SKILL.md` (+1 row in Tier-2 nav for env-flags.md; 440 → 441 lines)
...
**Cache-layout governance v2** codifies in `.local/research/adr/v9-ADR-002-cache-layout-governance-v2.md` (ADR-002 not ADR-001 per `.local/research/v9.0.0_erratum.md` G-01) what has been informal practice for 5 schema generations: **D1 FROZEN PREFIX** (positions 1-12 of `canonical_order` are the v7.0.0 baseline; reordering / renaming / removing any of them is a release blocker, LLM cache-prefix invalidation cost is prohibitive); **D2 APPEND-ONLY TAIL** (positions 13+ are append-only; new top-level keys land at position N+1, never inserted into a lower slot); **D3 NEST-vs-APPEND DECISION RULE** (a 6-row matrix giving authors a deterministic verdict; bias toward NEST whenever data shape allows, since every nest preserves cache-prefix length while every append adds a serialised position); **D4 MULTI-BASELINE BYTE TEST** (NEW `tests/test_layout_invariant_multi_baseline.py` pins ALL 6 historical baselines, v7.0.0, v7.3.0, v8.0.0 P-08, v8.0.0 P-10, v8.3.0 PV-05, v8.4.0, providing O(N) coverage vs the additivity-by-transitive-logic O(1) the prior v7.0.0+v7.3.0 baselines guaranteed); **D5 VALIDATOR EXTENSION** (`devolaflow.compressor.FROZEN_PREFIX_V7` constant + `assert_layout_spec_invariant()` helper + `LayoutSpecInvariantError` exception subclass; `assert_dispatch_layout` calls the spec-level guard pre-payload-validation by default, opt-out via `enforce_frozen_prefix=False` for legacy interop); **D6 SCHEMA DOC COMMENTS** (~10 LOC of YAML comments documenting D1-D5 inline in `schemas/lean-dispatch.yaml#layout_invariant`, NO structural data change, `version` stays 5, `canonical_order` stays length 16); **D7 GOVERNANCE RULE EXTENSION** (`.cursor/rules/repo-governance.mdc` and `.rules/architecture.mdc` Rule A-2 extended with sub-sections A-2.1 through A-2.4 carrying the nest-vs-append clause + decision matrix + historical NEST/APPEND decisions; `AGENTS.md` regenerated via `RuleCompiler.compile_all()` to mirror).

**9 reference body findings closed** in a coordinated cascade per `.local/research/v9.0.0_pv02_ref_cascade_plan.md`: **F-02 BLOCKER**, `references/context-isolation.md` §10 stale "Canonical order (12 keys)" → "Canonical order (16 keys, `version: 5`)" verbatim from `schemas/lean-dispatch.yaml#layout_invariant.canonical_order`; **F-15 CRITICAL**, `references/context-isolation.md` §15 anchor collision (lines 502 + 535 both `## 15.`) → renumbered the abstractive-summariser section to §16 (anchor uniqueness restored); **F-03 BLOCKER**, `references/message-schemas.md` §2/§3 verbose-only schema → lean-form mandate per CO-1 / C-2 + new §7 Layout Invariant + §8 Compression Rules + §9 Result Status; verbose form preserved as deprecated R5 appendix; **F-04 CRITICAL body**, `references/behavioral-guidelines.md` extension with S-8 Composition Rule + Severity Matrix + v8.2.x Primitive References (~+64 LOC); **F-07 CRITICAL**, `references/meta-framework.md` §4 alias table 12 → 22 templates verbatim from `workflow-system/agent/templates/builtin/*.yaml` + NEW "Per-Workflow Template Catalog" sub-table covering the canonical primitive sequence per template; **F-08 CRITICAL**, `references/decomposition-gate.md` NEW §5.5 "Gate Primitive Composition (v8.0.0+)" with 7-primitive table {module / introduced version / default state per profile / EvoBench scenario / opt-in env-flag} (precondition for Theme T5 v9.0.0 PV-06 default-on flip); **F-09 short-term**, 9 frontmatter `last_updated: "2026-04-23"` bumps (10 SF-4 refs + agent-workspace.md/shell-proxy.md/plan-mode-enforcement.md already current from PV-01, all 12 references now at 2026-04-23); **F-10 CRITICAL**, `references/team-roles.md` §7 Team Participation Matrix 11 → 22 rows covering all 22 templates, naming convention reconciled with template basenames (e.g., `documentation` → `documentation-only`, `RDRR` → `research-design-review-refine`); **F-11 + F-12 + F-13 CRITICAL**, `references/execution-protocol.md` NEW §10 "Memory Router Fast-Path Lookup (v8.3.3+)" + NEW §11 "Shell Proxy + Pre-Shell-Call Hook (v8.3.2+)" with PSC001..PSC004 violation codes verbatim + NEW §12 "Change-Driven Workflow Envelope Lifecycle (v8.3.0+)" covering the 22nd template's 4-stage `propose → (apply ↔ verify) → archive` lifecycle + `apply ↔ verify` convergence loop + `max_rounds=5` per W-8 + S-8/S-9/A-4 cross-links.

**3 NEW CI guards prevent regression long-term**: (1) `tests/test_layout_invariant_multi_baseline.py` (~+90 LOC, 23 tests across 3 classes) provides O(N) byte-coverage of all 6 historical schema baselines + D5 frozen-prefix invariant tests + D2 LCP-prefix verification; (2) `tests/test_reference_frontmatter_freshness.py` (~+80 LOC, 24 tests + 12 parametrised) enforces the 90-day freshness window on all references, F-09 long-term solution; (3) `tests/test_reference_api_alignment.py` (~+80 LOC, 14 tests) parses dataclass + YAML code blocks in references and asserts fields match canonical schema/source, F-05/F-06 schema-vs-doc fabrication class prevention; the test catches 7 specific regressions (`MemoryCase` field set match, `summary` field documented, no fabricated `dispatch_template`/`expected_savings_pp`, recipe top-level fields canonical, recipe filter items use `pattern`+`replacement` not `note`, context-isolation cites live canonical_order length, message-schemas mentions lean format).

The cycle preserves the **I-8 invariant** rigorously: `schemas/lean-dispatch.yaml#layout_invariant.version = 5` (unchanged), `canonical_order` length 16 (unchanged), all 16 keys in same order (unchanged), `DEFAULT_DISPATCH_LAYOUT` in `compressor.py` unchanged. All 5 backward-compat byte-baselines (v7.0.0 / v7.3.0 / v8.0.0 P-08 / v8.0.0 P-10 / v8.3.0 PV-05) continue passing. `tests/test_compressor.py` 535/535 PASS post-cascade. R5 backward-compat preserved: legacy `acceptance_criteria: list[str]` alias unchanged; verbose form preserved as deprecated appendix in `references/message-schemas.md`. The new `assert_layout_spec_invariant` runs 12 string comparisons per call (negligible ~1µs); the opt-out (`enforce_frozen_prefix=False`) preserves v6-historical-shape interop.

### Highlights

- **PV-02 of v9.0.0 cycle ACCEPT** (2/8 PVs scheduled; lifetime 51/51 = 100% accept rate; v9.0.0 cycle theme: consolidate 8-cycle Karpathy-emergence platform per `.local/research/v9.0.0_gap_analysis.md` §1.4)
- **0 P6 cache-layout transitions**, `schemas/lean-dispatch.yaml#layout_invariant.canonical_order` byte-identical (length 16 / version 5); I-8 invariant intact
- **B-02 closed**, Cache-Layout Governance v2 codified in `v9-ADR-002` + `compressor.py::FROZEN_PREFIX_V7` + `assert_layout_spec_invariant` + `LayoutSpecInvariantError` + 3 NEW golden YAML fixtures + multi-baseline test module
- **F-02 + F-15 closed**, `references/context-isolation.md` §10 12-keys → 16-keys verbatim + §15 → §16 renumber (anchor uniqueness restored)
- **F-03 closed**, `references/message-schemas.md` §2/§3 lean-form mandate per CO-1 / C-2 + NEW §7 Layout Invariant + §8 Compression Rules + §9 Result Status; verbose form preserved as deprecated R5 appendix
- **F-04 body closed**, `references/behavioral-guidelines.md` extension (S-8 Composition + Severity Matrix + v8.2.x Primitive References, ~+64 LOC); per-patch ceiling bumped 240 → 320 to absorb (still 32% of SF-1 Large-tier 1000)
- **F-07 + F-10 closed**, `references/meta-framework.md` §4 alias 12 → 22 templates verbatim + NEW Per-Workflow Template Catalog table; `references/team-roles.md` §7 11-row matrix → 22-row matrix matching all 22 templates from `templates/builtin/`
- **F-08 closed**, `references/decomposition-gate.md` NEW §5.5 7-primitive table (precondition for Theme T5 PV-06 default-on flip)
- **F-09 short-term + long-term closed**, 9 frontmatter `last_updated: "2026-04-23"` bumps (all 12 SF-4 references at 2026-04-23) + NEW `tests/test_reference_frontmatter_freshness.py` 90-day window CI guard
- **F-11 + F-12 + F-13 closed**, `references/execution-protocol.md` NEW §10 (Memory Router fast-path + `DEVOLAFLOW_MEMORY_ROUTER` env-flag) + NEW §11 (Shell Proxy + 5th lifecycle hook + PSC001..PSC004) + NEW §12 (Change-Driven envelope lifecycle + S-8/S-9/A-4 cross-links)
- **+61 NEW tests** (3218 → 3279 total): 23 multi-baseline + 24 frontmatter freshness + 14 API alignment; well under the +25 PV-02 cap forecast (the +25 forecast was for the multi-baseline test module alone; the freshness + API-alignment guards are bonus surface area for F-09 long-term + F-05/F-06 prevention)
- **AGENTS.md regenerated** with A-2 sub-sections A-2.1 through A-2.4 carrying the nest-vs-append clause + decision matrix; `.cursor/rules/repo-governance.mdc` mirrored via `RuleCompiler.compile_all()`
- **NEW v9-ADR-002-cache-layout-governance-v2.md** (~436 LOC) codifies D1-D7 with explicit Migration (M1-M4) + Test Plan + I-8 Invariant Compliance sections; A4 alternative ("no codification") explicitly rejected per the B-02 evidence (4-cycle stale ref + missing baselines)
- **EvoBench composite holds** (`v9.0.0_baseline.json` floor preserved per W-4); `tests/test_benchmarks.py` 36/36 PASS

### Files Changed (per PV-02 owned-files manifest)

**NEW (7 files):**

- `tests/test_layout_invariant_multi_baseline.py`, NEW multi-baseline byte-test module (~+450 LOC, 23 tests; covers v7.0.0/v7.3.0/v8.0.0 P-08/v8.0.0 P-10/v8.3.0 PV-05/v8.4.0 baselines + FROZEN_PREFIX_V7 invariant + D2 LCP-prefix verification)
- `tests/test_reference_frontmatter_freshness.py`, NEW 90-day freshness CI guard (~+220 LOC, 24 tests, 12 parametrised per-file freshness assertions)
- `tests/test_reference_api_alignment.py`, NEW dataclass-vs-source + YAML-vs-schema alignment CI guard (~+330 LOC, 14 tests; F-05/F-06/F-02/F-03 specific regression guards)
- `benchmarks/devolaflow_context/baselines/layout_invariant_v8.0.0.yaml`, NEW v8.0.0 P-08 golden YAML (~70 lines; 14-key payload)
- `benchmarks/devolaflow_context/baselines/layout_invariant_v8.3.0.yaml`, NEW v8.3.0 PV-05 golden YAML (~90 lines; 16-key payload)
- `benchmarks/devolaflow_context/baselines/layout_invariant_v8.4.0.yaml`, NEW v8.4.0 stable golden YAML (~90 lines; 16-key v8.4.0-era payload)
- `.local/research/adr/v9-ADR-002-cache-layout-governance-v2.md`, NEW ADR-002 (~436 LOC; D1-D7 + Migration + Test Plan + I-8 Compliance + Alternatives)

**MODIFIED:**

- `workflow-system/agent/references/context-isolation.md`, F-02 §10 "12 keys" → "16 keys (version: 5)" verbatim + §15 → §16 renumber + frontmatter `last_updated` bump (~+5 LOC)
- `workflow-system/agent/references/message-schemas.md`, F-03 §2/§3 lean-form preface + NEW §7 Layout Invariant + §8 Compression Rules + §9 Result Status + verbose form preserved as deprecated appendix + frontmatter bump (~+71 LOC)
- `workflow-system/agent/references/behavioral-guidelines.md`, F-04 body extension (S-8 Composition + Severity Matrix + v8.2.x Primitives) + frontmatter bump (~+64 LOC)
- `workflow-system/agent/references/meta-framework.md`, F-07 §4 alias 12 → 22 templates + NEW Per-Workflow Template Catalog + frontmatter bump (~+78 LOC)
- `workflow-system/agent/references/decomposition-gate.md`, F-08 NEW §5.5 7-primitive composition table + frontmatter bump (~+33 LOC)
- `workflow-system/agent/references/team-roles.md`, F-10 §7 11 → 22 row participation matrix + frontmatter bump (~+15 LOC)
- `workflow-system/agent/references/execution-protocol.md`, F-11 + F-12 + F-13 NEW §10/§11/§12 + frontmatter bump (~+102 LOC)
- `workflow-system/agent/references/agent-workspace.md`, cross-refs to shell-proxy / execution-protocol §10/§11/§12 + envelope policy clarification (~+11 LOC)
- `workflow-system/agent/references/agent-hierarchy.md`, frontmatter bump only
- `workflow-system/agent/references/repo-modes.md`, frontmatter bump only (PV-08 sustaining bundle handles body edits)
- `schemas/lean-dispatch.yaml#layout_invariant`, D6 doc comments only (~+45 LOC YAML comments documenting D1-D5; NO structural data change; version stays 5; canonical_order stays length 16)
- `src/devolaflow/compressor.py`, D5 `FROZEN_PREFIX_V7` constant + `FROZEN_PREFIX_LENGTH` + `LayoutSpecInvariantError` exception + `assert_layout_spec_invariant()` helper + `assert_dispatch_layout` extended with `enforce_frozen_prefix` keyword arg (default True; opt-out for v6-historical interop) (~+125 LOC including docstrings)
- `.cursor/rules/repo-governance.mdc`, A-2 extended with sub-sections A-2.1 (Frozen Prefix) + A-2.2 (Append-Only Tail) + A-2.3 (Nest-vs-Append Decision Rule with 6-row matrix) + A-2.4 (Multi-Baseline Byte Test); auto-regenerated via `RuleCompiler` from `.rules/architecture.mdc`
- `.rules/architecture.mdc`, A-2 source extended with same sub-sections A-2.1 through A-2.4 (compiler input)
- `AGENTS.md`, regenerated via `RuleCompiler.compile_all()` to mirror the A-2 extension
- `tests/test_behavioral_guidelines.py::test_pv04_section_within_tight_per_patch_line_ceiling`, per-patch ceiling bumped 240 → 320 to absorb F-04 body extension; rationale documented in test docstring (still 32% of SF-1 Large-tier 1000)
- 7 canonical version-sync locations per CP-3 / W-10, `8.4.1` → `8.4.2` (`__init__.py`, `pyproject.toml`, SKILL.md frontmatter + banner + body, `workflow-skill.yaml`, `generate_human_docs.py`, `test_smoke.py`, `README.md` badge + version example, `benchmark-results/index.html` SAMPLE_DATA version)
- `.local/research/v9.0.0_pv02_ref_cascade_plan.md`, NEW S02 design artifact (~330 LOC; 10 ref hunks with verbatim source pointers + cumulative LOC summary)
- `.local/research/v8.4.2_evaluation.md`, SI-3 evaluation report; weighted composite **9.225/10** (≥ 8.5 threshold)
- `.local/research/v8.4.2_nines.{json,md}`, NineS post-PV self-eval; composite **0.9206** (≥ 0.85 threshold per dispatch); 24/25 dimensions ≥ 0.85; code_coverage=0.0 attributable to pytest, cov budget timeout (pre-existing NineS limitation, not v8.4.2 regression)

**DELETED:** none.

### Cross-references

- v9.0.0 SI-1 planning gate: `.local/research/v9.0.0_gap_analysis.md` §5.2 (PV-02 closes B-02 + 9 reference findings)
- v9.0.0 SI-1 reference review: `.local/research/v9.0.0_reference_review.md` (PV-02 closes F-02 + F-03 + F-04 body + F-07 + F-08 + F-09 short+long + F-10 + F-11 + F-12 + F-13 + F-15 = 11 findings)
- v9.0.0 PV-02 implementation plan: `.local/research/v9.0.0_implementation_plan.md` §6.2 (5 stages, 7 waves, 17 tasks)
- v9.0.0 PV-02 cascade plan: `.local/research/v9.0.0_pv02_ref_cascade_plan.md` (S02 design, 10 hunks × verbatim source pointers)
- v9.0.0 PV-02 ADR: `.local/research/adr/v9-ADR-002-cache-layout-governance-v2.md` (D1-D7 + Migration M1-M4 + Test Plan + I-8 Compliance + 4 Alternatives Considered)
- v9.0.0 PV-02 SI-3 evaluation: `.local/research/v8.4.2_evaluation.md`
- v9.0.0 PV-02 NineS post-PV: `.local/research/v8.4.2_nines.{json,md}`
- v9.0.0 PV-01 ADR (predecessor): `.local/research/adr/v9-ADR-001-skill-headroom-reclamation.md`
- v7-ADR-001 cache-layout invariant (predecessor): `.local/research/adr/v7-ADR-001-cache-layout-invariant.md` (the v9-ADR-002 generalisation source)
- DevolaFlow canonical URL (per S-7): https://github.com/YoRHa-Agents/DevolaFlow
- NineS canonical URL (per S-7): https://github.com/YoRHa-Agents/NineS
